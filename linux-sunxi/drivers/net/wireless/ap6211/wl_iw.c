...
)
{
	...
	AP6211_DEBUG("Send IWEVCUSTOM Event as %s\n", extra);
...
)
{
	...
	if ((error = dev_wlc_ioctl(dev, WLC_REASSOC, &bssid, ETHER_ADDR_LEN))) {
		AP6211_ERR("%s: WLC_REASSOC failed (%d)\n", __FUNCTION__, error);
		return error;
	}
...
)
{
	...
	AP6211_DEBUG("%s: SIOCSIWAP\n", dev->name);

	if (awrq->sa_family != ARPHRD_ETHER) {
		AP6211_ERR("%s: Invalid Header...sa_family\n", __FUNCTION__);
		return -EINVAL;
	}
	...
	if (ETHER_ISBCAST(awrq->sa_data) || ETHER_ISNULLADDR(awrq->sa_data)) {
		...
		if ((error = dev_wlc_ioctl(dev, WLC_DISASSOC, &scbval, sizeof(scb_val_t)))) {
			AP6211_ERR("%s: WLC_DISASSOC failed (%d).\n", __FUNCTION__, error);
		}
		return 0;
	}
...
)
{
	...
	AP6211_DEBUG("%s: SIOCSIWMLME\n", dev->name);

	mlme = (struct iw_mlme *)extra;
	if (mlme == NULL) {
		AP6211_ERR("Invalid ioctl data.\n");
		return error;
	}
	...
	if (mlme->cmd == IW_MLME_DISASSOC) {
		...
		AP6211_ERR("%s: Invalid ioctl data.\n", __FUNCTION__);
		return error;
	}
...
)
{
	...
	if ((error = dev_wlc_ioctl(dev, WLC_SCAN_RESULTS, list, buflen))) {
		AP6211_ERR("%d: Scan results error %d\n", __LINE__, error);
		kfree(list);
		return error;
	}
...
)
{
	...
	while (p_buf != iscan->list_cur) {
		...
		AP6211_ERR("list->version %d != WL_BSS_INFO_VERSION\n", list->version);
	    }
...
)
{
	...
	AP6211_DEBUG("%s: SIOCGIWESSID\n", dev->name);

	if (!extra)
		return -EINVAL;

	if ((error = dev_wlc_ioctl(dev, WLC_GET_SSID, &ssid, sizeof(ssid)))) {
		AP6211_ERR("Error getting the SSID\n");
		return error;
	}
...
)
{
	...
	AP6211_DEBUG("%s: SIOCSIWENCODEEXT\n", dev->name);
	...
	if (key.len == 0) {
		if (iwe->ext_flags & IW_ENCODE_EXT_SET_TX_KEY) {
			AP6211_DEBUG("Changing the the primary Key to %d\n", key.index);
	 ...
	 */
		...
		if (iwe->ext_flags & IW_ENCODE_EXT_SET_TX_KEY) {
			AP6211_DEBUG("key is a Primary Key\n");
			key.flags = WL_PRIMARY_KEY;
		}
...
)
{
	...
	AP6211_DEBUG("%s: SIOCSIWPMKSA\n", dev->name);
	iwpmksa = (struct iw_pmksa *)extra;
	bzero((char *)eabuf, ETHER_ADDR_STR_LEN);
	if (iwpmksa->cmd == IW_PMKSA_FLUSH) {
		AP6211_DEBUG("wl_iw_set_pmksa - IW_PMKSA_FLUSH\n");
	...
	if (iwpmksa->cmd == IW_PMKSA_REMOVE) {
		...
		{
			uint j;
			AP6211_DEBUG("wl_iw_set_pmksa,IW_PMKSA_REMOVE - PMKID: %s = ",
				bcm_ether_ntoa(&pmkidptr->pmkid[0].BSSID,
				eabuf));
			for (j = 0; j < WPA2_PMKID_LEN; j++)
				AP6211_DUMP("%02x ", pmkidptr->pmkid[0].PMKID[j]);
			AP6211_DUMP("\n");
		}
	...
	if (iwpmksa->cmd == IW_PMKSA_ADD) {
			...
			WPA2_PMKID_LEN);
		{
			...
			BCM_REFERENCE(k);
			AP6211_DEBUG("wl_iw_set_pmksa,IW_PMKSA_ADD - PMKID: %s = ",
				bcm_ether_ntoa(&pmkid_array[k].BSSID,
				eabuf));
			for (j = 0; j < WPA2_PMKID_LEN; j++)
				AP6211_DUMP("%02x ", pmkid_array[k].PMKID[j]);
			AP6211_DUMP("\n");
		}
		pmkid_list.pmkids.npmkid++;
	}
	AP6211_DEBUG("PRINTING pmkid LIST - No of elements %d\n", pmkid_list.pmkids.npmkid);
	for (i = 0; i < pmkid_list.pmkids.npmkid; i++) {
		uint j;
		AP6211_DEBUG("PMKID[%d]: %s = ", i,
			bcm_ether_ntoa(&pmkid_array[i].BSSID,
			eabuf));
		for (j = 0; j < WPA2_PMKID_LEN; j++)
			AP6211_DUMP("%02x ", pmkid_array[i].PMKID[j]);
		AP6211_DEBUG("\n");
	}
	AP6211_DEBUG("\n");
...
{
	AP6211_DEBUG("%s: SIOCGIWENCODEEXT\n", dev->name);
	return 0;
}
...
)
{
	...
	AP6211_DEBUG("%s: SIOCSIWAUTH\n", dev->name);

	paramid = vwrq->flags & IW_AUTH_INDEX;
	paramval = vwrq->value;

	AP6211_DEBUG("%s: SIOCSIWAUTH, paramid = 0x%0x, paramval = 0x%0x\n",
	...
	case IW_AUTH_WPA_VERSION:
		...
		AP6211_DEBUG("%s: %d: setting wpa_auth to 0x%0x\n", __FUNCTION__, __LINE__, val);
	...
	case IW_AUTH_CIPHER_GROUP:
		...
		if (iw->privacy_invoked && !val) {
			AP6211_DEBUG("%s: %s: 'Privacy invoked' TRUE but clearing wsec, assuming "
			         "we're a WPS enrollee\n", dev->name, __FUNCTION__);
			if ((error = dev_wlc_intvar_set(dev, "is_WPS_enrollee", TRUE))) {
				AP6211_DEBUG("Failed to set iovar is_WPS_enrollee\n");
				return error;
			}
		} else if (val) {
			if ((error = dev_wlc_intvar_set(dev, "is_WPS_enrollee", FALSE))) {
				AP6211_DEBUG("Failed to clear iovar is_WPS_enrollee\n");
				return error;
			}
		}
	...
	case IW_AUTH_KEY_MGMT:
		...
		AP6211_DEBUG("%s: %d: setting wpa_auth to %d\n", __FUNCTION__, __LINE__, val);
		if ((error = dev_wlc_intvar_set(dev, "wpa_auth", val)))
			return error;
		break;
	...
	case IW_AUTH_80211_AUTH_ALG:
		/* open shared */
		AP6211_ERR("Setting the D11auth %d\n", paramval);
	...
	case IW_AUTH_ROAMING_CONTROL:
		AP6211_DEBUG("%s: IW_AUTH_ROAMING_CONTROL\n", __FUNCTION__);
...
)
{
	...
	case IW_AUTH_ROAMING_CONTROL:
		AP6211_ERR("%s: IW_AUTH_ROAMING_CONTROL\n", __FUNCTION__);
		/* driver control or user space app control */
		break;
...
#	define WL_IW_DONT_CARE	9999
	...
	if (cause) {
		...
		AP6211_DEBUG("Connection status: %s\n", stringBuf);
		return TRUE;
	} else {
		return FALSE;
	}
...
wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void* data)
{
	...
	case WLC_E_ACTION_FRAME:
		...
		if (datalen + 1 <= sizeof(extra)) {
			...
			AP6211_DEBUG("WLC_E_ACTION_FRAME len %d \n", wrqu.data.length);
		}
		break;
	...
	case WLC_E_ACTION_FRAME_COMPLETE:
		...
		if (sizeof(status) + 1 <= sizeof(extra)) {
			...
			AP6211_DEBUG("wl_iw_event status %d  \n", status);
		}
		break;
#endif /* WIRELESS_EXT > 14 */
#if WIRELESS_EXT > 17
	...
	case WLC_E_SCAN_COMPLETE:
		...
		cmd = SIOCGIWSCAN;
#endif
		AP6211_DEBUG("event WLC_E_SCAN_COMPLETE\n");
...
int wl_iw_get_wireless_stats(struct net_device *dev, struct iw_statistics *wstats)
{
	...
	AP6211_DEBUG("wl_iw_get_wireless_stats phy noise=%d\n *****", phy_noise);
	...
	AP6211_DEBUG("wl_iw_get_wireless_stats rssi=%d ****** \n", rssi);
	if (rssi <= WL_IW_RSSI_NO_SIGNAL)
		wstats->qual.qual = 0;
	else if (rssi <= WL_IW_RSSI_VERY_LOW)
		wstats->qual.qual = 1;
...
#if WIRELESS_EXT > 11
	AP6211_DEBUG("wl_iw_get_wireless_stats counters=%d\n *****", (int)sizeof(wl_cnt_t));

	memset(&cnt, 0, sizeof(wl_cnt_t));
	res = dev_wlc_bufvar_get(dev, "counters", (char *)&cnt, sizeof(wl_cnt_t));
	if (res)
	{
		AP6211_ERR("wl_iw_get_wireless_stats counters failed error=%d ****** \n", res);
		goto done;
	}

	cnt.version = dtoh16(cnt.version);
	if (cnt.version != WL_CNT_T_VERSION) {
		AP6211_DEBUG("\tIncorrect version of counters struct: expected %d; got %d\n",
	...
	AP6211_DEBUG("wl_iw_get_wireless_stats counters txframe=%d txbyte=%d\n",
		dtoh32(cnt.txframe), dtoh32(cnt.txbyte));
	AP6211_DEBUG("wl_iw_get_wireless_stats counters rxfrmtoolong=%d\n", dtoh32(cnt.rxfrmtoolong));
	AP6211_DEBUG("wl_iw_get_wireless_stats counters rxbadplcp=%d\n", dtoh32(cnt.rxbadplcp));
	AP6211_DEBUG("wl_iw_get_wireless_stats counters rxundec=%d\n", dtoh32(cnt.rxundec));
	AP6211_DEBUG("wl_iw_get_wireless_stats counters rxfragerr=%d\n", dtoh32(cnt.rxfragerr));
	AP6211_DEBUG("wl_iw_get_wireless_stats counters txfail=%d\n", dtoh32(cnt.txfail));
	AP6211_DEBUG("wl_iw_get_wireless_stats counters rxrunt=%d\n", dtoh32(cnt.rxrunt));
	AP6211_DEBUG("wl_iw_get_wireless_stats counters rxgiant=%d\n", dtoh32(cnt.rxgiant));
	...
	while (down_interruptible(&iscan->sysioc_sem) == 0) {
		...
		switch (status) {
			case WL_SCAN_RESULTS_PARTIAL:
				AP6211_DEBUG("iscanresults incomplete\n");
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27))
				rtnl_lock();
#endif
			...
			case WL_SCAN_RESULTS_SUCCESS:
				AP6211_DEBUG("iscanresults complete\n");
				iscan->iscan_state = ISCAN_STATE_IDLE;
				wl_iw_send_scan_complete(iscan);
				break;
			case WL_SCAN_RESULTS_PENDING:
				AP6211_DEBUG("iscanresults pending\n");
				/* Reschedule the timer */
				iscan->timer.expires = jiffies + msecs_to_jiffies(iscan->timer_ms);
				add_timer(&iscan->timer);
				iscan->timer_on = 1;
			...
			case WL_SCAN_RESULTS_ABORTED:
				AP6211_DEBUG("iscanresults aborted\n");
				iscan->iscan_state = ISCAN_STATE_IDLE;
				wl_iw_send_scan_complete(iscan);
				break;
			default:
				AP6211_DEBUG("iscanresults returned unknown status %d\n", status);
				break;
		 }
	}
