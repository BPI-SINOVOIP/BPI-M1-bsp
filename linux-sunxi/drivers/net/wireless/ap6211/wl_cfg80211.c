...
do {									\
	...
	if (unlikely(!wl_get_drv_status(wlpriv, READY, ndev))) {	\
		AP6211_DEBUG("device is not ready\n");			\
		return -EIO;						\
	}								\
} while (0)
...
wl_chspec_from_legacy(chanspec_t legacy_chspec)
{
	...
	if (wf_chspec_malformed(chspec)) {
		AP6211_ERR("wl_chspec_from_legacy: output chanspec (0x%04X) malformed\n",
		        chspec);
		return INVCHANSPEC;
	}
...
wl_chspec_to_legacy(chanspec_t chspec)
{
	...
	if (wf_chspec_malformed(chspec)) {
		AP6211_ERR("wl_chspec_to_legacy: input chanspec (0x%04X) malformed\n",
		        chspec);
		return INVCHANSPEC;
	}
...
wl_validate_wps_ie(char *wps_ie, s32 wps_ie_len, bool *pbc)
{
	...
	if (wps_ie == NULL || wps_ie_len < WPS_IE_FIXED_LEN) {
		AP6211_ERR("invalid argument : NULL\n"));
		return;
	}
	len = (u16)wps_ie[TLV_LEN_OFF];

	if (len > wps_ie_len) {
		AP6211_ERR("invalid length len %d, wps ie len %d\n", len, wps_ie_len);
		return;
	}
	AP6211_DEBUG("wps_ie len=%d\n", len);
	...
	while (len >= 4) {		/* must have attr id, attr len fields */
		...
		AP6211_DEBUG(" subel=%p, subelt_id=0x%x subelt_len=%u\n",
			subel, subelt_id, subelt_len);

		if (subelt_id == WPS_ID_VERSION) {
			AP6211_DEBUG("  attr WPS_ID_VERSION: %u\n", *subel);
		} else if (subelt_id == WPS_ID_REQ_TYPE) {
			AP6211_DEBUG("  attr WPS_ID_REQ_TYPE: %u\n", *subel);
		} else if (subelt_id == WPS_ID_CONFIG_METHODS) {
			valptr[0] = *subel;
			valptr[1] = *(subel + 1);
			AP6211_DEBUG("  attr WPS_ID_CONFIG_METHODS: %x\n", HTON16(val));
		} else if (subelt_id == WPS_ID_DEVICE_NAME) {
			char devname[100];
			memcpy(devname, subel, subelt_len);
			devname[subelt_len] = '\0';
			AP6211_DEBUG("  attr WPS_ID_DEVICE_NAME: %s (len %u)\n",
				devname, subelt_len);
		} else if (subelt_id == WPS_ID_DEVICE_PWD_ID) {
			valptr[0] = *subel;
			valptr[1] = *(subel + 1);
			AP6211_DEBUG("  attr WPS_ID_DEVICE_PWD_ID: %u\n", HTON16(val));
			*pbc = (HTON16(val) == DEV_PW_PUSHBUTTON) ? true : false;
		} else if (subelt_id == WPS_ID_PRIM_DEV_TYPE) {
			valptr[0] = *subel;
			valptr[1] = *(subel + 1);
			AP6211_DEBUG("  attr WPS_ID_PRIM_DEV_TYPE: cat=%u \n", HTON16(val));
			valptr[0] = *(subel + 6);
			valptr[1] = *(subel + 7);
			AP6211_DEBUG("  attr WPS_ID_PRIM_DEV_TYPE: subcat=%u\n", HTON16(val));
		} else if (subelt_id == WPS_ID_REQ_DEV_TYPE) {
			valptr[0] = *subel;
			valptr[1] = *(subel + 1);
			AP6211_DEBUG("  attr WPS_ID_REQ_DEV_TYPE: cat=%u\n", HTON16(val));
			valptr[0] = *(subel + 6);
			valptr[1] = *(subel + 7);
			AP6211_DEBUG("  attr WPS_ID_REQ_DEV_TYPE: subcat=%u\n", HTON16(val));
		} else if (subelt_id == WPS_ID_SELECTED_REGISTRAR_CONFIG_METHODS) {
			valptr[0] = *subel;
			valptr[1] = *(subel + 1);
			AP6211_DEBUG("  attr WPS_ID_SELECTED_REGISTRAR_CONFIG_METHODS"
				": cat=%u\n", HTON16(val));
		} else {
			AP6211_DEBUG("  unknown attr 0x%x\n", subelt_id);
		}
...
static chanspec_t wl_cfg80211_get_shared_freq(struct wiphy *wiphy)
{
	...
	if ((err = wldev_ioctl(dev, WLC_GET_BSSID, &bssid, sizeof(bssid), false))) {
		 ...
		 */
		AP6211_DEBUG("Not associated. Return a temp channel. \n");
	...
	*(u32 *) wl->extra_buf = htod32(WL_EXTRA_BUF_MAX);
	if ((err = wldev_ioctl(dev, WLC_GET_BSS_INFO, wl->extra_buf,
		WL_EXTRA_BUF_MAX, false))) {
			AP6211_ERR("Failed to get associated bss info, use temp channel \n");
			chspec = wl_ch_host_to_driver(WL_P2P_TEMP_CHAN);
	}
	else {
			bss = (struct wl_bss_info *) (wl->extra_buf + 4);
			chspec =  bss->chanspec;
			AP6211_DEBUG("Valid BSS Found. chanspec:%d \n", chspec);
	}
	return chspec;
}
...
static struct net_device* wl_cfg80211_add_monitor_if(char *name)
{
	...
	AP6211_DEBUG("wl_cfg80211_add_monitor_if: No more support monitor interface\n");
	return ERR_PTR(-EOPNOTSUPP);
#else
	struct net_device* ndev = NULL;

	dhd_add_monitor(name, &ndev);
	AP6211_DEBUG("wl_cfg80211_add_monitor_if net device returned: 0x%p\n", ndev);
	return ndev;
	...
	struct vif_params *params)
{
	...
	AP6211_DEBUG("if name: %s, type: %d\n", name, type);
	switch (type) {
	case NL80211_IFTYPE_ADHOC:
	case NL80211_IFTYPE_AP_VLAN:
	case NL80211_IFTYPE_WDS:
	case NL80211_IFTYPE_MESH_POINT:
		AP6211_ERR("Unsupported interface type\n");
		mode = WL_MODE_IBSS;
		return NULL;
	...
	if (wl->p2p_supported && (wlif_type != -1)) {
		if (wl_get_p2p_status(wl, IF_DELETING)) {
			...
			AP6211_DEBUG("%s: Released the lock and wait till IF_DEL is complete\n",
				__func__);
			...
			if (timeout > 0) {
				AP6211_ERR("IF DEL is Success\n");

			} else {
				AP6211_ERR("timeount < 0, return -EAGAIN\n");
		...
		if (!wl->wlfc_on && !disable_proptx) {
			...
			if (err < 0)
				AP6211_ERR("WLC_UP return err:%d\n", err);
			wl->wlfc_on = true;
		}
#endif /* PROP_TXSTATUS_VSDB */
		...
		if (unlikely(err)) {
			AP6211_ERR(" virtual iface add failed (%d) \n", err);
			return ERR_PTR(-ENOMEM);
		}
		...
		if (timeout > 0 && (!wl_get_p2p_status(wl, IF_ADD))) {
			...
			if (unlikely(!vwdev)) {
				AP6211_ERR("Could not allocate wireless device\n");
				return ERR_PTR(-ENOMEM);
			}
			vwdev->wiphy = wl->wdev->wiphy;
			AP6211_DEBUG(" virtual interface(%s) is created memalloc done \n",
				wl->p2p->vir_ifname);
			vwdev->iftype = type;
			...
			if (net_attach && !net_attach(wl->pub, _ndev->ifindex)) {
				...
				AP6211_ERR(" virtual interface(%s) is "
			...
			AP6211_ERR(" virtual interface(%s) is not created \n", wl->p2p->vir_ifname);
			memset(wl->p2p->vir_ifname, '\0', IFNAMSIZ);
	...
	if (wl->p2p_supported) {
		...
		if (wl->p2p->vif_created) {
			...
			if (wl_get_drv_status(wl, DISCONNECTING, dev) &&
				...
				AP6211_ERR("Wait for Link Down event for GC !\n");
			...
			if (wl_get_mode_by_netdev(wl, dev) == WL_MODE_AP) {
				 ...
				 */
				if (ret == 0) {
					AP6211_ERR("Wait for Link Down event for GO !!!\n");
			...
			*/
			...
			if (ret) {
				...
				AP6211_ERR("Firmware returned an error (%d) from p2p_ifdel"
			...
			if (timeout > 0 && (wl->p2p->vif_created == false)) {
				AP6211_DEBUG("IFDEL operation done\n");
			} else {
				AP6211_ERR("IFDEL didn't complete properly\n");
			}
	...
	struct vif_params *params)
{
	...
	AP6211_DEBUG("Enter type %d\n", type);
	switch (type) {
	case NL80211_IFTYPE_MONITOR:
	case NL80211_IFTYPE_WDS:
	case NL80211_IFTYPE_MESH_POINT:
		ap = 1;
		AP6211_ERR("type (%d) : currently we do not support this type\n",
			type);
		break;
	case NL80211_IFTYPE_ADHOC:
		mode = WL_MODE_IBSS;
	...
	if (ap) {
		...
		if (wl->p2p_supported && wl->p2p->vif_created) {
			AP6211_DEBUG("p2p_vif_created (%d) p2p_on (%d)\n", wl->p2p->vif_created,
			p2p_on(wl));
			...
			wlif_type = WL_P2P_IF_GO;
			AP6211_ERR("%s : ap (%d), infra (%d), iftype: (%d)\n",
			...
			if (!wl->ap_info &&
				...
				AP6211_ERR("struct ap_saved_ie allocation failed\n");
				return -ENOMEM;
			}
		} else {
			AP6211_ERR("Cannot change the interface for GO or SOFTAP\n");
			return -EINVAL;
		}
	} else {
		AP6211_DEBUG("Change_virtual_iface for transition from GO/AP to client/STA");
	}
	...
	void* _net_attach)
{
	...
	AP6211_DEBUG("Enter");
	if (!ndev) {
		AP6211_ERR("net is NULL\n");
		return 0;
	}
	if (wl->p2p_supported && wl_get_p2p_status(wl, IF_ADD)) {
		AP6211_DEBUG("IF_ADD event called from dongle, old interface name: %s,"
...
wl_cfg80211_ifdel_ops(struct net_device *ndev)
{
	...
	if (p2p_is_on(wl) && wl->p2p->vif_created &&
		...
		if (wl->scan_request &&
			...
			AP6211_DEBUG("ESCAN COMPLETED\n");
			wl_notify_escan_complete(wl, ndev, true, false);
			if (rollback_lock)
				rtnl_unlock();
		}
		AP6211_ERR("IF_DEL event called from dongle, net %x, vif name: %s\n",
	...
	u8 *ie, u32 ie_len)
{
	...
	AP6211_DEBUG(" found p2p ie ! lenth %d \n",
		p2p_ie->len);

	while (pos < end) {
		uint16 attr_len;
		if (pos + 2 >= end) {
			AP6211_DEBUG(" -- Invalid P2P attribute");
			return 0;
		}
		attr_len = ((uint16) (((pos + 1)[1] << 8) | (pos + 1)[0]));

		if (pos + 3 + attr_len > end) {
			AP6211_DEBUG("P2P: Attribute underflow "
				   "(len=%u left=%d)",
...
static void wl_scan_prep(struct wl_scan_params *params, struct cfg80211_scan_request *request)
{
	...
	AP6211_DEBUG("Preparing Scan request\n");
	AP6211_DEBUG("nprobes=%d\n", params->nprobes);
	AP6211_DEBUG("active_time=%d\n", params->active_time);
	AP6211_DEBUG("passive_time=%d\n", params->passive_time);
	AP6211_DEBUG("home_time=%d\n", params->home_time);
	AP6211_DEBUG("scan_type=%d\n", params->scan_type);
	...
	AP6211_DEBUG("### List of channelspecs to scan ###\n");
	if (n_channels > 0) {
		for (i = 0; i < n_channels; i++) {
			...
			if (request->channels[i]->band == IEEE80211_BAND_2GHZ) {
				...
				if (wl->curr_band == WLC_BAND_5G) {
					AP6211_DEBUG("In 5G only mode, omit 2G channel:%d\n", channel);
					continue;
				}
				...
				if (wl->curr_band == WLC_BAND_2G) {
					AP6211_DEBUG("In 2G only mode, omit 5G channel:%d\n", channel);
					continue;
				}
#endif /* WL_HOST_BAND_MGMT */
				chanspec |= WL_CHANSPEC_BAND_5G;
			...
			AP6211_DEBUG("Chan : %d, Channel spec: %x \n",
				channel, params->channel_list[j]);
			params->channel_list[j] = wl_chspec_host_to_driver(params->channel_list[j]);
			j++;
		}
	} else {
		AP6211_DEBUG("Scanning all channels\n");
	}
	n_channels = j;
	/* Copy ssid array if applicable */
	AP6211_DEBUG("### List of SSIDs to scan ###\n");
	if (n_ssids > 0) {
		...
		for (i = 0; i < n_ssids; i++) {
			...
			if (!ssid.SSID_len)
				AP6211_DEBUG("%d: Broadcast scan\n", i);
			else
				AP6211_DEBUG("%d: scan  for  %s size =%d\n", i,
				ssid.SSID, ssid.SSID_len);
			memcpy(ptr, &ssid, sizeof(wlc_ssid_t));
			ptr += sizeof(wlc_ssid_t);
		}
	} else {
		AP6211_DEBUG("Broadcast scan\n");
	}
...
wl_run_iscan(struct wl_iscan_ctrl *iscan, struct cfg80211_scan_request *request, u16 action)
{
	...
	if (unlikely(err)) {
		if (err == -EBUSY) {
			AP6211_ERR("system busy : iscan canceled\n");
		} else {
			AP6211_ERR("error (%d)\n", err);
		}
	}
...
wl_get_valid_channels(struct net_device *ndev, u8 *valid_chan_list, s32 size)
{
	...
	if (err != 0) {
		AP6211_ERR("get channels failed with %d\n", err);
	}
	...
	struct cfg80211_scan_request *request, uint16 action)
{
	...
	if (!wl->p2p_supported || !p2p_scan(wl)) {
		/* LEGACY SCAN TRIGGER */
		AP6211_DEBUG(" LEGACY E-SCAN START\n");
		...
		if (params_size + sizeof("escan") >= WLC_IOCTL_MEDLEN) {
			AP6211_ERR("ioctl buffer length not sufficient\n");
			kfree(params);
			err = -ENOMEM;
			goto exit;
		}
		...
		if (unlikely(err)) {
			if (err == BCME_EPERM)
				...
				AP6211_DEBUG(" Escan not permitted at this time (%d)\n", err);
			else
				AP6211_ERR(" Escan set error (%d)\n", err);
		}
		kfree(params);
	}
		...
		if (request && request->n_channels) {
			...
			AP6211_DEBUG(" chann number : %d\n", num_chans);
			default_chan_list = kzalloc(num_chans * sizeof(*default_chan_list),
				GFP_KERNEL);
			if (default_chan_list == NULL) {
				AP6211_ERR("channel list allocation failed \n");
				err = -ENOMEM;
				goto exit;
			}
			...
			if (num_chans == 3 && (
				...
				AP6211_DEBUG("P2P SEARCH PHASE START \n");
			} else if ((dev = wl_to_p2p_bss_ndev(wl, P2PAPI_BSSCFG_CONNECTION)) &&
				(wl_get_mode_by_netdev(wl, dev) == WL_MODE_AP)) {
				/* If you are already a GO, then do SEARCH only */
				AP6211_DEBUG("Already a GO. Do SEARCH Only");
				search_state = WL_P2P_DISC_ST_SEARCH;
				num_chans = n_nodfs;

			} else {
				AP6211_DEBUG("P2P SCAN STATE START \n");
				num_chans = n_nodfs;
			}
...
exit:
	if (unlikely(err)) {
		...
		if ((err == BCME_EPERM) && wl->scan_suppressed)
			AP6211_DEBUG("Escan failed: Scan Suppressed \n");
		else
			AP6211_ERR("error (%d)\n", err);
	}
	return err;
	...
	struct cfg80211_ssid *this_ssid)
{
	...
	if (WL_DRV_STATUS_SENDING_AF_FRM_EXT(wl)) {
		AP6211_ERR("Sending Action Frames. Try it again.\n");
		return -EAGAIN;
	}

	AP6211_DEBUG("Enter wiphy (%p)\n", wiphy);
	if (wl_get_drv_status_all(wl, SCANNING)) {
		if (wl->scan_request == NULL) {
			wl_clr_drv_status_all(wl, SCANNING);
			AP6211_DEBUG("<<<<<<<<<<<Force Clear Scanning Status>>>>>>>>>>>\n");
		} else {
			AP6211_ERR("Scanning already\n");
			return -EAGAIN;
		}
	}
	if (wl_get_drv_status(wl, SCAN_ABORTING, ndev)) {
		AP6211_ERR("Scanning being aborted\n");
		return -EAGAIN;
	}
	if (request && request->n_ssids > WL_SCAN_PARAMS_SSID_MAX) {
		AP6211_ERR("request null or n_ssids > WL_SCAN_PARAMS_SSID_MAX\n");
		return -EOPNOTSUPP;
	}
#ifdef WL_CFG80211_VSDB_PRIORITIZE_SCAN_REQUEST
	if (wl_get_drv_status_all(wl, REMAINING_ON_CHANNEL)) {
		AP6211_DEBUG("Remain_on_channel bit is set, somehow it didn't get cleared\n");
		wl_notify_escan_complete(wl, ndev, true, true);
	}
	...
	if (iscan_req) {
		...
		if (sr->ssid.SSID_len) {
			...
			AP6211_DEBUG("Specific scan ssid=\"%s\" len=%d\n",
		...
		err = wldev_ioctl(ndev, WLC_SET_PASSIVE_SCAN,
			&passive_scan, sizeof(passive_scan), true);
		if (unlikely(err)) {
			AP6211_DEBUG("WLC_SET_PASSIVE_SCAN error (%d)\n", err);
			goto scan_out;
		}
		...
		if (err) {
			if (err == -EBUSY) {
				AP6211_ERR("system busy : scan for \"%s\" "
					"canceled\n", sr->ssid.SSID);
			} else {
				AP6211_ERR("WLC_SCAN error (%d)\n", err);
			}
			goto scan_out;
		}
	}
...
scan_out:
	...
	if (err == BCME_BUSY || err == BCME_NOTREADY) {
		AP6211_ERR("Scan err = (%d), busy?%d", err, -EBUSY);
		err = -EBUSY;
	}
...
#define SCAN_EBUSY_RETRY_LIMIT 10
	if (err == -EBUSY) {
		if (busy_count++ > SCAN_EBUSY_RETRY_LIMIT) {
			...
			s32 ret = 0;
			busy_count = 0;
			AP6211_ERR("Unusual continuous EBUSY error, %d %d %d %d %d %d %d %d %d\n",
			...
			if ((ret = wldev_ioctl(ndev, WLC_GET_BSSID,
				...
				AP6211_ERR("FW is connected with " MACDBG "/n",
				MAC2STRDBG(bssid.octet));
			else
				AP6211_ERR("GET BSSID failed with %d\n", ret);
	...
	struct cfg80211_scan_request *request)
{
	...
	if (unlikely(err)) {
		if ((err == BCME_EPERM) && wl->scan_suppressed)
			AP6211_DEBUG("scan not permitted at this time (%d)\n", err);
		else
			AP6211_ERR("scan error (%d)\n", err);
		return err;
	}
...
wl_set_wpa_version(struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	if (sme->crypto.wpa_versions & NL80211_WAPI_VERSION_1) {
		AP6211_DEBUG(" * wl_set_wpa_version, set wpa_auth"
			" to WPA_AUTH_WAPI 0x400");
		val = WAPI_AUTH_PSK; /* | WAPI_AUTH_UNSPECIFIED; */
	}
#endif
	AP6211_DEBUG("setting wpa_auth to 0x%0x\n", val);
	err = wldev_iovar_setint_bsscfg(dev, "wpa_auth", val, bssidx);
	if (unlikely(err)) {
		AP6211_ERR("set wpa_auth failed (%d)\n", err);
		return err;
	}
...
wl_set_set_wapi_ie(struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	AP6211_DEBUG(" %s \n", __FUNCTION__);
 	...
 	if (sme->crypto.wpa_versions & NL80211_WAPI_VERSION_1) {
		...
		if (unlikely(err)) {
			AP6211_ERR("===> set_wapi_ie Error (%d)\n", err);
			return err;
		}
	} else
		AP6211_DEBUG(" * skip \n");
...
wl_set_auth_type(struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	case NL80211_AUTHTYPE_OPEN_SYSTEM:
		...
		AP6211_DEBUG("open system\n");
		break;
	case NL80211_AUTHTYPE_SHARED_KEY:
		val = WL_AUTH_SHARED_KEY;
		AP6211_DEBUG("shared key\n");
		break;
	case NL80211_AUTHTYPE_AUTOMATIC:
		val = WL_AUTH_OPEN_SHARED;
		AP6211_DEBUG("automatic\n");
		break;
	default:
		val = WL_AUTH_OPEN_SHARED;
		AP6211_ERR("invalid auth type (%d)\n", sme->auth_type);
		break;
	}

	err = wldev_iovar_setint_bsscfg(dev, "auth", val, bssidx);
	if (unlikely(err)) {
		AP6211_ERR("set auth failed (%d)\n", err);
		return err;
	}
	sec = wl_read_prof(wl, dev, WL_PROF_SEC);
...
#endif
		default:
			AP6211_ERR("invalid cipher pairwise (%d)\n",
...
#endif
		default:
			AP6211_ERR("invalid cipher group (%d)\n",
				sme->crypto.cipher_group);
			return -EINVAL;
		}
	}

	AP6211_DEBUG("pval (%d) gval (%d)\n", pval, gval);
	...
	if (is_wps_conn(sme)) {
		if (sme->privacy)
			err = wldev_iovar_setint_bsscfg(dev, "wsec", 4, bssidx);
				...
				AP6211_DEBUG(" NO, is_wps_conn, WAPI set to SMS4_ENABLED");
			...
			AP6211_DEBUG(" NO, is_wps_conn, Set pval | gval to WSEC");
...
wl_set_key_mgmt(struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	if (sme->crypto.n_akm_suites) {
		...
		if (unlikely(err)) {
			AP6211_ERR("could not get wpa_auth (%d)\n", err);
			return err;
		}
		if (val & (WPA_AUTH_PSK |
			WPA_AUTH_UNSPECIFIED)) {
...
#endif
		AP6211_DEBUG("setting wpa_auth to %d\n", val);

		err = wldev_iovar_setint_bsscfg(dev, "wpa_auth", val, bssidx);
		if (unlikely(err)) {
			AP6211_ERR("could not set wpa_auth (%d)\n", err);
			return err;
		}
	}
	sec = wl_read_prof(wl, dev, WL_PROF_SEC);
	...
	struct cfg80211_connect_params *sme)
{
	...
	AP6211_DEBUG("key len (%d)\n", sme->key_len);
	if (sme->key_len) {
		sec = wl_read_prof(wl, dev, WL_PROF_SEC);
		AP6211_DEBUG("wpa_versions 0x%x cipher_pairwise 0x%x\n",
		...
		)))
		{
			...
			if (unlikely(key.len > sizeof(key.data))) {
				AP6211_ERR("Too long key length (%u)\n", key.len);
				return -EINVAL;
			}
...
#endif
			default:
				AP6211_ERR("Invalid algorithm (%d)\n",
					sme->crypto.ciphers_pairwise[0]);
				return -EINVAL;
			}
			/* Set the new key/index */
			AP6211_DEBUG("key length (%d) key index (%d) algo (%d)\n",
			...
			if (unlikely(err)) {
				AP6211_ERR("WLC_SET_KEY error (%d)\n", err);
				return err;
			}
			if (sec->auth_type == NL80211_AUTHTYPE_SHARED_KEY) {
				AP6211_DEBUG("set auth_type to shared key\n");
	...
	struct cfg80211_connect_params *sme)
{
	...
	if (!wl_get_drv_status(wl, CONNECTED, dev)&&
		...
		if (!ETHER_ISNULLADDR(&bssid)) {
			...
			AP6211_DEBUG("drv status CONNECTED is not set, but connected in FW!" MACDBG "\n",
				MAC2STRDBG(bssid.octet));
			...
			while (wl_get_drv_status(wl, DISCONNECTING, dev)) {
				AP6211_ERR("Waiting for disconnection terminated.\n");
				msleep(20);
			}
		} else
			AP6211_DEBUG("Currently not associated!\n");
	}
	...
	if (p2p_is_on(wl) && (dev != wl_to_prmry_ndev(wl))) {
		...
		if ((wpa2_ie = bcm_parse_tlvs((u8 *)sme->ie, sme->ie_len,
			...
			AP6211_DEBUG(" WPA2 IE is found\n");
		}
		/* find the WPA_IE */
		if ((wpa_ie = wl_cfgp2p_find_wpaie((u8 *)sme->ie,
			sme->ie_len)) != NULL) {
			AP6211_DEBUG(" WPA IE is found\n");
		}
	...
	if (chan) {
		wl->channel = ieee80211_frequency_to_channel(chan->center_freq);
		chan_cnt = 1;
		AP6211_DEBUG("channel (%d), center_req (%d), %d channels\n", wl->channel,
			chan->center_freq, chan_cnt);
	} else
		wl->channel = 0;

#ifdef BCMWAPI_WPI
	AP6211_DEBUG("1. enable wapi auth\n");
	if (sme->crypto.wpa_versions & NL80211_WAPI_VERSION_1) {
		AP6211_DEBUG("2. set wapi ie  \n");
		err = wl_set_set_wapi_ie(dev, sme);
		if (unlikely(err))
			return err;
	} else
		AP6211_DEBUG("2. Not wapi ie  \n");
#endif
	AP6211_DEBUG("ie (%p), ie_len (%zd)\n", sme->ie, sme->ie_len);
	AP6211_DEBUG("3. set wapi version \n");
	err = wl_set_wpa_version(dev, sme);
	if (unlikely(err)) {
		AP6211_ERR("Invalid wpa_version\n");
		return err;
	}
#ifdef BCMWAPI_WPI
	if (sme->crypto.wpa_versions & NL80211_WAPI_VERSION_1)
		AP6211_DEBUG("4. WAPI Dont Set wl_set_auth_type\n");
	else {
		AP6211_DEBUG("4. wl_set_auth_type\n");
#endif
		err = wl_set_auth_type(dev, sme);
		if (unlikely(err)) {
			AP6211_ERR("Invalid auth type\n");
			return err;
		}
#ifdef BCMWAPI_WPI
	...
	if (unlikely(err)) {
		AP6211_ERR("Invalid ciper\n");
		return err;
	}

	err = wl_set_key_mgmt(dev, sme);
	if (unlikely(err)) {
		AP6211_ERR("Invalid key mgmt\n");
		return err;
	}

	err = wl_set_set_sharedkey(dev, sme);
	if (unlikely(err)) {
		AP6211_ERR("Invalid shared key\n");
		return err;
	}
	...
	*/
	...
	if (err) {
		...
		if (err == BCME_UNSUPPORTED) {
			AP6211_DEBUG("join iovar is not supported\n");
			goto set_ssid;
		} else
			AP6211_ERR("error (%d)\n", err);
	} else
...
set_ssid:
	...
	AP6211_DEBUG("join_param_size %d\n", join_params_size);
	...
	enum nl80211_tx_power_setting type, s32 dbm)
{
	...
	case NL80211_TX_POWER_LIMITED:
		if (dbm < 0) {
			AP6211_ERR("TX_POWER_LIMITTED - dbm is negative\n");
			return -EINVAL;
		}
		break;
	case NL80211_TX_POWER_FIXED:
		if (dbm < 0) {
			AP6211_ERR("TX_POWER_FIXED - dbm is negative..\n");
			return -EINVAL;
		}
		break;
	}
	...
	if (unlikely(err)) {
		AP6211_ERR("WLC_SET_RADIO error (%d)\n", err);
		return err;
	}
	...
	u8 key_idx, bool unicast, bool multicast)
{
	...
	AP6211_DEBUG("key index (%d)\n", key_idx);
	CHECK_SYS_UP(wl);
	err = wldev_iovar_getint_bsscfg(dev, "wsec", &wsec, bssidx);
	if (unlikely(err)) {
		AP6211_ERR("WLC_GET_WSEC error (%d)\n", err);
		return err;
	}
	if (wsec & WEP_ENABLED) {
	...
	u8 key_idx, const u8 *mac_addr, struct key_params *params)
{
	...
	if (key.len == 0) {
		...
		err = wldev_iovar_setbuf_bsscfg(dev, "wsec_key", &key, sizeof(key),
			wl->ioctl_buf, WLC_IOCTL_MAXLEN, bssidx, &wl->ioctl_buf_sync);
		if (unlikely(err)) {
			AP6211_ERR("key delete error (%d)\n", err);
			return err;
		}
	} else {
		if (key.len > sizeof(key.data)) {
			AP6211_ERR("Invalid key length (%d)\n", key.len);
			return -EINVAL;
		}
		AP6211_DEBUG("Setting the key index %d\n", key.index);
		...
		case WLAN_CIPHER_SUITE_WEP40:
			key.algo = CRYPTO_ALGO_WEP1;
			AP6211_DEBUG("WLAN_CIPHER_SUITE_WEP40\n");
			break;
		case WLAN_CIPHER_SUITE_WEP104:
			key.algo = CRYPTO_ALGO_WEP128;
			AP6211_DEBUG("WLAN_CIPHER_SUITE_WEP104\n");
			break;
		case WLAN_CIPHER_SUITE_TKIP:
			key.algo = CRYPTO_ALGO_TKIP;
			AP6211_DEBUG("WLAN_CIPHER_SUITE_TKIP\n");
			break;
		case WLAN_CIPHER_SUITE_AES_CMAC:
			key.algo = CRYPTO_ALGO_AES_CCM;
			AP6211_DEBUG("WLAN_CIPHER_SUITE_AES_CMAC\n");
			break;
		case WLAN_CIPHER_SUITE_CCMP:
			key.algo = CRYPTO_ALGO_AES_CCM;
			AP6211_DEBUG("WLAN_CIPHER_SUITE_CCMP\n");
			break;
#ifdef BCMWAPI_WPI
		case WLAN_CIPHER_SUITE_SMS4:
			key.algo = CRYPTO_ALGO_SMS4;
			AP6211_DEBUG("WLAN_CIPHER_SUITE_SMS4\n");
			break;
#endif
		default:
			AP6211_ERR("Invalid cipher (0x%x)\n", params->cipher);
			return -EINVAL;
		}
		swap_key_from_BE(&key);
	...
	struct key_params *params)
{
	...
	case WLAN_CIPHER_SUITE_SMS4:
		key.algo = CRYPTO_ALGO_SMS4;
		val = SMS4_ENABLED;
		AP6211_DEBUG(" * wl_cfg80211_add_key, set key "
	...
	if (unlikely(err)) {
		if (err == -EINVAL) {
			if (key.index >= DOT11_MAX_DEFAULT_KEYS) {
				...
				AP6211_DEBUG("invalid key index (%d)\n", key_idx);
			}
		} else {
			AP6211_ERR("WLC_SET_KEY error (%d)\n", err);
		}
	...
	u8 *mac, struct station_info *sinfo)
{
	...
	if (wl_get_mode_by_netdev(wl, dev) == WL_MODE_AP) {
		...
		if (err < 0) {
			AP6211_ERR("GET STA INFO failed, %d\n", err);
			return err;
		}
		...
		AP6211_DEBUG("STA %s : idle time : %d sec, connected time :%d ms\n",
		...
		if (memcmp(mac, curmacp, ETHER_ADDR_LEN)) {
			AP6211_ERR("Wrong Mac address: "MACDBG" != "MACDBG"\n",
		...
		if (err) {
			AP6211_ERR("Could not get rate (%d)\n", err);
		} else {
			rate = dtoh32(rate);
			sinfo->filled |= STATION_INFO_TX_BITRATE;
			sinfo->txrate.legacy = rate * 5;
			AP6211_DEBUG("Rate %d Mbps\n", (rate / 2));
		}
		...
		if (err) {
			AP6211_ERR("Could not get rssi (%d)\n", err);
...
get_station_err:
		if (err && (err != -ERESTARTSYS)) {
			...
			AP6211_ERR("force cfg80211_disconnected\n");
	...
	bool enabled, s32 timeout)
{
	...
	AP6211_DEBUG("%s: Enter power save enabled %d\n", dev->name, enabled);
	...
	if (_net_info->pm_block || wl->vsdb_mode) {
		...
		AP6211_DEBUG("%s:Do not enable the power save for pm_block %d or vsdb_mode %d\n",
			dev->name, _net_info->pm_block, wl->vsdb_mode);
		pm = PM_OFF;
	}
	pm = htod32(pm);
	AP6211_DEBUG("%s:power save %s\n", dev->name, (pm ? "enabled" : "disabled"));
	err = wldev_ioctl(dev, WLC_SET_PM, &pm, sizeof(pm), true);
	if (unlikely(err)) {
		if (err == -ENODEV)
			AP6211_DEBUG("net_device is not ready yet\n");
		else
			AP6211_ERR("error (%d)\n", err);
		return err;
	}
...
#endif
{
	...
	unsigned long flags;
	if (unlikely(!wl_get_drv_status(wl, READY, ndev))) {
		AP6211_DEBUG("device is not ready : status (%d)\n",
			(int)wl->status);
		return 0;
	}
	...
	s32 err)
{
	 ...
	 */
	if (primary_dev != dev) {
		AP6211_DEBUG("Not supporting Flushing pmklist on virtual"
			" interfaces than primary interface\n");
		return err;
	}

	AP6211_DEBUG("No of elements %d\n", pmk_list->pmkids.npmkid);
	for (i = 0; i < pmk_list->pmkids.npmkid; i++) {
		AP6211_DEBUG("PMKID[%d]: %pM =\n", i,
	...
	struct cfg80211_pmksa *pmksa)
{
	...
	AP6211_DEBUG("set_pmksa,IW_PMKSA_ADD - PMKID: %pM =\n",
	...
	struct cfg80211_pmksa *pmksa)
{
	...
	AP6211_DEBUG("del_pmksa,IW_PMKSA_REMOVE - PMKID: %pM =\n",
		&pmkid.pmkid[0].BSSID);
	for (i = 0; i < WPA2_PMKID_LEN; i++) {
...
wl_cfg80211_scan_alloc_params(int channel, int nprobes, int *out_params_size)
{
	...
	*out_params_size = 0;
	...
	if (params == NULL) {
		AP6211_ERR("%s: mem alloc failed (%d bytes)\n", __func__, params_size);
		return params;
	}
	...
	unsigned int duration, u64 *cookie)
{
	...
	AP6211_DEBUG("Enter, ifindex: %d, channel: %d, duration ms (%d) SCANNING ?? %s \n",
	...
	if (!wl->p2p) {
		AP6211_ERR("wl->p2p is not initialized\n");
		err = BCME_ERROR;
		goto exit;
	}
	...
	*cookie = id;
	...
	if (wl_get_drv_status(wl, SCANNING, ndev)) {
		...
		AP6211_DEBUG("scan is running. go to fake listen state\n");

		wl_set_drv_status(wl, FAKE_REMAINING_ON_CHANNEL, ndev);

		if (timer_pending(&wl->p2p->listen_timer)) {
			AP6211_DEBUG("cancel current listen timer \n");
...
exit:
	if (err == BCME_OK) {
		AP6211_DEBUG("Success\n");
		cfg80211_ready_on_channel(dev, *cookie, channel,
			channel_type, duration, GFP_KERNEL);
	} else {
		AP6211_ERR("Fail to Set (err=%d cookie:%llu)\n", err, *cookie);
	}
	return err;
}
...
{
	s32 err = 0;
	AP6211_DEBUG(" enter ) netdev_ifidx: %d \n", dev->ifindex);
	return err;
}
...
wl_cfg80211_afx_handler(struct work_struct *work)
{
	...
	if (afx_instance != NULL && wl->afx_hdl->is_active) {
		...
		if (unlikely(ret != BCME_OK)) {
			AP6211_ERR("ERROR occurred! returned value is (%d)\n", ret);
...
wl_cfg80211_af_searching_channel(struct wl_priv *wl, struct net_device *dev)
{
	...
	while ((wl->afx_hdl->retry < max_retry) &&
		...
		AP6211_DEBUG("Scheduling the action frame for sending.. retry %d\n",
			wl->afx_hdl->retry);
		...
		if (wl->afx_hdl->my_listen_chan) {
			AP6211_DEBUG("Scheduling Listen peer in my listen channel = %d\n",
				wl->afx_hdl->my_listen_chan);
	...
	struct p2p_config_af_params *config_af_params)
{
	...
	default:
		AP6211_DEBUG("Unknown p2p pub act frame subtype: %d\n",
			act_frm->subtype);
		err = BCME_BADARG;
	}
	return err;
	...
	if (category == DOT11_ACTION_CAT_PUBLIC) {
		if ((action == P2P_PUB_AF_ACTION) &&
			...
			if (action == P2PSD_ACTION_ID_GAS_IREQ ||
				...
				AP6211_DEBUG("Unknown action type: %d\n", action);
			}
		} else {
			AP6211_DEBUG("Unknown Frame: category 0x%x, action 0x%x, length %d\n",
				category, action, action_frame_len);
		}
	} else if (category == P2P_AF_CATEGORY) {
		/* do not configure anything. it will be sent with a default configuration */
	} else {
		AP6211_DEBUG("Unknown Frame: category 0x%x, action 0x%x\n",
			category, action);
...
/* Do not abort scan for VSDB. Scan will be aborted in firmware if necessary */
	...
	if (ack == false) {
		AP6211_ERR("Failed to send Action Frame(retry %d)\n", tx_retry);
	}
exit:
	...
	if (ack && config_af_params.extra_listen &&
		...
		if (extar_listen_time > 50) {
			...
			AP6211_DEBUG("Wait more time! actual af time:%d,"
	...
	AP6211_DEBUG("-- sending Action Frame is %s, listen chan: %d\n",
	...
	u64 *cookie)
{
	...
	if (bssidx == -1) {
		...
		AP6211_ERR("Can not find the bssidx for dev( %p )\n", dev);
		return -ENODEV;
	}
	if (p2p_is_on(wl)) {
		...
		if ((err = wl_cfgp2p_discover_enable_search(wl, false)) < 0) {
			AP6211_ERR("Can not disable discovery mode\n");
			return -EFAULT;
		}
	}
	*cookie = 0;
	...
	if (ieee80211_is_mgmt(mgmt->frame_control)) {
		if (ieee80211_is_probe_resp(mgmt->frame_control)) {
			...
			err = wldev_ioctl(dev, WLC_SCB_DEAUTHENTICATE_FOR_REASON, &scb_val,
				sizeof(scb_val_t), true);
			if (err < 0)
				AP6211_ERR("WLC_SCB_DEAUTHENTICATE_FOR_REASON error %d\n", err);
 ...
 */
		...
		AP6211_ERR("Driver only allows MGMT packet type\n");
		goto exit;
	}
	...
	if (af_params == NULL)
	{
		AP6211_ERR("unable to allocate frame\n");
		return -ENOMEM;
	}
	...
	AP6211_DEBUG("action_frame->len: %d\n", action_frame->len);
	...
	AP6211_DEBUG("channel from upper layer %d\n", wl->afx_hdl->peer_listen_chan);
	...
	u16 frame_type, bool reg)
{
	...
	AP6211_DEBUG("%s: frame_type: %x, reg: %d\n", __func__, frame_type, reg);
	...
	enum nl80211_channel_type channel_type)
{
	...
	AP6211_ERR("netdev_ifidx(%d), chan_type(%d) target channel(%d) \n",
	...
	if (chan->band == IEEE80211_BAND_5GHZ) {
		...
		if (err) {
			if (err != BCME_UNSUPPORTED) {
				AP6211_ERR("bw_cap failed, %d\n", err);
				return err;
			} else {
				err = wldev_iovar_getint(dev, "mimo_bw_cap", &bw_cap);
				if (err) {
					AP6211_ERR("error get mimo_bw_cap (%d)\n", err);
				}
	...
	if (wf_chspec_valid(chspec)) {
		...
		if (fw_chspec != INVCHANSPEC) {
			if ((err = wldev_iovar_setint(dev, "chanspec",
				...
				if (err < 0) {
					AP6211_ERR("WLC_SET_CHANNEL error %d"
					"chip may not be supporting this channel\n", err);
				}
			} else if (err) {
				AP6211_ERR("failed to set chanspec error %d\n", err);
			}
		} else {
			AP6211_ERR("failed to convert host chanspec to fw chanspec\n");
			err = BCME_ERROR;
		}
	} else {
change_bw:
		...
		if (bw)
			goto set_channel;
		AP6211_ERR("Invalid chanspec 0x%x\n", chspec);
		err = BCME_ERROR;
	}
	return err;
}
...
wl_validate_opensecurity(struct net_device *dev, s32 bssidx)
{
	...
	if (err < 0) {
		AP6211_ERR("auth error %d\n", err);
		return BCME_ERROR;
	}
	/* set wsec */
	err = wldev_iovar_setint_bsscfg(dev, "wsec", 0, bssidx);
	if (err < 0) {
		AP6211_ERR("wsec error %d\n", err);
		return BCME_ERROR;
	}
	/* set upper-layer auth */
	err = wldev_iovar_setint_bsscfg(dev, "wpa_auth", WPA_AUTH_NONE, bssidx);
	if (err < 0) {
		AP6211_ERR("wpa_auth error %d\n", err);
		return BCME_ERROR;
	}
...
wl_validate_wpa2ie(struct net_device *dev, bcm_tlv_t *wpa2ie, s32 bssidx)
{
	...
	if ((len -= (WPA_IE_SUITE_COUNT_LEN + (WPA_SUITE_LEN * suite_count))) >= RSN_CAP_LEN) {
		...
		if (err < 0) {
			AP6211_ERR("wme_bss_disable error %d\n", err);
			return BCME_ERROR;
		}
	} else {
		AP6211_DEBUG("There is no RSN Capabilities. remained len %d\n", len);
	}
...
wl_validate_wpaie(struct net_device *dev, wpa_ie_fixed_t *wpaie, s32 bssidx)
{
	...
	if (len < WPA_SUITE_LEN) {
		AP6211_DEBUG("no multicast cipher suite\n");
		goto exit;
	}
	...
	if (len < WPA_IE_SUITE_COUNT_LEN) {
		AP6211_DEBUG(" no auth key mgmt suite\n");
		goto exit;
	}
	...
	s32 bssidx)
{
	...
	if (dev_role == NL80211_IFTYPE_P2P_GO && (ies->wpa2_ie)) {
		...
		AP6211_DEBUG("P2P GO: validating wpa2_ie");
		if (wl_validate_wpa2ie(dev, ies->wpa2_ie, bssidx)  < 0)
			return BCME_ERROR;

	} else if (dev_role == NL80211_IFTYPE_AP) {

		AP6211_DEBUG("SoftAP: validating security");
	...
	u32 dev_role, s32 bssidx)
{
	...
	AP6211_DEBUG("interval (%d) \ndtim_period (%d) \n",
		info->beacon_interval, info->dtim_period);

	if (info->beacon_interval) {
		if ((err = wldev_ioctl(dev, WLC_SET_BCNPRD,
			&info->beacon_interval, sizeof(s32), true)) < 0) {
			AP6211_ERR("Beacon Interval Set Error, %d\n", err);
			return err;
		}
	}
	...
	if (info->dtim_period) {
		if ((err = wldev_ioctl(dev, WLC_SET_DTIMPRD,
			...
			AP6211_ERR("DTIM Interval Set Error, %d\n", err);
			return err;
		}
	}
	...
	if (info->hidden_ssid) {
		if ((err = wldev_iovar_setint(dev, "closednet", 1)) < 0)
			AP6211_ERR("failed to set hidden : %d\n", err);
		AP6211_DEBUG("hidden_ssid_enum_val: %d \n", info->hidden_ssid);
	}
...
wl_cfg80211_parse_ies(u8 *ptr, u32 len, struct parsed_ies *ies)
{
	...
	if ((ies->wps_ie = wl_cfgp2p_find_wpsie(ptr, len)) != NULL) {
		AP6211_DEBUG("WPSIE in beacon \n");
		ies->wps_ie_len = ies->wps_ie->length + WPA_RSN_IE_TAG_FIXED_LEN;
	} else {
		AP6211_ERR("No WPSIE in beacon \n");
	}

	/* find the RSN_IE */
	if ((ies->wpa2_ie = bcm_parse_tlvs(ptr, len,
		DOT11_MNG_RSN_ID)) != NULL) {
		AP6211_DEBUG(" WPA2 IE found\n");
		ies->wpa2_ie_len = ies->wpa2_ie->len;
	}

	/* find the WPA_IE */
	if ((ies->wpa_ie = wl_cfgp2p_find_wpaie(ptr, len)) != NULL) {
		AP6211_DEBUG(" WPA found\n");
	...
	u32 dev_role, s32 bssidx)
{
	...
	if (dev_role == NL80211_IFTYPE_P2P_GO) {
		...
		if (!is_bssup && (ies->wpa2_ie != NULL)) {
			...
			if (err < 0) {
				AP6211_ERR("GO SSID setting error %d\n", err);
				goto exit;
			}

			if ((err = wl_cfgp2p_bss(wl, dev, bssidx, 1)) < 0) {
				AP6211_ERR("GO Bring up error %d\n", err);
				goto exit;
			}
		} else
			AP6211_DEBUG("Bss is already up\n");
	} else if ((dev_role == NL80211_IFTYPE_AP) &&
		(wl_get_drv_status(wl, AP_CREATING, dev))) {
		/* Device role SoftAP */
		err = wldev_ioctl(dev, WLC_DOWN, &ap, sizeof(s32), true);
		if (err < 0) {
			AP6211_ERR("WLC_DOWN error %d\n", err);
			goto exit;
		}
		err = wldev_ioctl(dev, WLC_SET_INFRA, &infra, sizeof(s32), true);
		if (err < 0) {
		...
		if ((err = wldev_ioctl(dev, WLC_SET_AP, &ap, sizeof(s32), true)) < 0) {
			AP6211_ERR("setting AP mode failed %d \n", err);
			goto exit;
		}

		err = wldev_ioctl(dev, WLC_UP, &ap, sizeof(s32), true);
		if (unlikely(err)) {
			AP6211_ERR("WLC_UP error (%d)\n", err);
			goto exit;
		}
		...
		if ((err = wldev_ioctl(dev, WLC_SET_SSID, &join_params,
			...
			AP6211_DEBUG("SoftAP set SSID (%s) success\n", join_params.ssid.SSID);
	...
	s32 bssidx)
{
	...
	if (wl_cfg80211_parse_ies((u8 *)info->tail,
		info->tail_len, ies) < 0) {
		AP6211_ERR("Beacon get IEs failed \n");
		err = -EINVAL;
		goto fail;
	}
	...
	if ((err = wl_cfgp2p_set_management_ie(wl, dev, bssidx,
		...
		info->tail_len)) < 0) {
		AP6211_ERR("Set Beacon IE Failed \n");
	} else {
		AP6211_DEBUG("Applied Vndr IEs for Beacon \n");
	}

	/* Parse Probe Response IEs */
	if (wl_cfg80211_parse_ies((u8 *)info->proberesp_ies,
		info->proberesp_ies_len, &prb_ies) < 0) {
		AP6211_ERR("PRB RESP get IEs failed \n");
		err = -EINVAL;
		goto fail;
	}
	...
	if ((err = wl_cfgp2p_set_management_ie(wl, dev, bssidx,
		...
		AP6211_ERR("Set Probe Resp IE Failed \n");
	} else {
		AP6211_DEBUG("Applied Vndr IEs for Probe Resp \n");
	}
	...
	s32 bssidx)
{
	...
	if (ies->wps_ie) {
		if (wl->ap_info->wps_ie &&
			...
			AP6211_DEBUG(" WPS IE is changed\n");
			kfree(wl->ap_info->wps_ie);
			wl->ap_info->wps_ie = kmemdup(ies->wps_ie, ies->wps_ie_len, GFP_KERNEL);
		} else if (wl->ap_info->wps_ie == NULL) {
			AP6211_DEBUG(" WPS IE is added\n");
			wl->ap_info->wps_ie = kmemdup(ies->wps_ie, ies->wps_ie_len, GFP_KERNEL);
		}
	...
	u8* mac_addr)
{
	...
	AP6211_DEBUG("Entry\n");
	if (mac_addr == NULL) {
		AP6211_DEBUG("mac_addr is NULL ignore it\n");
		return 0;
	}
	...
	if (wldev_ioctl(dev, WLC_SCB_DEAUTHENTICATE_FOR_REASON, &scb_val,
		sizeof(scb_val_t), true))
		AP6211_ERR("WLC_SCB_DEAUTHENTICATE_FOR_REASON failed\n");
	AP6211_DEBUG("Disconnect STA : %s scb_val.val %d\n",
	...
	struct cfg80211_ap_settings *info)
{
	...
	AP6211_DEBUG("Enter \n");
	if (dev == wl_to_prmry_ndev(wl)) {
		AP6211_DEBUG("Start AP req on primary iface: Softap\n");
		dev_role = NL80211_IFTYPE_AP;
	} else if (dev == wl->p2p_net) {
		/* Group Add request on p2p0 */
		AP6211_DEBUG("Start AP req on P2P iface: GO\n");
		dev = wl_to_prmry_ndev(wl);
	...
	if (p2p_is_on(wl) &&
		...
		dev_role = NL80211_IFTYPE_P2P_GO;
		AP6211_DEBUG("Start AP req on P2P connection iface\n");
	}

	if ((err = wl_cfg80211_bcn_set_params(info, dev,
		dev_role, bssidx)) < 0) {
		AP6211_ERR("Beacon params set failed \n");
		goto fail;
	}
	...
	if ((wl_cfg80211_bcn_validate_sec(dev, &ies,
		dev_role, bssidx)) < 0)
	{
		AP6211_ERR("Beacon set security failed \n");
		goto fail;
	}

	if ((err = wl_cfg80211_bcn_bringup_ap(dev, &ies,
		dev_role, bssidx)) < 0) {
		AP6211_ERR("Beacon bring up AP/GO failed \n");
		goto fail;
	}

	AP6211_DEBUG("** AP/GO Created **\n");

fail:
	if (err) {
		AP6211_ERR("ADD/SET beacon failed\n");
		wldev_iovar_setint(dev, "mpc", 1);
	}
	...
	struct cfg80211_beacon_data *info)
{
	...
	if (dev_role == NL80211_IFTYPE_AP) {
		if (wl_cfg80211_hostapd_sec(dev, &ies, bssidx) < 0) {
			AP6211_ERR("Hostapd update sec failed \n");
			err = -EINVAL;
			goto fail;
		}
	}
	...
	struct beacon_parameters *info)
{
	...
	AP6211_DEBUG("interval (%d) dtim_period (%d) head_len (%d) tail_len (%d)\n",
	...
	if (wl_cfgp2p_set_management_ie(wl, dev, bssidx,
		VNDR_IE_BEACON_FLAG, (u8 *)info->tail,
		info->tail_len) < 0) {
		AP6211_ERR("Beacon set IEs failed \n");
		goto fail;
	} else {
		AP6211_DEBUG("Applied Vndr IEs for Beacon \n");
	}
	...
	AP6211_DEBUG("** ADD/SET beacon done **\n");
                             ...
                             struct cfg80211_sched_scan_request *request)
{
	...
	AP6211_DEBUG("ssids:%d pno_time:%d pno_repeat:%d pno_freq:%d \n",
		request->n_ssids, pno_time, pno_repeat, pno_freq_expo_max);


	if (!request || !request->n_ssids || !request->n_match_sets) {
		AP6211_ERR("Invalid sched scan req!! n_ssids:%d \n", request->n_ssids);
		return -EINVAL;
	}
	...
	if (request->n_match_sets > 0) {
		for (i = 0; i < request->n_match_sets; i++) {
			...
			AP6211_DEBUG(">>> PNO filter set for ssid (%s) \n", ssid->ssid);
			ssid_count++;
		}
	}
	...
	if (request->n_ssids > 0) {
		for (i = 0; i < request->n_ssids; i++) {
			...
			AP6211_DEBUG(">>> Active scan req for ssid (%s) \n", request->ssids[i].ssid);
	...
	if (ssid_count) {
		if ((ret = dhd_dev_pno_set(dev, ssids_local, request->n_match_sets,
			...
			AP6211_ERR("PNO setup failed!! ret=%d \n", ret);
			return -EINVAL;
		}

		/* Enable the PNO */
		if (dhd_dev_pno_enable(dev, 1) < 0) {
			AP6211_ERR("PNO enable failed!! ret=%d \n", ret);
			return -EINVAL;
		}
...
int wl_cfg80211_sched_scan_stop(struct wiphy *wiphy, struct net_device *dev)
{
	...
	if (dhd_dev_pno_enable(dev, 0) < 0)
		AP6211_ERR("PNO disable failed");

	if (dhd_dev_pno_reset(dev) < 0)
		AP6211_ERR("PNO reset failed");

	if (wl->scan_request && wl->sched_scan_running) {
		AP6211_DEBUG(">>> Sched scan running. Aborting it..\n");
		wl_notify_escan_complete(wl, dev, true, true);
	}
	...
	struct regulatory_request *request)
{
	...
	if (!request || !wl) {
		AP6211_ERR("Invalid arg\n");
		return -EINVAL;
	}

	AP6211_DEBUG("ccode: %c%c Initiator: %d\n",
		request->alpha2[0], request->alpha2[1], request->initiator);

	/* We support only REGDOM_SET_BY_USER as of now */
	if (request->initiator != NL80211_REGDOM_SET_BY_USER) {
		AP6211_ERR("reg_notifier for intiator:%d not supported \n",
			request->initiator);
		return -ENOTSUPP;
	}
	...
	if (request->alpha2[0] == '0' && request->alpha2[1] == '0') {
		/* world domain */
		AP6211_ERR("World domain. Setting XY/4 \n");
	...
	if ((ret = wldev_iovar_setbuf(wl_to_prmry_ndev(wl), "country", (char *)&cspec,
		...
		AP6211_ERR("set country Failed :%d\n", ret);
		goto exit;
	}

	if ((ret = wl_update_wiphybands(wl, false)) < 0) {
		AP6211_ERR("wl_update_wiphybands failed\n");
		goto exit;
	}

	AP6211_DEBUG("%s: set country '%s/%d' done\n",
...
static s32 wl_setup_wiphy(struct wireless_dev *wdev, struct device *sdiofunc_dev)
{
	...
	if (unlikely(!wdev->wiphy)) {
		AP6211_ERR("Couldn not allocate wiphy device\n");
		err = -ENOMEM;
		return err;
	}
	...
	AP6211_DEBUG("Registering custom regulatory)\n");
	wdev->wiphy->flags |= WIPHY_FLAG_CUSTOM_REGULATORY;
	wiphy_apply_custom_regulatory(wdev->wiphy, &brcm_regdom);
	/* Now we can register wiphy with cfg80211 module */
	err = wiphy_register(wdev->wiphy);
	if (unlikely(err < 0)) {
		AP6211_ERR("Couldn not register wiphy device (%d)\n", err);
...
static s32 wl_inform_bss(struct wl_priv *wl)
{
	...
	AP6211_DEBUG("scanned AP count (%d)\n", bss_list->count);
...
static s32 wl_inform_single_bss(struct wl_priv *wl, struct wl_bss_info *bi, u8 is_roam_done)
{
	...
	if (unlikely(dtoh32(bi->length) > WL_BSS_INFO_MAX)) {
		AP6211_DEBUG("Beacon is larger than buffer. Discarding\n");
		return err;
	}
	...
	if (unlikely(!notif_bss_info)) {
		AP6211_ERR("notif_bss_info alloc failed\n");
		return -ENOMEM;
	}
	...
	if (is_roam_done && ((int)(*(ie_offset)) == WLAN_EID_SSID &&
		...
		AP6211_ERR("WAR trace: Changing the SSID Info, from beacon %d\n",
...
#endif
	if (freq == 0) {
		AP6211_ERR("Invalid channel, fail to chcnage channel to freq\n");
		kfree(notif_bss_info);
		return -EINVAL;
	}
	channel = ieee80211_get_channel(wiphy, freq);
	if (unlikely(!channel)) {
		AP6211_ERR("ieee80211_get_channel error\n");
		kfree(notif_bss_info);
		return -EINVAL;
	}
	AP6211_DEBUG("BSSID %pM, channel %d, rssi %d, capability 0x04%x, mgmt_type %d, "
	...
	if (unlikely(!cbss)) {
		AP6211_ERR("cfg80211_inform_bss_frame error\n");
		kfree(notif_bss_info);
		return -EINVAL;
	}
...
static bool wl_is_linkup(struct wl_priv *wl, const wl_event_msg_t *e, struct net_device *ndev)
{
	...
	AP6211_DEBUG("event %d, status %d flags %x\n", event, status, flags);
	...
	AP6211_DEBUG("wl_is_linkup false\n");
	return false;
}
...
static bool wl_is_linkdown(struct wl_priv *wl, const wl_event_msg_t *e)
{
	...
	event == WLC_E_DEAUTH) {
#if (WL_DBG_LEVEL > 0)
	AP6211_ERR("Link down Reason : WLC_E_%s\n", wl_dbg_estr[event]);
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (len) {
		...
		if (body == NULL) {
			AP6211_ERR("wl_notify_connect_status: Failed to allocate body\n");
			return WL_INVALID;
		}
	}
	memset(&bssid, 0, ETHER_ADDR_LEN);
	AP6211_DEBUG("Enter event %d ndev %p\n", event, ndev);
...
exit:
	...
	if (((event == WLC_E_ASSOC_IND) || (event == WLC_E_REASSOC_IND)) &&
		...
		if (!data) {
			AP6211_ERR("No IEs present in ASSOC/REASSOC_IND");
			return -EINVAL;
		}
	...
	const wl_event_msg_t *e)
{
	...
	AP6211_DEBUG("event type : %d, reason : %d\n", event, reason);
	if (sec) {
		switch (event) {
		case WLC_E_ASSOC:
		case WLC_E_AUTH:
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (wl_get_mode_by_netdev(wl, ndev) == WL_MODE_AP) {
		...
		AP6211_DEBUG("wl_notify_connect_status : event %d status : %d ndev %p\n",
		...
		if (wl_is_linkup(wl, e, ndev)) {
			...
			act = true;
			if (wl_is_ibssmode(wl, ndev)) {
				AP6211_DEBUG("cfg80211_ibss_joined\n");
				cfg80211_ibss_joined(ndev, (s8 *)&e->addr,
					GFP_KERNEL);
				AP6211_DEBUG("joined in IBSS network\n");
			} else {
				if (!wl_get_drv_status(wl, DISCONNECTING, ndev)) {
					AP6211_DEBUG("wl_bss_connect_done succeeded with " MACDBG "\n",
						MAC2STRDBG((u8*)(&e->addr)));
					wl_bss_connect_done(wl, ndev, e, data, true);
					AP6211_DEBUG("joined in BSS network \"%s\"\n",
					((struct wlc_ssid *)
			...
			if (wl_get_drv_status(wl, CONNECTED, ndev)) {
				...
				AP6211_DEBUG("link down if %s may call cfg80211_disconnected. "
					"event : %d, reason=%d from " MACDBG "\n",
					ndev->name, event, ntoh32(e->reason),
					MAC2STRDBG((u8*)(&e->addr)));
				if (memcmp(curbssid, &e->addr, ETHER_ADDR_LEN) != 0) {
					AP6211_ERR("BSSID of event is not the connected BSSID"
				...
				if (! wl_get_drv_status(wl, DISCONNECTING, ndev)) {
					...
					*/
					...
					err = wldev_ioctl(ndev, WLC_DISASSOC, &scbval,
						sizeof(scb_val_t), true);
					if (err < 0) {
						AP6211_ERR("WLC_DISASSOC error %d\n", err);
						err = 0;
					}
				...
				AP6211_DEBUG("link down, during connecting\n");
#ifdef ESCAN_RESULT_PATCH
			...
			AP6211_DEBUG("connect failed event=%d e->status %d e->reason %d \n",
...
static s32 wl_get_assoc_ies(struct wl_priv *wl, struct net_device *ndev)
{
	...
	if (unlikely(err)) {
		AP6211_ERR("could not get assoc info (%d)\n", err);
		return err;
	}
	...
	if (assoc_info.req_len) {
		err = wldev_iovar_getbuf(ndev, "assoc_req_ies", NULL, 0, wl->extra_buf,
			WL_ASSOC_INFO_MAX, NULL);
		if (unlikely(err)) {
			AP6211_ERR("could not get assoc req (%d)\n", err);
			return err;
		}
		...
		if (conn_info->req_ie_len <= MAX_REQ_LINE)
			...
			AP6211_ERR("%s IE size %d above max %d size \n",
	...
	if (assoc_info.resp_len) {
		err = wldev_iovar_getbuf(ndev, "assoc_resp_ies", NULL, 0, wl->extra_buf,
			WL_ASSOC_INFO_MAX, NULL);
		if (unlikely(err)) {
			AP6211_ERR("could not get assoc resp (%d)\n", err);
			return err;
		}
	...
	AP6211_DEBUG("req len (%d) resp len (%d)\n", conn_info->req_ie_len,
        ...
        size_t *join_params_size)
{
	...
	if (ch != 0) {
		...
		*join_params_size += WL_ASSOC_PARAMS_FIXED_SIZE +
		...
		AP6211_DEBUG("join_params->params.chanspec_list[0]= %X, %d channels\n",
			join_params->params.chanspec_list[0],
			join_params->params.chanspec_num);
	}
}
...
static s32 wl_update_bss_info(struct wl_priv *wl, struct net_device *ndev, u8 is_roam_done)
{
	...
	mutex_lock(&wl->usr_sync);
	if (!bss) {
		AP6211_DEBUG("Could not find the AP\n");
		*(u32 *) wl->extra_buf = htod32(WL_EXTRA_BUF_MAX);
		err = wldev_ioctl(ndev, WLC_GET_BSS_INFO,
			wl->extra_buf, WL_EXTRA_BUF_MAX, false);
		if (unlikely(err)) {
			AP6211_ERR("Could not get bss info %d\n", err);
		...
		AP6211_DEBUG("Found the AP in the list - BSSID %pM\n", bss->bssid);
	...
	if (tim) {
		...
		*/
		...
		if (unlikely(err)) {
			AP6211_ERR("WLC_GET_DTIMPRD error (%d)\n", err);
			goto update_bss_info_out;
		}
	}
	...
	const wl_event_msg_t *e, void *data)
{
	...
	AP6211_DEBUG("wl_bss_roaming_done succeeded to " MACDBG "\n",
		MAC2STRDBG((u8*)(&e->addr)));
	...
	AP6211_DEBUG("Report roaming result\n");
	...
	const wl_event_msg_t *e, void *data, bool completed)
{
	...
	if (wl_get_drv_status(wl, CONNECTED, ndev)) {
		if (memcmp(curbssid, connect_req_bssid, ETHER_ADDR_LEN) == 0) {
			AP6211_DEBUG(" Connected event of connected device e=%d s=%d, ignore it\n",
	...
	if (wl_get_drv_status(wl, CONNECTING, ndev)) {
		...
		cfg80211_connect_result(ndev,
			...
			GFP_KERNEL);
		if (completed)
			AP6211_DEBUG("Report connect result - connection succeeded\n");
		else
			AP6211_ERR("Report connect result - connection failed\n");
	}
	return err;
}
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (unlikely(err)) {
		AP6211_ERR("scan busy (%d)\n", err);
		goto scan_done_out;
	}
	channel_inform.scan_channel = dtoh32(channel_inform.scan_channel);
	if (unlikely(channel_inform.scan_channel)) {

		AP6211_DEBUG("channel_inform.scan_channel (%d)\n",
	...
	if (unlikely(err) && unlikely(!wl->scan_suppressed)) {
		AP6211_ERR("%s Scan_results error (%d)\n", ndev->name, err);
		err = -EINVAL;
		goto scan_done_out;
	}
...
scan_done_out:
	...
	spin_unlock_irqrestore(&wl->cfgdrv_lock, flags);
	AP6211_DEBUG("cfg80211_scan_done\n");
	mutex_unlock(&wl->usr_sync);
	return err;
}
static s32
	...
	u8 **pheader, u32 *body_len, u8 *pbody)
{
	...
	*pheader = kzalloc(totlen, GFP_KERNEL);
	if (*pheader == NULL) {
		AP6211_ERR("memory alloc failed \n");
		return -ENOMEM;
	}
...
wl_stop_wait_next_action_frame(struct wl_priv *wl, struct net_device *ndev)
{
	if (wl_get_drv_status_all(wl, SENDING_ACT_FRM) &&
		...
		AP6211_DEBUG("*** Wake UP ** abort actframe iovar\n");
		...
		AP6211_DEBUG("*** Wake UP ** abort listen for next af frame\n");
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (event == WLC_E_ACTION_FRAME_RX) {
		...
		if (err < 0)
			 AP6211_ERR("WLC_GET_BSSID error %d\n", err);
		memcpy(da.octet, wl->ioctl_buf, ETHER_ADDR_LEN);
		err = wl_frame_get_mgmt(FC_ACTION, &da, &e->addr, &bssid,
			&mgmt_frame, &mgmt_frame_len,
			(u8 *)((wl_event_rx_frame_data_t *)rxframe + 1));
		if (err < 0) {
			AP6211_ERR("%s: Error in receiving action frame len %d channel %d freq %d\n",
		...
		if (wl_cfgp2p_is_pub_action(&mgmt_frame[DOT11_MGMT_HDR_LEN],
			...
			if (sd_act_frm && wl_get_drv_status_all(wl, WAITING_NEXT_ACT_FRM)) {
				if (wl->next_af_subtype == sd_act_frm->action) {
					AP6211_DEBUG("We got a right next frame of SD!(%d)\n",
						sd_act_frm->action);
		...
		if (act_frm) {
			...
			if (wl_get_drv_status_all(wl, WAITING_NEXT_ACT_FRM)) {
				if (wl->next_af_subtype == act_frm->subtype) {
					AP6211_DEBUG("We got a right next frame!(%d)\n",
						act_frm->subtype);
					wl_clr_drv_status(wl, WAITING_NEXT_ACT_FRM,
						(ndev == wl->p2p_net) ?
						wl_to_prmry_ndev(wl) : ndev);
		...
		if (event == WLC_E_P2P_PROBREQ_MSG) {
			...
			if (wl->p2p &&
				...
				AP6211_DEBUG("Filtering P2P probe_req while "
	...
	AP6211_DEBUG("%s: mgmt_frame_len (%d) , e->datalen (%d), channel (%d), freq (%d)\n", __func__,
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (e->event_type == WLC_E_PFN_NET_LOST) {
		AP6211_DEBUG("PFN NET LOST event. Do Nothing \n");
		return 0;
	}
	AP6211_DEBUG(">>> PFN NET FOUND event. count:%d \n", pfn_result->count);
	if (pfn_result->count > 0) {
		...
		for (i = 0; i < pfn_result->count; i++) {
			netinfo = &pnetinfo[i];
			if (!netinfo) {
				AP6211_ERR("Invalid netinfo ptr. index:%d", i);
				err = -EINVAL;
				goto out_err;
			}
			AP6211_DEBUG(">>> SSID:%s Channel:%d \n",
		...
		if (wl_get_p2p_status(wl, DISCOVERY_ON)) {
			AP6211_DEBUG(">>> P2P discovery was ON. Disabling it\n");
		...
		AP6211_DEBUG(">>> Doing Full ESCAN on PNO event\n");
		err = wl_do_escan(wl, wiphy, ndev, NULL);
#else
		AP6211_DEBUG(">>> Doing targeted ESCAN on PNO event\n");
		err = wl_do_escan(wl, wiphy, ndev, &request);
#endif
		if (err) {
			wl_clr_drv_status(wl, SCANNING, ndev);
		...
		AP6211_ERR("FALSE PNO Event. (pfn_count == 0) \n");
	}
out_err:
	if (channel)
		kfree(channel);
...
static s32 wl_init_priv_mem(struct wl_priv *wl)
{
	...
	if (unlikely(!wl->scan_results)) {
		AP6211_ERR("Scan results alloc failed\n");
		goto init_priv_mem_out;
	}
	wl->conf = (void *)kzalloc(sizeof(*wl->conf), GFP_KERNEL);
	if (unlikely(!wl->conf)) {
		AP6211_ERR("wl_conf alloc failed\n");
		goto init_priv_mem_out;
	}
	wl->scan_req_int =
	    (void *)kzalloc(sizeof(*wl->scan_req_int), GFP_KERNEL);
	if (unlikely(!wl->scan_req_int)) {
		AP6211_ERR("Scan req alloc failed\n");
		goto init_priv_mem_out;
	}
	wl->ioctl_buf = (void *)kzalloc(WLC_IOCTL_MAXLEN, GFP_KERNEL);
	if (unlikely(!wl->ioctl_buf)) {
		AP6211_ERR("Ioctl buf alloc failed\n");
		goto init_priv_mem_out;
	}
	...
	if (unlikely(!wl->extra_buf)) {
		AP6211_ERR("Extra buf alloc failed\n");
		goto init_priv_mem_out;
	}
	wl->iscan = (void *)kzalloc(sizeof(*wl->iscan), GFP_KERNEL);
	if (unlikely(!wl->iscan)) {
		AP6211_ERR("Iscan buf alloc failed\n");
		goto init_priv_mem_out;
	}
	wl->pmk_list = (void *)kzalloc(sizeof(*wl->pmk_list), GFP_KERNEL);
	if (unlikely(!wl->pmk_list)) {
		AP6211_ERR("pmk list alloc failed\n");
		goto init_priv_mem_out;
	}
	wl->sta_info = (void *)kzalloc(sizeof(*wl->sta_info), GFP_KERNEL);
	if (unlikely(!wl->sta_info)) {
		AP6211_ERR("sta info  alloc failed\n");
		goto init_priv_mem_out;
	}
	...
	if (unlikely(!wl->conn_info)) {
		AP6211_ERR("wl->conn_info  alloc failed\n");
		goto init_priv_mem_out;
	}
	wl->ie = (void *)kzalloc(sizeof(*wl->ie), GFP_KERNEL);
	if (unlikely(!wl->ie)) {
		AP6211_ERR("wl->ie  alloc failed\n");
		goto init_priv_mem_out;
	}
	...
	if (unlikely(!wl->afx_hdl)) {
		AP6211_ERR("afx hdl  alloc failed\n");
		goto init_priv_mem_out;
	} else {
...
static void wl_term_iscan(struct wl_priv *wl)
{
	...
	if (wl->iscan_on && iscan->tsk) {
		...
		AP6211_DEBUG("SIGTERM\n");
		send_sig(SIGTERM, iscan->tsk, 1);
		AP6211_DEBUG("kthread_stop\n");
...
static void wl_notify_iscan_complete(struct wl_iscan_ctrl *iscan, bool aborted)
{
	...
	if (!wl_get_drv_status(wl, SCANNING, ndev)) {
		wl_clr_drv_status(wl, SCANNING, ndev);
		AP6211_ERR("Scan complete while device not scanning\n");
		return;
	}
	spin_lock_irqsave(&wl->cfgdrv_lock, flags);
	wl_clr_drv_status(wl, SCANNING, ndev);
	...
	struct wl_scan_results **bss_list)
{
	...
	AP6211_DEBUG("results->count = %d\n", results->count);
	AP6211_DEBUG("results->buflen = %d\n", results->buflen);
...
static void wl_scan_timeout(unsigned long data)
{
	...
	if (!(wl->scan_request)) {
		AP6211_ERR("timer expired but no scan request\n");
		return;
	}
	bzero(&msg, sizeof(wl_event_msg_t));
		AP6211_ERR("timer expired\n");
	if (wl->escan_on) {
...
static s32 wl_invoke_iscan(struct wl_priv *wl)
{
	...
	if (wl->iscan_on && !iscan->tsk) {
		...
		if (IS_ERR(iscan->tsk)) {
			AP6211_ERR("Could not create iscan thread\n");
			iscan->tsk = NULL;
			return -ENOMEM;
		}
	}
	...
	switch (state) {
		case NETDEV_DOWN:
			while (work_pending(&wdev->cleanup_work) && refcnt < 100) {
				if (refcnt%5 == 0)
					AP6211_ERR("%s : [NETDEV_DOWN] work_pending (%d th)\n",
						__FUNCTION__, refcnt);
	...
	bool aborted, bool fw_abort)
{
	...
	{
		AP6211_ERR("ndev is different %p %p\n", wl->escan_info.ndev, ndev);
		return err;
	}
	...
	if (wl->scan_request) {
		...
		AP6211_DEBUG("wl->scan_request is NULL may be internal scan."
	...
	if (fw_abort && !in_atomic()) {
		...
		if (params == NULL) {
			AP6211_ERR("scan params allocation failed \n");
			err = -ENOMEM;
		} else {
			/* Do a scan abort to stop the driver's scan engine */
			err = wldev_ioctl(dev, WLC_SCAN, params, params_size, true);
			if (err < 0) {
				AP6211_ERR("scan abort  failed \n");
			}
		}
	}
	if (timer_pending(&wl->scan_timeout))
	...
	if (wl->sched_scan_req && !wl->scan_request) {
		AP6211_DEBUG(">>> REPORTING SCHED SCAN RESULTS \n");
		if (aborted)
	...
	const wl_event_msg_t *e, void *data)
{
	...
	AP6211_DEBUG(" enter event type : %d, status : %d \n",
		ntoh32(e->event_type), ntoh32(e->status));
	...
	if (!ndev || !wl->escan_on ||
		...
		AP6211_ERR("escan is not ready ndev %p wl->escan_on %d drv_status 0x%x\n",
			ndev, wl->escan_on, wl_get_drv_status(wl, SCANNING, ndev));
		goto exit;
	}
	if (status == WLC_E_STATUS_PARTIAL) {
		AP6211_DEBUG("WLC_E_STATUS_PARTIAL \n");
		escan_result = (wl_escan_result_t *) data;
		if (!escan_result) {
			AP6211_ERR("Invalid escan result (NULL pointer)\n");
			goto exit;
		}
		if (dtoh16(escan_result->bss_count) != 1) {
			AP6211_ERR("Invalid bss_count %d: ignoring\n", escan_result->bss_count);
			goto exit;
		}
		bi = escan_result->bss_info;
		if (!bi) {
			AP6211_ERR("Invalid escan bss info (NULL pointer)\n");
			goto exit;
		}
		bi_length = dtoh32(bi->length);
		if (bi_length != (dtoh32(escan_result->buflen) - WL_ESCAN_RESULTS_FIXED_SIZE)) {
			AP6211_ERR("Invalid bss_info length %d: ignoring\n", bi_length);
			goto exit;
		}

		if (!(wl_to_wiphy(wl)->interface_modes & BIT(NL80211_IFTYPE_ADHOC))) {
			if (dtoh16(bi->capability) & DOT11_CAP_IBSS) {
				AP6211_DEBUG("Ignoring IBSS result\n");
				goto exit;
			}
		}
		...
		if (wl_get_drv_status_all(wl, FINDING_COMMON_CHANNEL)) {
			...
			if (p2p_dev_addr && !memcmp(p2p_dev_addr,
				...
				AP6211_DEBUG("ACTION FRAME SCAN : Peer " MACDBG " found, channel : %d\n",
...
#endif
		{
				...
				if ((p2p_ie = wl_cfgp2p_find_p2pie(((u8 *) bi) + bi->ie_offset,
					bi->ie_length)) == NULL) {
						AP6211_ERR("Couldn't find P2PIE in probe"
							" response/beacon\n");
						goto exit;
				}
#ifdef WL_HOST_BAND_MGMT
			...
			for (i = 0; i < list->count; i++) {
				...
				if (!bcmp(&bi->BSSID, &bss->BSSID, ETHER_ADDR_LEN) &&
					...
					*/
					...
					AP6211_DEBUG("%s("MACDBG"), i=%d prev: RSSI %d"
					...
					if ((bss->flags & WL_BSS_FLAGS_RSSI_ONCHANNEL) ==
						...
						*/
						AP6211_DEBUG("%s("MACDBG"), same onchan"
						", RSSI: prev %d new %d\n",
						...
						*/
						AP6211_DEBUG("%s("MACDBG"), prev onchan"
						", RSSI: prev %d new %d\n",
						bss->SSID, MAC2STRDBG(bi->BSSID.octet),
						bss->RSSI, bi->RSSI);
						bi->RSSI = bss->RSSI;
					...
					if (dtoh32(bss->length) != bi_length) {
						...
						AP6211_DEBUG("bss info replacement"
							" is occured(bcast:%d->probresp%d)\n",
							bss->ie_length, bi->ie_length);
						AP6211_DEBUG("%s("MACDBG"), replacement!(%d -> %d)\n",
						bss->SSID, MAC2STRDBG(bi->BSSID.octet),
						prev_len, bi_length);

						if (list->buflen - prev_len + bi_length
							> ESCAN_BUF_SIZE) {
							AP6211_ERR("Buffer is too small: keep the"
			...
			if (bi_length > ESCAN_BUF_SIZE - list->buflen) {
				AP6211_ERR("Buffer is too small: ignoring\n");
				goto exit;
			}
			if (strlen(bi->SSID) == 0) { // terence: fix for hidden SSID
				AP6211_DEBUG("Skip hidden SSID %pM\n", &bi->BSSID);
				goto exit;
			}
		...
		wl->escan_info.escan_state = WL_ESCAN_STATE_IDLE;
		if (wl_get_drv_status_all(wl, FINDING_COMMON_CHANNEL)) {
			AP6211_DEBUG("ACTION FRAME SCAN DONE\n");
			wl_clr_p2p_status(wl, SCANNING);
			wl_clr_drv_status(wl, SCANNING, wl->afx_hdl->dev);
			if (wl->afx_hdl->peer_chan == WL_INVALID)
				complete(&wl->act_frm_scan);
	...
	{
		escan_result = (wl_escan_result_t *) data;
		AP6211_ERR("WLC_E_STATUS_NEWSCAN : scan_request[%p]\n", wl->scan_request);
		AP6211_ERR("sync_id[%d], bss_count[%d]\n", escan_result->sync_id,
			escan_result->bss_count);
	} else if (status == WLC_E_STATUS_TIMEOUT) {
		AP6211_ERR("WLC_E_STATUS_TIMEOUT : scan_request[%p]\n", wl->scan_request);
		AP6211_ERR("escan_on[%d], reason[0x%x]\n", wl->escan_on, e->reason);
		if (e->reason == 0xFFFFFFFF) {
			wl_notify_escan_complete(wl, wl->escan_info.ndev, true, true);
		}
	} else {
		AP6211_ERR("unexpected Escan Event %d : abort\n", status);
		wl->escan_info.escan_state = WL_ESCAN_STATE_IDLE;
		if (wl_get_drv_status_all(wl, FINDING_COMMON_CHANNEL)) {
			AP6211_DEBUG("ACTION FRAME SCAN DONE\n");
			wl_clr_p2p_status(wl, SCANNING);
	...
	if (enable && connected_cnt > 1) {
		for_each_ndev(wl, iter, next) {
			...
			if ((err = wldev_iovar_getint(iter->ndev, "roam_off",
				...
				AP6211_ERR("%s:Failed to get current roam setting err %d\n",
					iter->ndev->name, err);
				continue;
			}
			if ((err = wldev_iovar_setint(iter->ndev, "roam_off", 1)) != BCME_OK) {
				AP6211_ERR(" %s:failed to set roam_off : %d\n",
					iter->ndev->name, err);
			}
		}
	}
	...
	enum wl_status state, bool set)
{
	...
	struct net_info *iter, *next;
	struct net_device *primary_dev = wl_to_prmry_ndev(wl);
	AP6211_DEBUG("Enter state %d set %d _net_info->pm_restore %d iface %s\n",
	...
	if (set) {
		...
		if (wl_get_mode_by_netdev(wl, _net_info->ndev) == WL_MODE_AP) {
			pm = PM_OFF;
			AP6211_DEBUG("%s:AP power save %s\n", _net_info->ndev->name,
				pm ? "enabled" : "disabled");
			if ((err = wldev_ioctl(_net_info->ndev, WLC_SET_PM,
				&pm, sizeof(pm), true)) != 0) {
				if (err == -ENODEV)
					AP6211_DEBUG("%s:net_device is not ready\n",
						_net_info->ndev->name);
				else
					AP6211_ERR("%s:error (%d)\n", _net_info->ndev->name, err);
			}
			if (wl_add_remove_eventmsg(primary_dev, WLC_E_P2P_PROBREQ_MSG, false))
				AP6211_ERR(" failed to unset WLC_E_P2P_PROPREQ_MSG\n");
			return 0;
		}
		...
		for_each_ndev(wl, iter, next) {
			...
			if ((err = wldev_ioctl(iter->ndev, WLC_SET_PM, &pm,
				sizeof(pm), true)) != 0) {
				if (err == -ENODEV)
					AP6211_DEBUG("%s:netdev not ready\n", iter->ndev->name);
				else
					AP6211_ERR("%s:error (%d)\n", iter->ndev->name, err);
		...
		for_each_ndev(wl, iter, next) {
			if (iter->pm_restore) {
				AP6211_DEBUG("%s:restoring power save %s\n",
...
static s32 wl_cfg80211_attach_p2p(void)
{
	...
	if (wl_cfgp2p_register_ndev(wl) < 0) {
		AP6211_ERR("%s: P2P attach failed. \n", __func__);
		return -ENODEV;
	}
...
s32 wl_cfg80211_attach_post(struct net_device *ndev)
{
	...
	AP6211_DEBUG("In\n");
	if (unlikely(!ndev)) {
		AP6211_ERR("ndev is invaild\n");
		return -ENODEV;
	}
	wl = wlcfg_drv_priv;
	if (unlikely(!wl)) {
		AP6211_ERR("wl is invaild\n");
		return -EINVAL;
	}
	if (!wl_get_drv_status(wl, READY, ndev)) {
			if (wl->wdev &&
				...
				if (wl->p2p_net) {
					...
					AP6211_ERR("%s: p2p_dev_addr="MACDBG "\n",
						wl->p2p_net->name,
						MAC2STRDBG(wl->p2p_net->dev_addr));
				} else {
					AP6211_ERR("p2p_net not yet populated."
...
s32 wl_cfg80211_attach(struct net_device *ndev, void *data)
{
	...
	if (err) {
		AP6211_ERR("Failed to alloc net_info (%d)\n", err);
		goto cfg80211_attach_out;
	}
	err = wl_init_priv(wl);
	if (err) {
		AP6211_ERR("Failed to init iwm_priv (%d)\n", err);
		goto cfg80211_attach_out;
	}

	err = wl_setup_rfkill(wl, TRUE);
	if (err) {
		AP6211_ERR("Failed to setup rfkill %d\n", err);
		goto cfg80211_attach_out;
	}
	err = register_netdevice_notifier(&wl_cfg80211_netdev_notifier);
	if (err) {
		AP6211_ERR("Failed to register notifierl %d\n", err);
		goto cfg80211_attach_out;
	}
...
static int wl_is_p2p_event(struct wl_event_q *e)
{
	...
	*/
	...
	case WLC_E_ACTION_FRAME_COMPLETE:
		...
		if (e->emsg.ifidx != 0) {
			AP6211_DEBUG("P2P Event on Virtual I/F (ifidx:%d) \n",
			e->emsg.ifidx);
			...
			AP6211_DEBUG("P2P Event on Primary I/F (ifidx:%d)."
				" Sent it to p2p0 \n", e->emsg.ifidx);
			return TRUE;
		}
		break;
	...
	default:
		AP6211_DEBUG("NON-P2P Event %d on ifidx (ifidx:%d) \n",
...
static s32 wl_event_handler(void *data)
{
...
#else
	AP6211_ERR("tsk Enter, tsk = 0x%08x\n", (unsigned int)tsk);
#endif
	...
	while (down_interruptible (&tsk->sema) == 0) {
		...
		while ((e = wl_deq_event(wl))) {
			AP6211_DEBUG("event type (%d), if idx: %d\n", e->etype, e->emsg.ifidx);
			...
			if (e->etype < WLC_E_LAST && wl->evt_handler[e->etype]) {
				...
				AP6211_DEBUG("Unknown Event (%d): ignoring\n", e->etype);
			}
			wl_put_event(e);
		}
		DHD_OS_WAKE_UNLOCK(wl->pub);
	}
	AP6211_ERR("%s was terminated\n", __func__);
...
wl_cfg80211_event(struct net_device *ndev, const wl_event_msg_t * e, void *data)
{
	...
	AP6211_DEBUG("event_type (%d):" "WLC_E_" "%s\n", event_type, estr);
#endif /* (WL_DBG_LEVEL > 0) */

	if (event_type == WLC_E_PFN_NET_FOUND) {
		AP6211_DEBUG(" PNOEVENT: PNO_NET_FOUND\n");
	}
	else if (event_type == WLC_E_PFN_NET_LOST) {
		AP6211_DEBUG(" PNOEVENT: PNO_NET_LOST\n");
	}
...
static s32 wl_config_ifmode(struct wl_priv *wl, struct net_device *ndev, s32 iftype)
{
	...
	case NL80211_IFTYPE_MONITOR:
	case NL80211_IFTYPE_WDS:
		AP6211_ERR("type (%d) : currently we do not support this mode\n",
			iftype);
		err = -EINVAL;
		return err;
	case NL80211_IFTYPE_ADHOC:
	...
	default:
		err = -EINVAL;
		AP6211_ERR("invalid type (%d)\n", iftype);
		return err;
	}
	infra = htod32(infra);
	err = wldev_ioctl(ndev, WLC_SET_INFRA, &infra, sizeof(infra), true);
	if (unlikely(err)) {
		AP6211_ERR("WLC_SET_INFRA error (%d)\n", err);
		return err;
	}
...
void wl_cfg80211_add_to_eventbuffer(struct wl_eventmsg_buf *ev, u16 event, bool set)
{
	...
	if (ev->num < MAX_EVENT_BUF_NUM) {
		...
		AP6211_ERR("evenbuffer doesn't support > %u events. Update"
	...
	wl_eventmsg_buf_t *ev)
{
	...
	if (unlikely(ret)) {
		AP6211_ERR("Get event_msgs error (%d)\n", ret);
		goto exit;
	}
	...
	if (unlikely(ret)) {
		AP6211_ERR("Set event_msgs error (%d)\n", ret);
	}
...
#define LOCAL_BUF_LEN 1024
	...
	if (pbuf == NULL) {
		AP6211_ERR("failed to allocate local buf\n");
		return -ENOMEM;
	}
	...
	if (err != 0) {
		AP6211_ERR("get chanspecs failed with %d\n", err);
		kfree(pbuf);
		return err;
	}
#undef LOCAL_BUF_LEN
	...
	for (i = 0; i < dtoh32(list->count); i++) {
		...
		if (CHSPEC_IS40(c)) {
			...
			AP6211_DEBUG("HT80 center channel : %d\n", channel);
			continue;
		}
		if (CHSPEC_IS2G(c) && (channel >= CH_MIN_2G_CHANNEL) &&
			...
			AP6211_ERR("Invalid channel Sepc. 0x%x.\n", c);
			continue;
		}
...
s32 wl_update_wiphybands(struct wl_priv *wl, bool notify)
{
	...
	if (unlikely(err)) {
		AP6211_ERR("error read bandlist (%d)\n", err);
		goto end_bands;
	}
	...
	if (unlikely(err)) {
		AP6211_ERR("error reading nmode (%d)\n", err);
	} else {
	...
	if (err) {
		AP6211_ERR("wl_construct_reginfo() fails err=%d\n", err);
		if (err != BCME_UNSUPPORTED)
			goto end_bands;
		err = 0;
	}
...
static s32 __wl_cfg80211_up(struct wl_priv *wl)
{
	...
	if (unlikely(err && err != -EINPROGRESS)) {
		AP6211_ERR("wl_config_ifmode failed\n");
	}
	...
	if (wl_cfg80211_set_band(ndev, WLC_BAND_AUTO) < 0) {
		AP6211_ERR("roam_band set failed\n");
		err = -1;
	}
#endif /* WL_HOST_BAND_MGMT */
...
s32 wl_cfg80211_up(void *para)
{
	...
	if ((err = wldev_ioctl(wl_to_prmry_ndev(wl), WLC_GET_VERSION, &val,
		sizeof(int), false) < 0)) {
		AP6211_ERR("WLC_GET_VERSION failed, err=%d\n", err);
		return err;
	}
	val = dtoh32(val);
	if (val != WLC_IOCTL_VERSION && val != 1) {
		AP6211_ERR("Version mismatch, please upgrade. Got %d, expected %d or 1\n",
			val, WLC_IOCTL_VERSION);
		return BCME_VERSION;
	}
	ioctl_version = val;
	AP6211_DEBUG("WLC_GET_VERSION=%d\n", ioctl_version);
	...
	if (unlikely(err))
		AP6211_ERR("__wl_cfg80211_up failed\n");
	mutex_unlock(&wl->usr_sync);
	return err;
}
...
int wl_cfg80211_hang(struct net_device *dev, u16 reason)
{
	...
	AP6211_ERR("In : chip crash eventing\n");
...
static __used s32 wl_add_ie(struct wl_priv *wl, u8 t, u8 l, u8 *v)
{
	...
	if (unlikely(ie->offset + l + 2 > WL_TLV_INFO_MAX)) {
		AP6211_ERR("ei crosses buffer boundary\n");
		return -ENOSPC;
	}
	ie->buf[ie->offset] = t;
...
static s32 wl_mrg_ie(struct wl_priv *wl, u8 *ie_stream, u16 ie_size)
{
	...
	if (unlikely(ie->offset + ie_size > WL_TLV_INFO_MAX)) {
		AP6211_ERR("ei_stream crosses buffer boundary\n");
		return -ENOSPC;
	}
...
static s32 wl_cp_ie(struct wl_priv *wl, u8 *dst, u16 dst_size)
{
	...
	if (unlikely(ie->offset > dst_size)) {
		AP6211_ERR("dst_size is not enough\n");
		return -ENOSPC;
	}
...
{
	wl_dbg_level = level;
	AP6211_DEBUG("%s: wl_dbg_level = 0x%x\n", __FUNCTION__, wl_dbg_level);
}
            ...
            uint8 ie_id, uint8 *data, uint8 data_len)
{
	...
	if ((pktflag & ~(VNDR_IE_BEACON_FLAG | VNDR_IE_PRBRSP_FLAG |
	            ...
	            VNDR_IE_CUSTOM_FLAG))) {
		AP6211_ERR("cfg80211 Add IE: Invalid packet flag 0x%x\n", pktflag);
		return -1;
	}
	...
	if (!ie_setbuf) {
		AP6211_ERR("Error allocating buffer for IE\n");
		return -ENOMEM;
	}

	if (wl->iw_ie_len == data_len && !memcmp(wl->iw_ie, data, data_len)) {
		AP6211_ERR("Previous IW IE is equals to current IE\n");
		return err;
	}
	...
	if (wl->iw_ie_len != 0) {
		AP6211_DEBUG("Different IW_IE was already set. clear first\n");
...
wl_cfg80211_set_band(struct net_device *ndev, int band)
{
	...
	if ((band < WLC_BAND_AUTO) || (band > WLC_BAND_2G)) {
		AP6211_ERR("Invalid band\n");
		return -EINVAL;
	}

	if ((ret = wldev_iovar_setbuf(ndev, "roam_band", &band,
		sizeof(int), ioctl_buf, sizeof(ioctl_buf), NULL)) < 0) {
		AP6211_ERR("seting roam_band failed code=%d\n", ret);
		return ret;
	}

	AP6211_DEBUG("Setting band to %d\n", band);
	wl->curr_band = band;
...
static void wl_cfg80211_work_handler(struct work_struct *work)
{
	...
	if (!wl) {
		AP6211_ERR("wl_priv ptr NULL\n");
		return;
	}

	if (wl->scan_suppressed) {
		/* There is pending scan_suppress. Clean it */
		AP6211_ERR("Clean up from timer after %d msec\n", WL_SCAN_SUPPRESS_TIMEOUT);
...
int wl_cfg80211_scan_suppress(struct net_device *dev, int suppress)
{
	...
	if (suppress == wl->scan_suppressed) {
		AP6211_DEBUG("No change in scan_suppress state. Ignoring cmd..\n");
		return 0;
	}
	...
	if ((ret = wldev_ioctl(dev, WLC_SET_SCANSUPPRESS,
		...
		AP6211_ERR("Scan suppress setting failed ret:%d \n", ret);
	} else {
		AP6211_DEBUG("Scan suppress %s \n", suppress ? "Enabled" : "Disabled");
	...
	if (wl->scan_suppressed) {
		if (ret) {
			AP6211_ERR("Retry scan_suppress reset at a later time \n");
			mod_timer(&wl->scan_supp_timer,
				jiffies + msecs_to_jiffies(WL_SCAN_SUPPRESS_RETRY));
		} else {
			AP6211_DEBUG("Start wlan_timer to clear of scan_suppress \n");
			mod_timer(&wl->scan_supp_timer,
