...
static int dhd_mon_if_subif_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	if (mon_if == NULL || mon_if->real_ndev == NULL) {
		AP6211_DEBUG(" cannot find matched net dev, skip the packet\n");
		goto fail;
	}
	...
	AP6211_DEBUG("radiotap len (should be 14): %d\n", rtap_len);
	...
	if ((frame_ctl & IEEE80211_FCTL_FTYPE) == IEEE80211_FTYPE_DATA) {
		...
		AP6211_DEBUG("if name: %s, matched if name %s\n", ndev->name, mon_if->real_ndev->name);
...
static void dhd_mon_if_set_multicast_list(struct net_device *ndev)
{
	...
	if (mon_if == NULL || mon_if->real_ndev == NULL) {
		...
		AP6211_DEBUG("enter, if name: %s, matched if name %s\n",
		ndev->name, mon_if->real_ndev->name);
	}
}
...
int dhd_add_monitor(char *name, struct net_device **new_ndev)
{
	...
	AP6211_DEBUG("enter, if name: %s\n", name);
	if (!name || !new_ndev) {
		AP6211_DEBUG("invalid parameters\n");
		ret = -EINVAL;
		goto out;
	}
	...
	if (idx == -1) {
		AP6211_DEBUG("exceeds maximum interfaces\n");
		ret = -EFAULT;
		goto out;
	}
	...
	if (!ndev) {
		AP6211_DEBUG("failed to allocate memory\n");
		ret = -ENOMEM;
		goto out;
	}
	...
	if (ret) {
		AP6211_DEBUG(" register_netdevice failed (%d)\n", ret);
		goto out;
	}
	...
	*dhd_mon = &g_monitor;
	...
	AP6211_DEBUG("net device returned: 0x%p\n", ndev);
	AP6211_DEBUG("found a matched net device, name %s\n", g_monitor.mon_if[idx].real_ndev->name);
...
int dhd_del_monitor(struct net_device *ndev)
{
	...
	MONITOR_STATE_INTERFACE_DELETED)
		AP6211_DEBUG("interface not found in monitor IF array, is this a monitor IF? 0x%p\n",
			ndev);
	mutex_unlock(&g_monitor.lock);
