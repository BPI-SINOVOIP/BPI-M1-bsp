...
sdioh_sdmmc_card_enablefuncs(sdioh_info_t *sd)
{
	...
	if (err_ret) {
		AP6211_ERR("bcmsdh_sdmmc: Failed to enable F1 Err: 0x%08x", err_ret);
	}
...
sdioh_attach(osl_t *osh, void *bar0, uint irq)
{
	...
	if (gInstance == NULL) {
		AP6211_ERR("%s: SDIO Device not present\n", __FUNCTION__);
		return NULL;
	}

	if ((sd = (sdioh_info_t *)MALLOC(osh, sizeof(sdioh_info_t))) == NULL) {
		AP6211_ERR("sdioh_attach: out of memory, malloced %d bytes\n", MALLOCED(osh));
		return NULL;
	}
	bzero((char *)sd, sizeof(sdioh_info_t));
	sd->osh = osh;
	if (sdioh_sdmmc_osinit(sd) != 0) {
		AP6211_ERR("%s:sdioh_sdmmc_osinit() failed\n", __FUNCTION__);
	...
	if (gInstance->func[1]) {
		...
		if (err_ret) {
			AP6211_ERR("bcmsdh_sdmmc: Failed to set F1 blocksize\n");
		}

		/* Release host controller F1 */
		sdio_release_host(gInstance->func[1]);
	} else {
		AP6211_ERR("%s:gInstance->func[1] is null\n", __FUNCTION__);
		MFREE(sd->osh, sd, sizeof(sdioh_info_t));
		return NULL;
	}
	...
	if (gInstance->func[2]) {
		...
		if (err_ret) {
			AP6211_ERR("bcmsdh_sdmmc: Failed to set F2 blocksize to %d\n",
				sd_f2_blocksize);
		}
		...
		AP6211_ERR("%s:gInstance->func[2] is null\n", __FUNCTION__);
		MFREE(sd->osh, sd, sizeof(sdioh_info_t));
		return NULL;
	}
...
sdioh_enable_func_intr(void)
{
	...
	if (gInstance->func[0]) {
		...
		if (err) {
			AP6211_ERR("%s: error for read SDIO_CCCR_IENx : 0x%x\n", __FUNCTION__, err);
		...
		if (err) {
			AP6211_ERR("%s: error for write SDIO_CCCR_IENx : 0x%x\n", __FUNCTION__, err);
			return SDIOH_API_RC_FAIL;
		}
	}
...
{
	AP6211_DEBUG("%s: Entering\n", __FUNCTION__);
	if (fn == NULL) {
		AP6211_ERR("%s: interrupt handler is NULL, not registering\n", __FUNCTION__);
		return SDIOH_API_RC_FAIL;
	}
#if !defined(OOB_INTR_ONLY)
	sd->intr_handler = fn;
                           ...
                           void *params, int plen, void *arg, int len, bool set)
{
	...
	case IOV_GVAL(IOV_HOSTREG):
	{
		...
		AP6211_DEBUG("%s: rreg%d at offset %d\n", __FUNCTION__,
	...
	case IOV_SVAL(IOV_HOSTREG):
	{
		...
		AP6211_DEBUG("%s: wreg%d value 0x%08x at offset %d\n", __FUNCTION__, sd_ptr->value,
		                  (sd_ptr->offset & 1) ? 8 : ((sd_ptr->offset & 2) ? 16 : 32),
		                  sd_ptr->offset);
		break;
	}
...
sdioh_cis_read(sdioh_info_t *sd, uint func, uint8 *cisd, uint32 length)
{
	...
	AP6211_DEBUG("%s: Func = %d\n", __FUNCTION__, func);

	if (!sd->func_cis_ptr[func]) {
		bzero(cis, length);
		AP6211_ERR("%s: no func_cis_ptr[%d]\n", __FUNCTION__, func);
		return SDIOH_API_RC_FAIL;
	}

	AP6211_ERR("%s: func_cis_ptr[%d]=0x%04x\n", __FUNCTION__, func, sd->func_cis_ptr[func]);

	for (count = 0; count < length; count++) {
		offset =  sd->func_cis_ptr[func] + count;
		if (sdioh_sdmmc_card_regread (sd, 0, offset, 1, &foo) < 0) {
			AP6211_ERR("%s: regread failed: Can't read CIS\n", __FUNCTION__);
			return SDIOH_API_RC_FAIL;
		}
...
sdioh_request_byte(sdioh_info_t *sd, uint rw, uint func, uint regaddr, uint8 *byte)
{
	...
	AP6211_DEBUG("%s: rw=%d, func=%d, addr=0x%05x\n", __FUNCTION__, rw, func, regaddr);
	...
	if(rw) { /* CMD52 Write */
		if (func == 0) {
			...
			if (regaddr == SDIOD_CCCR_IOEN) {
				if (gInstance->func[2]) {
					...
					if (*byte & SDIO_FUNC_ENABLE_2) {
						...
						if (err_ret) {
							AP6211_ERR("bcmsdh_sdmmc: enable F2 failed:%d",
								err_ret);
						}
					} else {
						/* Disable Function 2 */
						...
						if (err_ret) {
							AP6211_ERR("bcmsdh_sdmmc: Disab F2 failed:%d",
								err_ret);
						}
					}
					sdio_release_host(gInstance->func[2]);
			...
			/* to allow abort command through F1 */
				...
				AP6211_ERR("bcmsdh_sdmmc: F0 Wr:0x%02x: write disallowed\n", regaddr);
			} else {
	...
	if (ret != 0)
		AP6211_DEBUG("%s data timeout.\n", __FUNCTION__);	

	if (err_ret) {
		AP6211_ERR("bcmsdh_sdmmc: Failed to %s byte F%d:@0x%05x=%02x, Err: %d\n",
                                   ...
                                   uint32 *word, uint nbytes)
{
	...
	if (func == 0) {
		AP6211_ERR("%s: Only CMD52 allowed to F0.\n", __FUNCTION__);
		return SDIOH_API_RC_FAIL;
	}

	AP6211_DEBUG("%s: cmd_type=%d, rw=%d, func=%d, addr=0x%05x, nbytes=%d\n",
                     ...
                     uint addr, void *pkt)
{
	...
	AP6211_DEBUG("%s: %s %dB to func%d:%08x, %d blks with DMA, %dB leftover\n",
	...
	if (0 != dma_len) {
		...
		for (pnext = pkt;
			...
			if (SGCount >= SDIOH_SDMMC_MAX_SG_ENTRIES) {
				AP6211_ERR("%s: sg list entries exceed limit\n",
					__FUNCTION__);
				return (SDIOH_API_RC_FAIL);
			}
		}
		...
		if (0 != err_ret) {
			AP6211_ERR("%s:CMD53 %s failed with code %d\n",
			       __FUNCTION__,
			       write ? "write" : "read",
			       err_ret);
			AP6211_ERR("%s:Disabling rxchain and fire it with PIO\n",
			       __FUNCTION__);
			sd->use_rxchain = FALSE;
			pkt = pprev;
			lft_len = ttl_len;
	...
	if (0 != lft_len) {
		...
		for (pnext = pkt; pnext; pnext = PKTNEXT(sd->osh, pnext)) {
			...
			if ((pkt_len % 64) == 32) {
				AP6211_DEBUG("%s: Rounding up TX packet +=32\n", __FUNCTION__);
				pkt_len += 32;
			}
			...
			if (err_ret)
				AP6211_ERR("%s: %s FAILED %p[%d], addr=0x%05x, pkt_len=%d, ERR=%d\n",
				       __FUNCTION__,
				       (write) ? "TX" : "RX",
				       pnext, SGCount, addr, pkt_len, err_ret);
			else
				AP6211_DEBUG("%s: %s xfr'd %p[%d], addr=0x%05x, len=%d\n",
					__FUNCTION__,
					(write) ? "TX" : "RX",
                     ...
                     uint addr, uint reg_width, uint buflen_u, uint8 *buffer, void *pkt)
{
	...
	if (pkt == NULL) {
		AP6211_DEBUG("%s: Creating new %s Packet, len=%d\n",
		...
		{
			AP6211_ERR("%s: PKTGET failed: len %d\n",
		...
		AP6211_DEBUG("%s: Creating aligned %s Packet, len=%d\n",
		...
		AP6211_DEBUG("%s: Aligned %s Packet, direct DMA\n",
...
sdioh_sdmmc_card_regread(sdioh_info_t *sd, int func, uint32 regaddr, int regsize, uint32 *data)
{
	...
	if ((func == 0) || (regsize == 1)) {
		...
		*data = temp;
		*data &= 0xff;
		AP6211_DEBUG("%s: byte read data=0x%02x\n",
		         __FUNCTION__, *data);
	} else {
		...
		AP6211_DEBUG("%s: word read data=0x%08x\n",
		         __FUNCTION__, *data);
	}
...
static void IRQHandler(struct sdio_func *func)
{
	...
	AP6211_DEBUG("bcmsdh_sdmmc: ***IRQHandler\n");
	sd = gInstance->sd;
	...
	if (sd->use_client_ints) {
		...
		AP6211_ERR("bcmsdh_sdmmc: ***IRQHandler\n");
...
static void IRQHandlerF2(struct sdio_func *func)
{
	...
	AP6211_DEBUG("bcmsdh_sdmmc: ***IRQHandlerF2\n");
...
sdioh_sdmmc_card_regwrite(sdioh_info_t *sd, int func, uint32 regaddr, int regsize, uint32 data)
{
	...
	if ((func == 0) || (regsize == 1)) {
		...
		temp = data & 0xff;
		sdioh_request_byte(sd, SDIOH_READ, func, regaddr, &temp);
		AP6211_DEBUG("%s: byte write data=0x%02x\n",
		         __FUNCTION__, data);
	} else {
		if (regsize == 2)
			data &= 0xffff;
		...
		AP6211_DEBUG("%s: word write data=0x%08x\n",
		         __FUNCTION__, data);
	}
		...
		if ((ret = sdio_reset_comm(gInstance->func[0]->card))) {
			...
			if (gInstance->func[2]) {
				...
				if (sdio_set_block_size(gInstance->func[2],
					sd_f2_blocksize)) {
					AP6211_ERR("bcmsdh_sdmmc: Failed to set F2 "
						"blocksize to %d\n", sd_f2_blocksize);
				}
