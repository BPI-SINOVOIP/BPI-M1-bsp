                              ...
                              const struct sdio_device_id *id)
{
	...
	if (func) {
		AP6211_DEBUG("bcmsdh_sdmmc: %s Enter\n", __FUNCTION__);
		AP6211_DEBUG("sdio_bcmsdh: func->class=%x\n", func->class);
		AP6211_DEBUG("sdio_vendor: 0x%04x\n", func->vendor);
		AP6211_DEBUG("sdio_device: 0x%04x\n", func->device);
		AP6211_DEBUG("Function#: 0x%04x\n", func->num);
		...
		if (func->num == 1) {
			...
			if(func->device == 0x4) { /* 4318 */
				...
				AP6211_DEBUG("NIC found, calling bcmsdh_probe...\n");
	...
	#endif
			AP6211_DEBUG("F2 found, calling bcmsdh_probe...\n");
			ret = bcmsdh_probe(&func->dev);
		}
	} else {
		ret = -ENODEV;
...
static void bcmsdh_sdmmc_remove(struct sdio_func *func)
{
	if (func) {
		...
		if (gInstance->func[2]) {
			AP6211_DEBUG("F2 found, calling bcmsdh_remove...\n");
...
static int bcmsdh_sdmmc_suspend(struct device *pdev)
{
	...
	if (!(sdio_flags & MMC_PM_KEEP_POWER)) {
		AP6211_ERR("%s: can't keep power while host is suspended\n", __FUNCTION__);
		return  -EINVAL;
	}
	...
	if (ret) {
		AP6211_ERR("%s: error while trying to keep power\n", __FUNCTION__);
		return ret;
	}
...
sdioh_interrupt_set(sdioh_info_t *sd, bool enable)
{
	...
	if (enable && !(sd->intr_handler && sd->intr_handler_arg)) {
		AP6211_ERR("%s: no handler registered, will not enable\n", __FUNCTION__);
