...
static int wl_android_get_link_speed(struct net_device *net, char *command, int total_len)
{
	...
	AP6211_DEBUG("%s: command result is %s\n", __FUNCTION__, command);
...
static int wl_android_get_rssi(struct net_device *net, char *command, int total_len)
{
	...
	if ((ssid.SSID_len == 0) || (ssid.SSID_len > DOT11_MAX_SSID_LEN)) {
		AP6211_ERR("%s: wldev_get_ssid failed\n", __FUNCTION__);
	} else {
		memcpy(command, ssid.SSID, ssid.SSID_len);
		bytes_written = ssid.SSID_len;
	}
	bytes_written += snprintf(&command[bytes_written], total_len, " rssi %d", rssi);
	AP6211_DEBUG("%s: command result is %s (%d)\n", __FUNCTION__, command, bytes_written);
	return bytes_written;
}
...
static int wl_android_set_suspendopt(struct net_device *dev, char *command, int total_len)
{
	...
	if (ret_now != suspend_flag) {
		if (!(ret = net_os_set_suspend(dev, ret_now, 1)))
			AP6211_DEBUG("%s: Suspend Flag %d -> %d\n",
...
static int wl_android_set_pno_setup(struct net_device *dev, char *command, int total_len)
{
	...
	AP6211_DEBUG("%s: command=%s, len=%d\n", __FUNCTION__, command, total_len);

	if (total_len < (strlen(CMD_PNOSETUP_SET) + sizeof(cmd_tlv_t))) {
		AP6211_ERR("%s argument=%d less min size\n", __FUNCTION__, total_len);
		goto exit_proc;
	}
	...
	if ((cmd_tlv_temp->prefix == PNO_TLV_PREFIX) &&
		...
		if ((nssid = wl_iw_parse_ssid_list_tlv(&str_ptr, ssids_local,
			...
			AP6211_ERR("SSID is not presented or corrupted ret=%d\n", nssid);
			goto exit_proc;
		} else {
			if ((str_ptr[0] != PNO_TLV_TYPE_TIME) || (tlv_size_left <= 1)) {
				AP6211_ERR("%s scan duration corrupted field size %d\n",
					__FUNCTION__, tlv_size_left);
				goto exit_proc;
			}
			str_ptr++;
			pno_time = simple_strtoul(str_ptr, &str_ptr, 16);
			AP6211_DEBUG("%s: pno_time=%d\n", __FUNCTION__, pno_time);

			if (str_ptr[0] != 0) {
				if ((str_ptr[0] != PNO_TLV_FREQ_REPEAT)) {
					AP6211_ERR("%s pno repeat : corrupted field\n",
						__FUNCTION__);
					goto exit_proc;
				}
				str_ptr++;
				pno_repeat = simple_strtoul(str_ptr, &str_ptr, 16);
				AP6211_DEBUG("%s :got pno_repeat=%d\n", __FUNCTION__, pno_repeat);
				if (str_ptr[0] != PNO_TLV_FREQ_EXPO_MAX) {
					AP6211_ERR("%s FREQ_EXPO_MAX corrupted field size\n",
						__FUNCTION__);
					goto exit_proc;
				}
				str_ptr++;
				pno_freq_expo_max = simple_strtoul(str_ptr, &str_ptr, 16);
				AP6211_DEBUG("%s: pno_freq_expo_max=%d\n",
					__FUNCTION__, pno_freq_expo_max);
			}
		}
	} else {
		AP6211_ERR("%s get wrong TLV command\n", __FUNCTION__);
		goto exit_proc;
	}
...
int wl_android_wifi_on(struct net_device *dev)
{
	...
	if (!g_wifi_on) {
		...
		if (ret != 0) {
			AP6211_ERR("failed to power up wifi chip, max retry reached **\n\n");
			goto exit;
		}
...
static int wl_android_set_fwpath(struct net_device *net, char *command, int total_len)
{
	...
	if (strstr(fw_path, "apsta") != NULL) {
		AP6211_DEBUG("GOT APSTA FIRMWARE\n");
		ap_fw_loaded = TRUE;
	} else {
		AP6211_DEBUG("GOT STA FIRMWARE\n");
		ap_fw_loaded = FALSE;
	}
	return 0;
}
...
wl_android_set_pmk(struct net_device *dev, char *command, int total_len)
{
	...
	if (error) {
		AP6211_ERR("Failed to set PMK for OKC, error = %d\n", error);
	}
#ifdef OKC_DEBUG
	AP6211_ERR("PMK is ");
	for (i = 0; i < 32; i++)
		AP6211_ERR("%02X ", pmk[i]);
...
wl_android_okc_enable(struct net_device *dev, char *command, int total_len)
{
	...
	if (error) {
		AP6211_ERR("Failed to %s OKC, error = %d\n",
...
int wl_android_set_roam_mode(struct net_device *dev, char *command, int total_len)
{
	...
	if (sscanf(command, "%*s %d", &mode) != 1) {
		AP6211_ERR("%s: Failed to get Parameter\n", __FUNCTION__);
		return -1;
	}

	error = wldev_iovar_setint(dev, "roam_off", mode);
	if (error) {
		AP6211_ERR("%s: Failed to set roaming Mode %d, error = %d\n",
		__FUNCTION__, mode, error);
		return -1;
	}
	else
		AP6211_ERR("%s: succeeded to set roaming Mode %d, error = %d\n",
		__FUNCTION__, mode, error);
	return 0;
}
...
#define PRIVATE_COMMAND_MAX_LEN	8192
	...
	{
		AP6211_ERR("%s: too long priavte command\n", __FUNCTION__);
		ret = -EINVAL;
	}
	command = kmalloc(priv_cmd.total_len, GFP_KERNEL);
	if (!command)
	{
		AP6211_ERR("%s: failed to allocate memory\n", __FUNCTION__);
		ret = -ENOMEM;
		goto exit;
	}
	...
	AP6211_DEBUG("%s: Android private cmd \"%s\" on %s\n", __FUNCTION__, command, ifr->ifr_name);

	if (strnicmp(command, CMD_START, strlen(CMD_START)) == 0) {
		AP6211_DEBUG("%s, Received regular START command\n", __FUNCTION__);
	...
	if (!g_wifi_on) {
		AP6211_ERR("%s: Ignore private cmd \"%s\" - iface %s is down\n",
			__FUNCTION__, command, ifr->ifr_name);
		ret = 0;
		goto exit;
	}
	...
	if (bytes_written >= 0) {
		...
		if (copy_to_user(priv_cmd.buf, command, bytes_written)) {
			AP6211_ERR("%s: failed to copy data to user buffer\n", __FUNCTION__);
			ret = -EFAULT;
		}
	}
	else {
...
wl_free_rssi_cache(wl_rssi_cache_ctrl_t *rssi_cache_ctrl)
{
	...
	for (;node;) {
		AP6211_DEBUG("%s: Free %d with BSSID %pM\n",
...
wl_delete_dirty_rssi_cache(wl_rssi_cache_ctrl_t *rssi_cache_ctrl)
{
	...
	for (;node;) {
		...
		if (node->dirty >= max || node->dirty >= RSSICACHE_LEN) {
			...
			AP6211_DEBUG("%s: Del %d with BSSID %pM\n",
				__FUNCTION__, i, &node->BSSID);
			kfree(node);
			if (tmp == 1) {
				node = *rssi_head;
...
wl_update_connected_rssi_cache(wl_rssi_cache_ctrl_t *rssi_cache_ctrl, struct net_device *net)
{
	...
	for (;node;) {
		if (!memcmp(&node->BSSID, &bssid, ETHER_ADDR_LEN)) {
			AP6211_DEBUG("%s: Update %d with BSSID %pM, RSSI=%d\n",
	...
	for (i = 0; i < ss_list->count; i++) {
		...
		for (;node;) {
			if (!memcmp(&node->BSSID, &bi->BSSID, ETHER_ADDR_LEN)) {
				AP6211_DEBUG("%s: Update %d with BSSID %pM, RSSI=%d, SSID \"%s\"\n",
		...
		if (!leaf) {
			AP6211_ERR("%s: Memory alloc failure %d\n",
				__FUNCTION__, sizeof(wl_rssi_cache_t));
			return;
		}
		AP6211_DEBUG("%s: Add %d with cached BSSID %pM, RSSI=%d, SSID \"%s\" in the leaf\n",
				__FUNCTION__, k, &bi->BSSID, dtoh16(bi->RSSI), bi->SSID);
...
wl_get_avg_rssi(wl_rssi_cache_ctrl_t *rssi_cache_ctrl, void *addr)
{
	...
	if (rssi >= -2)
		rssi = -2;
	if (rssi == -200) {
		AP6211_ERR("%s: BSSID %pM does not in RSSI cache\n",
		__FUNCTION__, addr);
	}
	return (int16)rssi;
}
...
wl_delete_dirty_bss_cache(wl_bss_cache_ctrl_t *bss_cache_ctrl)
{
	...
	for (;node;) {
		...
		if (node->dirty >= BSSCACHE_LEN) {
			...
			AP6211_DEBUG("%s: Del %d with BSSID %pM, RSSI=%d, SSID \"%s\"\n",
				__FUNCTION__, i, &node->results.bss_info->BSSID,
	...
	for (i=0; i < ss_list->count; i++) {
		...
		for (;node;) {
			if (!memcmp(&node->results.bss_info->BSSID, &bi->BSSID, ETHER_ADDR_LEN)) {
				...
				if (!leaf) {
					AP6211_ERR("%s: Memory alloc failure %d and keep old BSS info\n",
...
wl_run_bss_cache_timer(wl_bss_cache_ctrl_t *bss_cache_ctrl, int kick_off)
{
	...
	if (*timer) {
		if (kick_off) {
			...
			add_timer(*timer);
			AP6211_DEBUG("%s: timer starts\n", __FUNCTION__);
		} else {
			del_timer_sync(*timer);
			AP6211_DEBUG("%s: timer stops\n", __FUNCTION__);
		}
	}
}
...
wl_init_bss_cache_ctrl(wl_bss_cache_ctrl_t *bss_cache_ctrl)
{
	...
	if (!bss_cache_ctrl->m_timer) {
		AP6211_ERR("%s: Memory alloc failure\n", __FUNCTION__ );
		return;
	}
...
int wl_android_wifictrl_func_add(void)
{
	...
	ret = wifi_add_dev();
	if (ret) {
		AP6211_ERR("%s: platform_driver_register failed\n", __FUNCTION__);
		return ret;
	}
	...
	if (down_timeout(&wifi_control_sem,  msecs_to_jiffies(1000)) != 0) {
		ret = -EINVAL;
		AP6211_ERR("%s: platform_driver_register timeout\n", __FUNCTION__);
	}
...
void* wl_android_prealloc(int section, unsigned long size)
{
	...
	if (wifi_control_data && wifi_control_data->mem_prealloc) {
		...
		if (alloc_ptr) {
			AP6211_DEBUG("success alloc section %d\n", section);
			if (size != 0L)
	...
	AP6211_ERR("can't alloc section %d\n", section);
	return NULL;
}
...
{
	int ret = 0;
	AP6211_DEBUG("## Calling platform_driver_register\n");
...
static void wifi_del_dev(void)
{
	AP6211_DEBUG("## Unregister platform_driver_register\n");
