...
_sb_scan(si_info_t *sii, uint32 sba, void *regs, uint bus, uint32 sbba, uint numcores)
{
	...
	if (bus >= SB_MAXBUSES) {
		AP6211_ERR("_sb_scan: bus 0x%08x at level %d is too deep to scan\n", sbba, bus);
		return 0;
	}
	AP6211_DEBUG("_sb_scan: scan bus 0x%08x assume %u cores\n", sbba, numcores);
	...
	for (i = 0, next = sii->numcores; i < numcores && next < SB_BUS_MAXCORES; i++, next++) {
		...
		if ((BUSTYPE(sii->pub.bustype) == SI_BUS) && (sii->coresba[next] == sba)) {
			AP6211_DEBUG("_sb_scan: reuse mapped regs %p for core %u\n", regs, next);
			sii->regs[next] = regs;
		}
		...
		if (sii->coreid[next] == CC_CORE_ID) {
			...
			if (((ccrev == 4) || (ccrev >= 6)))
				...
				if (chip == BCM4306_CHIP_ID)	/* < 4306c0 */
					...
					AP6211_ERR("sb_chip2numcores: unsupported chip 0x%x\n",
					          chip);
					ASSERT(0);
					numcores = 1;
				}
			}
			AP6211_DEBUG("_sb_scan: there are %u cores in the chip %s\n", numcores,
				sii->pub.issim ? "QT" : "");
		}
	...
	AP6211_DEBUG("_sb_scan: found %u cores on bus 0x%08x\n", i, sbba);
...
sb_admatch(si_info_t *sii, uint asidx)
{
	...
	default:
		AP6211_ERR("%s: Address space index (%d) out of range\n", __FUNCTION__, asidx);
		return 0;
	}
...
sb_core_disable(si_t *sih, uint32 bits)
{
	...
	if (R_SBREG(sii, &sb->sbtmstatehigh) & SBTMH_BUSY)
		AP6211_ERR("%s: target state still busy\n", __FUNCTION__);
