...
#ifdef DHD_DEBUG
#ifndef SRCBASE
#define SRCBASE        "drivers/net/wireless/ap6211"
#endif
#define DHD_COMPILED "\nCompiled in " SRCBASE
#else
#define DHD_COMPILED
	...
	if (dhd_pkt_filter_enable && (!value ||
		...
		for (i = 0; i < dhd->pktfilter_count; i++) {
			...
			if (value && (i == dhd->pktfilter_count -1) &&
				...
				AP6211_DEBUG("Do not turn on ARP white list pkt filter:"
...
static int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	...
	AP6211_DEBUG("%s: enter, value = %d in_suspend=%d\n",
...
#endif
				/* Kernel suspended */
				AP6211_ERR("%s: force extra Suspend setting\n", __FUNCTION__);
...
dhd_ifname2idx(dhd_info_t *dhd, char *name)
{
	...
	AP6211_DEBUG("%s: return idx %d for \"%s\"\n", __FUNCTION__, i, name);
...
dhd_ifname(dhd_pub_t *dhdp, int ifidx)
{
	...
	if (ifidx < 0 || ifidx >= DHD_MAX_IFS) {
		AP6211_ERR("%s: ifidx %d out of range\n", __FUNCTION__, ifidx);
		return "<if_bad>";
	}

	if (dhd->iflist[ifidx] == NULL) {
		AP6211_ERR("%s: null i/f %d\n", __FUNCTION__, ifidx);
		return "<if_null>";
	}
...
_dhd_set_multicast_list(dhd_info_t *dhd, int ifidx)
{
	...
	if (!(bufp = buf = MALLOC(dhd->pub.osh, buflen))) {
		AP6211_ERR("%s: out of memory for mcast_list, cnt %d\n",
	...
	if (ret < 0) {
		AP6211_ERR("%s: set mcast_list failed, cnt %d\n",
			dhd_ifname(&dhd->pub, ifidx), cnt);
	...
	if (!(buf = MALLOC(dhd->pub.osh, buflen))) {
		AP6211_ERR("%s: out of memory for allmulti\n", dhd_ifname(&dhd->pub, ifidx));
		return;
	}
	allmulti = htol32(allmulti);

	if (!bcm_mkiovar("allmulti", (void*)&allmulti, sizeof(allmulti), buf, buflen)) {
		AP6211_ERR("%s: mkiovar failed for allmulti, datalen %d buflen %u\n",
	...
	ret = dhd_wl_ioctl(&dhd->pub, ifidx, &ioc, ioc.buf, ioc.len);
	if (ret < 0) {
		AP6211_ERR("%s: set allmulti %d failed\n",
	...
	ret = dhd_wl_ioctl(&dhd->pub, ifidx, &ioc, ioc.buf, ioc.len);
	if (ret < 0) {
		AP6211_ERR("%s: set promisc %d failed\n",
		           dhd_ifname(&dhd->pub, ifidx), ltoh32(allmulti));
	}
}
...
_dhd_set_mac_address(dhd_info_t *dhd, int ifidx, struct ether_addr *addr)
{
	...
	if (!bcm_mkiovar("cur_etheraddr", (char*)addr, ETHER_ADDR_LEN, buf, 32)) {
		AP6211_ERR("%s: mkiovar failed for cur_etheraddr\n", dhd_ifname(&dhd->pub, ifidx));
		return -1;
	}
	memset(&ioc, 0, sizeof(ioc));
	ioc.cmd = WLC_SET_VAR;
	...
	ret = dhd_wl_ioctl(&dhd->pub, ifidx, &ioc, ioc.buf, ioc.len);
	if (ret < 0) {
		AP6211_ERR("%s: set cur_etheraddr failed\n", dhd_ifname(&dhd->pub, ifidx));
	} else {
...
dhd_op_if(dhd_if_t *ifp)
{
	...
	case DHD_IF_ADD:
		 ...
		 */
		if (ifp->net != NULL) {
			AP6211_ERR("%s: ERROR: netdev:%s already exists, try free & unregister \n",
		...
		if (!(ifp->net = alloc_etherdev(sizeof(dhd)))) {
			AP6211_ERR("%s: OOM - alloc_etherdev\n", __FUNCTION__);
			ret = -ENOMEM;
		}
		if (ret == 0) {
			...
			if ((err = dhd_net_attach(&dhd->pub, ifp->idx)) != 0) {
				AP6211_ERR("%s: dhd_net_attach failed, err %d\n",
					__FUNCTION__, err);
				ret = -EOPNOTSUPP;
			} else {
#if defined(SOFTAP)
...
#endif
				AP6211_DEBUG(" ==== pid:%x, net_device for if:%s created ===\n\n",
	...
	case DHD_IF_DEL:
		...
		if (ifp->net != NULL) {
			AP6211_DEBUG("%s: got 'DHD_IF_DEL' state\n", __FUNCTION__);
			netif_stop_queue(ifp->net);
#ifdef WL_CFG80211
			if (dhd->dhd_state & DHD_ATTACH_STATE_CFG80211) {
		...
		for (i = 0; i < DHD_MAX_IFS; i++) {
			if (dhd->iflist[i]) {
				...
				if (dhd->iflist[i] == NULL) {
					AP6211_DEBUG("%s: interface %d just been removed,!\n", __FUNCTION__, i);
					continue;
				}
#ifdef SOFTAP
				if (in_ap && dhd->set_macaddress == i+1)  {
					AP6211_DEBUG("attempt to set MAC for %s in AP Mode,"
				...
				if (in_ap && dhd->iflist[i]->set_multicast)  {
					AP6211_DEBUG("attempt to set MULTICAST list for %s"
					 "in AP Mode, blocked. \n", dhd->iflist[i]->net->name);
...
dhd_start_xmit(struct sk_buff *skb, struct net_device *net)
{
	...
	if (dhd->pub.busstate == DHD_BUS_DOWN || dhd->pub.hang_was_sent) {
		AP6211_ERR("%s: xmit rejected pub.up=%d busstate=%d \n",
			__FUNCTION__, dhd->pub.up, dhd->pub.busstate);
		netif_stop_queue(net);
		/* Send Event when bus down detected during data session */
		if (dhd->pub.up) {
			AP6211_ERR("%s: Event HANG sent up\n", __FUNCTION__);
	...
	if (skb_headroom(skb) < dhd->pub.hdrlen + htsfdlystat_sz) {
		...
		AP6211_DEBUG("%s: insufficient headroom\n",
		          dhd_ifname(&dhd->pub, ifidx));
		dhd->pub.tx_realloc++;
		...
		dev_kfree_skb(skb);
		if ((skb = skb2) == NULL) {
			AP6211_ERR("%s: skb_realloc_headroom failed\n",
			           dhd_ifname(&dhd->pub, ifidx));
			ret = -ENOMEM;
			goto done;
		}
	...
	if (!(pktbuf = PKTFRMNATIVE(dhd->pub.osh, skb))) {
		AP6211_ERR("%s: PKTFRMNATIVE failed\n",
		           dhd_ifname(&dhd->pub, ifidx));
...
dhd_rx_frame(dhd_pub_t *dhdp, int ifidx, void *pktbuf, int numpkt, uint8 chan)
{
	...
	for (i = 0; pktbuf && i < numpkt; i++, pktbuf = pnext) {
		...
		if (ifp == NULL) {
			AP6211_ERR("%s: ifp is NULL. drop packet\n",
				__FUNCTION__);
			PKTFREE(dhdp->osh, pktbuf, TRUE);
			continue;
		}
		...
		if (!ifp->net || ifp->net->reg_state != NETREG_REGISTERED || !dhd->pub.up) {
			...
			AP6211_ERR("%s: net device is NOT registered yet. drop packet\n",
			__FUNCTION__);
			PKTFREE(dhdp->osh, pktbuf, TRUE);
			continue;
		}
		...
		AP6211_ERR("RX DUMP - %s\n", _get_packet_type_str(protocol));

#ifdef DHD_RX_FULL_DUMP
		if (protocol != ETHER_TYPE_BRCM) {
			for (k = 0; k < skb->len; k++) {
				AP6211_ERR("%02X ", dump_data[k]));
				if ((k & 15) == 15)
					AP6211_ERR("\n");
			}
			AP6211_ERR("\n");
		...
		if (protocol != ETHER_TYPE_BRCM) {
			if (dump_data[0] == 0xFF) {
				...
				AP6211_ERR("%s: MULTICAST: " MACDBG "\n",
					__FUNCTION__, MAC2STRDBG(dump_data));
			}

			if (protocol == ETHER_TYPE_802_1X) {
				AP6211_ERR("ETHER_TYPE_802_1X: "
...
dhd_toe_get(dhd_info_t *dhd, int ifidx, uint32 *toe_ol)
{
	...
	if ((ret = dhd_wl_ioctl(&dhd->pub, ifidx, &ioc, ioc.buf, ioc.len)) < 0) {
		...
		if (ret == -EIO) {
			AP6211_ERR("%s: toe not supported by device\n",
				dhd_ifname(&dhd->pub, ifidx));
			return -EOPNOTSUPP;
		}

		AP6211_DEBUG("%s: could not get toe_ol: ret=%d\n", dhd_ifname(&dhd->pub, ifidx), ret);
		return ret;
	}
...
dhd_toe_set(dhd_info_t *dhd, int ifidx, uint32 toe_ol)
{
	...
	if ((ret = dhd_wl_ioctl(&dhd->pub, ifidx, &ioc, ioc.buf, ioc.len)) < 0) {
		AP6211_ERR("%s: could not set toe_ol: ret=%d\n",
			dhd_ifname(&dhd->pub, ifidx), ret);
		return ret;
	}
	...
	if ((ret = dhd_wl_ioctl(&dhd->pub, ifidx, &ioc, ioc.buf, ioc.len)) < 0) {
		AP6211_ERR("%s: could not set toe: ret=%d\n", dhd_ifname(&dhd->pub, ifidx), ret);
		return ret;
	}
...
dhd_ethtool(dhd_info_t *dhd, void *uaddr)
{
	...
	case ETHTOOL_GDRVINFO:
		...
		if (copy_to_user(uaddr, &info, sizeof(info)))
			return -EFAULT;
		AP6211_DEBUG("%s: given %*s, returning %s\n", __FUNCTION__,
...
static bool dhd_check_hang(struct net_device *net, dhd_pub_t *dhdp, int error)
{
	...
	if (dhd->thr_sysioc_ctl.thr_pid < 0) {
		AP6211_ERR("%s : skipped due to negative pid - unloading?\n", __FUNCTION__);
		return FALSE;
	}

	if ((error == -ETIMEDOUT) || (error == -EREMOTEIO) ||
		((dhdp->busstate == DHD_BUS_DOWN) && (!dhdp->dongle_reset))) {
		AP6211_ERR("%s: Event HANG send up due to  re=%d te=%d e=%d s=%d\n", __FUNCTION__,
...
dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	if (dhd->pub.hang_was_sent) {
		AP6211_ERR("%s: HANG was sent up earlier\n", __FUNCTION__);
	...
	ifidx = dhd_net2idx(dhd, net);
	AP6211_DEBUG("%s: ifidx %d, cmd 0x%04x\n", __FUNCTION__, ifidx, cmd);
	...
	if (ioc.buf) {
		if (ioc.len == 0) {
			AP6211_DEBUG("%s: ioc.len=0, returns BCME_BADARG \n", __FUNCTION__);
	...
	if (ioc.buf) {
		...
		if (strcmp("tsport", ioc.buf) == 0) {
			if (ioc.set) {
				...
				AP6211_ERR("current timestamp port: %d \n", tsport);
			}
			return BCME_OK;
		}
	}
...
dhd_open(struct net_device *net)
{
	...
	if (mutex_is_locked(&_dhd_sdio_mutex_lock_) != 0) {
		AP6211_ERR("%s : dhd_open: call dev open before insmod complete!\n", __FUNCTION__);
	}
	mutex_lock(&_dhd_sdio_mutex_lock_);
#endif 

        AP6211_DEBUG("%s, firmware path %s\n", __func__, firmware_path);
	...
	if (ret != 0) {
		AP6211_ERR("%s: failed with code %d\n", __FUNCTION__, ret);
		ret = -1;
		goto exit;
	}
#endif
	...
	ifidx = dhd_net2idx(dhd, net);
	AP6211_DEBUG("%s: ifidx %d\n", __FUNCTION__, ifidx);

	if (ifidx < 0) {
		AP6211_ERR("%s: Error: called with invalid IF\n", __FUNCTION__);
		ret = -1;
		goto exit;
	}

	if (!dhd->iflist[ifidx] || dhd->iflist[ifidx]->state == DHD_IF_DEL) {
		AP6211_ERR("%s: Error: called when IF already deleted\n", __FUNCTION__);
		ret = -1;
		goto exit;
	}
	...
	if (ifidx == 0) {
		...
		if (unlikely(wl_cfg80211_up(NULL))) {
			AP6211_ERR("%s: failed to bring up cfg80211\n", __FUNCTION__);
			ret = -1;
			goto exit;
		}
#endif /* WL_CFG80211 */
...
int dhd_do_driver_init(struct net_device *net)
{
	...
	if (!net) {
		AP6211_ERR("Primary Interface not initialized \n");
		return -EINVAL;
	}
	...
	if (mutex_is_locked(&_dhd_sdio_mutex_lock_) != 0) {
		AP6211_ERR("%s : dhdsdio_probe is already running!\n", __FUNCTION__);
		return 0;
	}
	 ...
	 */
	if (dhd->pub.busstate == DHD_BUS_DATA) {
		AP6211_DEBUG("Driver already Inititalized. Nothing to do");
		return 0;
	}

	if (dhd_open(net) < 0) {
		AP6211_ERR("Driver Init Failed \n");
		return -1;
	}
...
{
	if (MALLOCED(osh)) {
		AP6211_ERR("%s: MEMORY LEAK %d bytes\n", __FUNCTION__, MALLOCED(osh));
	}
	osl_detach(osh);
...
dhd_del_if(dhd_info_t *dhd, int ifidx)
{
	...
	AP6211_DEBUG("%s: idx %d\n", __FUNCTION__, ifidx);

	ASSERT(dhd && ifidx && (ifidx < DHD_MAX_IFS));
	ifp = dhd->iflist[ifidx];
	if (!ifp) {
		AP6211_ERR("%s: Null interface\n", __FUNCTION__);
		return;
	}
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	if (!(dhd = MALLOC(osh, sizeof(dhd_info_t)))) {
		AP6211_ERR("%s: OOM - alloc dhd_info\n", __FUNCTION__);
		goto fail;
	}
	...
	if (!dhd->wl_wifi || !dhd->wl_rxwake || !dhd->wl_ctrlwake || !dhd->wl_wdwake) {
		AP6211_ERR("%s: mem alloc for wake lock failed\n", __FUNCTION__);
		goto fail;
	}
	...
	if (dhd_prot_attach(&dhd->pub) != 0) {
		AP6211_ERR("dhd_prot_attach failed\n");
		goto fail;
	}
	...
	if (unlikely(wl_cfg80211_attach(net, &dhd->pub))) {
		AP6211_ERR("wl_cfg80211_attach failed\n");
		goto fail;
	}
	...
	if (!(dhd_state &  DHD_ATTACH_STATE_CFG80211)) {
		if (wl_iw_attach(net, (void *)&dhd->pub) != 0) {
			AP6211_ERR("wl_iw_attach failed\n");
			goto fail;
		}
...
fail:
	if (dhd_state < DHD_ATTACH_STATE_DHD_ALLOC) {
		...
		AP6211_DEBUG("%s: Calling dhd_detach dhd_state 0x%x &dhd->pub %p\n",
...
dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	if  ((dhd->pub.busstate == DHD_BUS_DOWN) &&
		...
		if (!(dhd_bus_download_firmware(dhd->pub.bus, dhd->pub.osh,
			fw_path, nv_path))) {
			AP6211_ERR("%s: dhdsdio_probe_download failed. firmware = %s nvram = %s\n",
	...
	if ((ret = dhd_bus_init(&dhd->pub, FALSE)) != 0) {
		...
		AP6211_ERR("%s, dhd_bus_init failed %d\n", __FUNCTION__, ret);
#ifdef DHDTHREAD
		if (dhd->threads_only)
			dhd_os_sdunlock(dhdp);
#endif /* DHDTHREAD */
	...
	if (bcmsdh_register_oob_intr(dhdp)) {
		...
		AP6211_ERR("%s Host failed to register for OOB\n", __FUNCTION__);
#ifdef DHDTHREAD
		if (dhd->threads_only)
			dhd_os_sdunlock(dhdp);
#endif /* DHDTHREAD */
	...
	if (dhd->pub.busstate != DHD_BUS_DATA) {
		...
		dhd_os_spin_unlock(&dhd->pub, flags);
		del_timer_sync(&dhd->timer);
		AP6211_ERR("%s failed bus is not ready\n", __FUNCTION__);
#ifdef DHDTHREAD
		if (dhd->threads_only)
			dhd_os_sdunlock(dhdp);
#endif /* DHDTHREAD */
...
dhd_get_concurrent_capabilites(dhd_pub_t *dhd)
{
	...
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, buf, sizeof(buf),
		FALSE, 0)) < 0) {
		AP6211_ERR("%s: Get Capability failed (error=%d)\n",
			__FUNCTION__, ret);
		return 0;
	}
	if (strstr(buf, "vsdb")) {
	...
	if (strstr(buf, "p2p") == NULL) {
		AP6211_DEBUG("Chip does not support p2p\n");
		return 0;
	}
	else {
		...
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, buf, sizeof(buf),
			FALSE, 0)) < 0) {
			AP6211_ERR("%s: Get P2P failed (error=%d)\n", __FUNCTION__, ret);
			return 0;
		}
		else {
			if (buf[0] == 1) {
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if (!ret) {
		...
		if (ret < 0) {
			AP6211_ERR("%s: can't set MAC address , error=%d\n", __FUNCTION__, ret);
			return BCME_NOTUP;
		}
		...
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, buf, sizeof(buf),
			FALSE, 0)) < 0) {
			AP6211_ERR("%s: can't get MAC address , error=%d\n", __FUNCTION__, ret);
			return BCME_NOTUP;
		}
	...
	if ((!op_mode && strstr(fw_path, "_apsta") != NULL) ||
		...
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
			...
			AP6211_ERR("%s mpc for HostAPD failed  %d\n", __FUNCTION__, ret);
		}
#endif
		...
		if (dhd->op_mode & DHD_FLAG_P2P_MODE) {
			...
			if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR,
				...
				AP6211_ERR("%s APSTA for P2P failed ret= %d\n", __FUNCTION__, ret);
			}
			...
			if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR,
				iovbuf, sizeof(iovbuf), TRUE, 0)) < 0) {
				AP6211_ERR("%s p2p_da_override ret= %d\n", __FUNCTION__, ret);
			} else {
				AP6211_DEBUG("dhd_preinit_ioctls: p2p_da_override succeeded\n");
			}
		}
#else
	...
	AP6211_ERR("Firmware up: op_mode=0x%04x, "
	...
	if (dhd->dhd_cspec.ccode[0] != 0) {
		...
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0)
			AP6211_ERR("%s: country code setting failed\n", __FUNCTION__);
	}

	/* Set Listen Interval */
	bcm_mkiovar("assoc_listen", (char *)&listen_interval, 4, iovbuf, sizeof(iovbuf));
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0)
		AP6211_ERR("%s assoc_listen failed %d\n", __FUNCTION__, ret);
	...
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_ROAM_TRIGGER, roam_trigger,
		...
		AP6211_ERR("%s: roam trigger set failed %d\n", __FUNCTION__, ret);
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_ROAM_SCAN_PERIOD, roam_scan_period,
		sizeof(roam_scan_period), TRUE, 0)) < 0)
		AP6211_ERR("%s: roam scan period set failed %d\n", __FUNCTION__, ret);
	if ((dhd_wl_ioctl_cmd(dhd, WLC_SET_ROAM_DELTA, roam_delta,
		sizeof(roam_delta), TRUE, 0)) < 0)
		AP6211_ERR("%s: roam delta set failed %d\n", __FUNCTION__, ret);
	bcm_mkiovar("fullroamperiod", (char *)&roam_fullscan_period, 4, iovbuf, sizeof(iovbuf));
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0)
		AP6211_ERR("%s: roam fullscan period set failed %d\n", __FUNCTION__, ret);
#endif /* ROAM_ENABLE */
	...
	if (ap_fw_loaded == FALSE)
		...
		if (!(dhd->op_mode & DHD_FLAG_HOSTAP_MODE)) {
			if ((res = dhd_keep_alive_onoff(dhd)) < 0)
				AP6211_ERR("%s set keeplive failed %d\n",
				__FUNCTION__, res);
		}
	}
	...
	if ((ret  = dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, iovbuf, sizeof(iovbuf), FALSE, 0)) < 0) {
		AP6211_ERR("%s read Event mask failed %d\n", __FUNCTION__, ret);
		goto done;
	}
	...
	bcm_mkiovar("event_msgs", eventmask, WL_EVENTING_MASK_LEN, iovbuf, sizeof(iovbuf));
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0) {
		AP6211_ERR("%s Set Event mask failed %d\n", __FUNCTION__, ret);
		goto done;
	}
	...
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0)
		AP6211_ERR("%s wl nmode 0 failed %d\n", __FUNCTION__, ret);
#else
#ifdef AMPDU_HOSTREORDER
	...
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_UP, (char *)&up, sizeof(up), TRUE, 0)) < 0) {
		AP6211_ERR("%s Setting WL UP failed %d\n", __FUNCTION__, ret);
		goto done;
	}
#endif
	...
	if ((ret  = dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, buf, sizeof(buf), FALSE, 0)) < 0)
		AP6211_ERR("%s failed %d\n", __FUNCTION__, ret);
	else {
		bcmstrtok(&ptr, "\n", 0);
		/* Print fw version info */
		AP6211_ERR("Firmware version = %s\n", buf);
		...
		if (strstr(buf, MANUFACTRING_FW) != NULL) {
			...
			AP6211_ERR("%s : adjust IOCTL response time for Manufactring Firmware\n",
			__FUNCTION__);
		}
	}
...
int dhd_change_mtu(dhd_pub_t *dhdp, int new_mtu, int ifidx)
{
	...
	if (netif_running(dev)) {
		AP6211_ERR("%s: Must be down to change its MTU", dev->name);
		return BCME_NOTDOWN;
	}
...
#define DHD_MAX_MTU 1752
	...
	if ((new_mtu < DHD_MIN_MTU) || (new_mtu > DHD_MAX_MTU)) {
		AP6211_ERR("%s: MTU size %d is invalid.\n", __FUNCTION__, new_mtu);
		return BCME_BADARG;
	}
...
aoe_update_host_ipv4_table(dhd_pub_t *dhd_pub, u32 ipa, bool add, int idx)
{
	...
	AP6211_DEBUG("%s: hostip table read from Dongle:\n", __FUNCTION__);
#ifdef AOE_DBG
	...
	for (i = 0; i < MAX_IPV4_ENTRIES; i++) {
		if (add && (ipv4_buf[i] == 0)) {
				...
				AP6211_DEBUG("%s: Saved new IP in temp arp_hostip[%d]\n",
				__FUNCTION__, i);
		} else if (ipv4_buf[i] == ipa) {
			ipv4_buf[i]	= 0;
			AP6211_DEBUG("%s: removed IP:%x from temp table %d\n",
				__FUNCTION__, ipa, i);
		}
		...
		if (ipv4_buf[i] != 0) {
			...
			AP6211_DEBUG("%s: added IP:%x to dongle arp_hostip[%d]\n\n",
	...
	AP6211_DEBUG("%s: read back arp_hostip table:\n", __FUNCTION__);
	dhd_print_buf(ipv4_buf, 32, 4); /* max 8 IPs 4b each */
#endif
}
	...
	void *ptr)
{
	...
	if (dhd_pub->arp_version == 1) {
		...
		if (idx < DHD_MAX_IFS)
			AP6211_DEBUG("ifidx : %p %s %d\n", dhd->iflist[idx]->net,
				dhd->iflist[idx]->name, dhd->iflist[idx]->idx);
		else {
			AP6211_ERR("Cannot find ifidx for(%s) set to 0\n", ifa->ifa_label);
			idx = 0;
		}
	}
	...
	switch (event) {
		case NETDEV_UP:
			AP6211_DEBUG("%s: [%s] Up IP: 0x%x\n",
				__FUNCTION__, ifa->ifa_label, ifa->ifa_address);

			if (dhd->pub.busstate != DHD_BUS_DATA) {
				AP6211_ERR("%s: bus not ready, exit\n", __FUNCTION__);
				if (dhd->pend_ipaddr) {
					AP6211_ERR("%s: overwrite pending ipaddr: 0x%x\n",
...
#ifdef AOE_IP_ALIAS_SUPPORT
			AP6211_DEBUG("%s:add aliased IP to AOE hostip cache\n",
				__FUNCTION__);
		...
		case NETDEV_DOWN:
			AP6211_DEBUG("%s: [%s] Down IP: 0x%x\n",
				__FUNCTION__, ifa->ifa_label, ifa->ifa_address);
			dhd->pend_ipaddr = 0;
#ifdef AOE_IP_ALIAS_SUPPORT
			AP6211_DEBUG("%s:interface is down, AOE clr all for this if\n",
				__FUNCTION__);
		...
		default:
			AP6211_DEBUG("%s: do noting for [%s] Event: %lu\n",
...
dhd_net_attach(dhd_pub_t *dhdp, int ifidx)
{
	...
	if (ifidx == 0) {
		...
		if (!memcmp(temp_addr, dhd->iflist[0]->mac_addr,
			ETHER_ADDR_LEN)) {
			AP6211_ERR("%s interface [%s]: set locally administered bit in MAC\n",
			__func__, net->name);
			temp_addr[0] |= 0x02;
		}
	}
	...
	if ((err = register_netdev(net)) != 0) {
		AP6211_ERR("couldn't register the net device, err %d\n", err);
		goto fail;
	}
	AP6211_ERR("Broadcom Dongle Host Driver: register interface [%s] MAC: "MACDBG"\n",
		net->name,
...
void dhd_detach(dhd_pub_t *dhdp)
{
	...
	AP6211_DEBUG("%s: Enter state 0x%x\n", __FUNCTION__, dhd->dhd_state);
#ifdef ARP_OFFLOAD_SUPPORT
	...
	if (dhdp) {
		...
		for (i = 0; i < ARRAYSIZE(dhdp->reorder_bufs); i++) {
			if (dhdp->reorder_bufs[i]) {
				...
				AP6211_DEBUG("free flow id buf %d, maxidx is %d, buf_size %d\n",
...
dhd_module_init(void)
{
	...
	AP6211_DEBUG("%s: Enter\n", __FUNCTION__);
   AP6211_INFO("--->The Drive has been modified for BananaPro by LeMaker team<---\n");

	ap6211_gpio_wifi_init();
	...
	if (gpio_request(WL_HOST_WAKE_DEF_GPIO, "wl_host_wake")) {
		AP6211_ERR("[%s] get wl_host_wake gpio failed\n", __FUNCTION__);
		wl_host_wake = -1;
		return -1;
	}
	...
	AP6211_DEBUG("got gpio%d, mapped to irqno%d\n", wl_host_wake, wl_host_wake_irqno);
	...
	do {
		...
		AP6211_ERR("Invalid module parameters.\n");
		return -EINVAL;
	} while (0);
#endif 
	...
	do {
		...
		AP6211_ERR("failed to power up wifi chip, retry again (%d left) **\n\n",
			retry+1);
		dhd_bus_unreg_sdio_notify();
#if defined(CONFIG_WIFI_CONTROL_FUNC)
		wl_android_wifictrl_func_del();
	...
	if (!chip_up) {
		AP6211_ERR("failed to power up wifi chip, max retry reached, exits **\n\n");
		return -ENODEV;
	}
#else
	dhd_customer_gpio_wlan_ctrl(WLAN_POWER_ON);
	...
	if (!error)
		...
		AP6211_ERR("%s: sdio_register_driver failed\n", __FUNCTION__);
		goto fail_1;
	}
	...
	if ((down_timeout(&dhd_registration_sem,
		...
		(dhd_registration_check != TRUE)) {
		error = -ENODEV;
		AP6211_ERR("%s: sdio_register_driver timeout or error \n", __FUNCTION__);
		goto fail_2;
	}
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)) */
#if defined(WL_CFG80211)
...
dhd_sendup_event(dhd_pub_t *dhdp, wl_event_msg_t *event, void *data)
{
	...
	case WLC_E_BTA_HCI_EVENT: {
		...
		if ((p = PKTGET(dhdp->osh, pktlen, FALSE))) {
			...
			AP6211_ERR("%s: unable to alloc sk_buf", __FUNCTION__);
		}
		break;
	} /* case WLC_E_BTA_HCI_EVENT */
#endif /* WLBTAMP */
...
dhd_dev_reset(struct net_device *dev, uint8 flag)
{
	...
	if (flag == TRUE) {
		...
		if (dhd_wl_ioctl_cmd(&dhd->pub, WLC_DOWN, NULL, 0, TRUE, 0) < 0) {
			AP6211_DEBUG("%s: wl down failed\n", __FUNCTION__);
		}
	}

	ret = dhd_bus_devreset(&dhd->pub, flag);
	if (ret) {
		AP6211_ERR("%s: dhd_bus_devreset: %d\n", __FUNCTION__, ret);
		return ret;
	}
...
void dhd_set_version_info(dhd_pub_t *dhdp, char *fw)
{
	...
	i = snprintf(&info_string[i], sizeof(info_string) - i,
        ...
        AP6211_ERR("Chip: %x Rev %x Pkg %x", dhd_bus_chip_id(dhdp), dhd_bus_chiprev_id(dhdp), dhd_bus_chippkg_id(dhdp));
}
...
int dhd_ioctl_entry_local(struct net_device *net, wl_ioctl_t *ioc, int cmd)
{
	...
	if (!net || !netdev_priv(net)) {
		AP6211_ERR("%s invalid parameter\n", __FUNCTION__);
		return -EINVAL;
	}
...
static void dhd_dump_htsfhisto(histo_t *his, char *s)
{
	...
	for (i = 0; i < (NUMBIN-2); i++) {
		curval += 500;
		AP6211_DUMP("%d ",  his->bin[i]);
		pktcnt += his->bin[i];
	}
	AP6211_DUMP(" max: %d TotPkt: %d neg: %d [%s]\n", his->bin[NUMBIN-2], pktcnt,
		his->bin[NUMBIN-1], s);
}
...
static void dhd_dump_latency(void)
{
	...
	AP6211_DEBUG("T1       T2       T3       T4           d1  d2   t4-t1     i    \n");
	for (i = 0; i < TSMAX; i++) {
		d1 = ts[i].t2 - ts[i].t1;
		d2 = ts[i].t3 - ts[i].t2;
		d3 = ts[i].t4 - ts[i].t3;
		...
		AP6211_DUMP("%08X %08X %08X %08X \t%d %d %d   %d i=%d\n",
			ts[i].t1, ts[i].t2, ts[i].t3, ts[i].t4,
			d1, d2, d3, d4, i);
	}

	AP6211_DEBUG("current idx = %d \n", tsidx);

	AP6211_DEBUG("Highest latency %d pkt no.%d total=%d\n", maxdelay, maxdelaypktno, tspktcnt);
	AP6211_DEBUG("%08X %08X %08X %08X \t%d %d %d   %d\n",
...
dhd_ioctl_htsf_get(dhd_info_t *dhd, int ifidx)
{
	...
	if ((ret = dhd_wl_ioctl(&dhd->pub, ifidx, &ioc, ioc.buf, ioc.len)) < 0) {
		if (ret == -EIO) {
			AP6211_ERR("%s: tsf is not supported by device\n",
				dhd_ifname(&dhd->pub, ifidx));
			return -EOPNOTSUPP;
		}
		return ret;
	...
	AP6211_DEBUG("TSF_h=%04X lo=%08X Calc:htsf=%08X, coef=%d.%d%d delta=%d ",
		tsf_buf.high, tsf_buf.low, s2, dhd->htsf.coef, dhd->htsf.coefdec1,
		dhd->htsf.coefdec2, s2-tsf_buf.low);
	AP6211_DEBUG("lasttsf=%08X lastcycle=%08X\n", dhd->htsf.last_tsf, dhd->htsf.last_cycle);
	return 0;
}
...
void htsf_update(dhd_info_t *dhd, void *data)
{
	...
	if (data == NULL)
		AP6211_DEBUG(" tsf update ata point er is null \n");

	memcpy(&prev_tsf, &cur_tsf, sizeof(tsf_t));
	memcpy(&cur_tsf, data, sizeof(tsf_t));

	if (cur_tsf.low == 0) {
		AP6211_DEBUG(" ---- 0 TSF, do not update, return\n");
		return;
	}
	...
	if (cur_tsf.low > prev_tsf.low)
		...
		AP6211_DEBUG(" ---- tsf low is smaller cur_tsf= %08X, prev_tsf=%08X, \n",
		 cur_tsf.low, prev_tsf.low);
		if (cur_tsf.high > prev_tsf.high) {
			tsf_delta = cur_tsf.low + (0xFFFFFFFF - prev_tsf.low);
			AP6211_DEBUG(" ---- Wrap around tsf coutner  adjusted TSF=%08X\n", tsf_delta);
		}
		else
