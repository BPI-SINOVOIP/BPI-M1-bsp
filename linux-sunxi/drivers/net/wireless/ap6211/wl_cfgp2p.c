	...
	if (wl_cfgp2p_is_pub_action(frame, frame_len)) {
		...
		switch (pact_frm->subtype) {
			case P2P_PAF_GON_REQ:
				AP6211_DEBUG("%s P2P Group Owner Negotiation Req Frame\n",
					(tx)? "TX": "RX");
				break;
			case P2P_PAF_GON_RSP:
				AP6211_DEBUG("%s P2P Group Owner Negotiation Rsp Frame\n",
					(tx)? "TX": "RX");
				break;
			case P2P_PAF_GON_CONF:
				AP6211_DEBUG("%s P2P Group Owner Negotiation Confirm Frame\n",
					(tx)? "TX": "RX");
				break;
			case P2P_PAF_INVITE_REQ:
				AP6211_DEBUG("%s P2P Invitation Request  Frame\n",
					(tx)? "TX": "RX");
				break;
			case P2P_PAF_INVITE_RSP:
				AP6211_DEBUG("%s P2P Invitation Response Frame\n",
					(tx)? "TX": "RX");
				break;
			case P2P_PAF_DEVDIS_REQ:
				AP6211_DEBUG("%s P2P Device Discoverability Request Frame\n",
					(tx)? "TX": "RX");
				break;
			case P2P_PAF_DEVDIS_RSP:
				AP6211_DEBUG("%s P2P Device Discoverability Response Frame\n",
					(tx)? "TX": "RX");
				break;
			case P2P_PAF_PROVDIS_REQ:
				AP6211_DEBUG("%s P2P Provision Discovery Request Frame\n",
					(tx)? "TX": "RX");
				break;
			case P2P_PAF_PROVDIS_RSP:
				AP6211_DEBUG("%s P2P Provision Discovery Response Frame\n",
					(tx)? "TX": "RX");
				break;
			default:
				AP6211_DEBUG("%s Unknown P2P Public Action Frame\n",
					(tx)? "TX": "RX");
		...
		switch (act_frm->subtype) {
			case P2P_AF_NOTICE_OF_ABSENCE:
				AP6211_DEBUG("%s P2P Notice of Absence Frame\n",
					(tx)? "TX": "RX");
				break;
			case P2P_AF_PRESENCE_REQ:
				AP6211_DEBUG("%s P2P Presence Request Frame\n",
					(tx)? "TX": "RX");
				break;
			case P2P_AF_PRESENCE_RSP:
				AP6211_DEBUG("%s P2P Presence Response Frame\n",
					(tx)? "TX": "RX");
				break;
			case P2P_AF_GO_DISC_REQ:
				AP6211_DEBUG("%s P2P Discoverability Request Frame\n",
					(tx)? "TX": "RX");
				break;
			default:
				AP6211_DEBUG("%s Unknown P2P Action Frame\n",
					(tx)? "TX": "RX");
		}
		...
		switch (sd_act_frm->action) {
			case P2PSD_ACTION_ID_GAS_IREQ:
				AP6211_DEBUG("%s P2P GAS Initial Request\n",
					(tx)? "TX" : "RX");
				break;
			case P2PSD_ACTION_ID_GAS_IRESP:
				AP6211_DEBUG("%s P2P GAS Initial Response\n",
					(tx)? "TX" : "RX");
				break;
			case P2PSD_ACTION_ID_GAS_CREQ:
				AP6211_DEBUG("%s P2P GAS Comback Request\n",
					(tx)? "TX" : "RX");
				break;
			case P2PSD_ACTION_ID_GAS_CRESP:
				AP6211_DEBUG("%s P2P GAS Comback Response\n",
					(tx)? "TX" : "RX");
				break;
			default:
				AP6211_DEBUG("%s Unknown P2P GAS Frame\n",
					(tx)? "TX" : "RX");
		}
...
wl_cfgp2p_init_priv(struct wl_priv *wl)
{
	if (!(wl->p2p = kzalloc(sizeof(struct p2p_info), GFP_KERNEL))) {
		AP6211_ERR("struct p2p_info allocation failed\n");
		return -ENOMEM;
	}
...
wl_cfgp2p_set_firm_p2p(struct wl_priv *wl)
{
	...
	if (ret && ret != BCME_UNSUPPORTED) {
		AP6211_ERR("failed to update device address ret %d\n", ret);
	}
	return ret;
}
            ...
            chanspec_t chspec)
{
	...
	AP6211_DEBUG("---wl p2p_ifadd "MACDBG" %s %u\n",
	...
	if (unlikely(err < 0))
		AP6211_DEBUG("'wl p2p_ifadd' error %d\n", err);
	else if (if_type == WL_P2P_IF_GO) {
		err = wldev_ioctl(ndev, WLC_SET_SCB_TIMEOUT, &scb_timeout, sizeof(u32), true);
		if (unlikely(err < 0))
			AP6211_DEBUG("'wl scb_timeout' error %d\n", err);
	}
	return err;
}
...
wl_cfgp2p_ifdisable(struct wl_priv *wl, struct ether_addr *mac)
{
	...
	AP6211_DEBUG("------primary idx %d : wl p2p_ifdis "MACDBG"\n",
		netdev->ifindex, MAC2STRDBG(mac->octet));
	ret = wldev_iovar_setbuf(netdev, "p2p_ifdis", mac, sizeof(*mac),
		wl->ioctl_buf, WLC_IOCTL_MAXLEN, &wl->ioctl_buf_sync);
	if (unlikely(ret < 0)) {
		AP6211_DEBUG("'wl p2p_ifdis' error %d\n", ret);
	}
	return ret;
}
...
wl_cfgp2p_ifdel(struct wl_priv *wl, struct ether_addr *mac)
{
	...
	AP6211_DEBUG("------primary idx %d : wl p2p_ifdel "MACDBG"\n",
	    netdev->ifindex, MAC2STRDBG(mac->octet));
	ret = wldev_iovar_setbuf(netdev, "p2p_ifdel", mac, sizeof(*mac),
		wl->ioctl_buf, WLC_IOCTL_MAXLEN, &wl->ioctl_buf_sync);
	if (unlikely(ret < 0)) {
		AP6211_DEBUG("'wl p2p_ifdel' error %d\n", ret);
	}
	return ret;
}
            ...
            chanspec_t chspec)
{
	...
	AP6211_DEBUG("---wl p2p_ifchange "MACDBG" %s %u"
		" chanspec 0x%04x\n", MAC2STRDBG(ifreq.addr.octet),
		(if_type == WL_P2P_IF_GO) ? "go" : "client",
		(chspec & WL_CHANSPEC_CHAN_MASK) >> WL_CHANSPEC_CHAN_SHIFT,
		ifreq.chspec);
	...
	if (unlikely(err < 0)) {
		AP6211_DEBUG("'wl p2p_ifupd' error %d\n", err);
	} else if (if_type == WL_P2P_IF_GO) {
...
wl_cfgp2p_set_p2p_mode(struct wl_priv *wl, u8 mode, u32 channel, u16 listen_ms, int bssidx)
{
	...
	if (unlikely(bssidx == WL_INVALID || bssidx >= P2PAPI_BSSCFG_MAX)) {
		AP6211_ERR(" %d index out of range\n", bssidx);
		return -1;
	}

	dev = wl_to_p2p_bss_ndev(wl, bssidx);
	if (unlikely(dev == NULL)) {
		AP6211_ERR("bssidx %d is not assigned\n", bssidx);
...
wl_cfgp2p_get_disc_idx(struct wl_priv *wl, s32 *index)
{
	...
	AP6211_DEBUG("p2p_dev bsscfg_idx=%d ret=%d\n", *index, ret);

	if (unlikely(ret <  0)) {
	    AP6211_ERR("'p2p_dev' error %d\n", ret);
		return ret;
	}
	return ret;
}
...
wl_cfgp2p_init_discovery(struct wl_priv *wl)
{
	...
	if (wl_to_p2p_bss_bssidx(wl, P2PAPI_BSSCFG_DEVICE) != 0) {
		AP6211_ERR("do nothing, already initialized\n");
		return ret;
	}

	ret = wl_cfgp2p_set_discovery(wl, 1);
	if (ret < 0) {
		AP6211_ERR("set discover error\n");
		return ret;
	}
	...
	if (unlikely(ret != 0)) {
		AP6211_ERR("unable to set WL_P2P_DISC_ST_SCAN\n");
...
wl_cfgp2p_deinit_discovery(struct wl_priv *wl)
{
	...
	if (wl_to_p2p_bss_bssidx(wl, P2PAPI_BSSCFG_DEVICE) == 0) {
		AP6211_ERR("do nothing, not initialized\n");
		return -1;
	}
	...
	const u8 *ie, u32 ie_len)
{
	...
	if (wl_get_p2p_status(wl, DISCOVERY_ON)) {
		AP6211_DEBUG(" DISCOVERY is already initialized, we have nothing to do\n");
		goto set_ie;
	}
	...
	if (unlikely(ret < 0)) {
		AP6211_ERR(" init discovery error %d\n", ret);
		goto exit;
	}
...
set_ie:
	if (ie_len) {
		...
		if (unlikely(ret < 0)) {
			AP6211_ERR("set probreq ie occurs error %d\n", ret);
			goto exit;
		}
	}
exit:
...
#define P2PAPI_SCAN_AF_SEARCH_DWELL_TIME_MS 100
	...
	if (memsize > sizeof(scanparambuf)) {
		AP6211_ERR(" scanpar buf too small (%u > %u)\n",
	...
	if (search_state == WL_P2P_DISC_ST_SEARCH) {
		...
		AP6211_ERR(" invalid search state %d\n", search_state);
		return -1;
	}
	...
	AP6211_DEBUG("nprobes # %d, active_time %d\n",
	...
	AP6211_DEBUG("SCAN CHANNELS : ");

	for (i = 0; i < num_chans; i++) {
		if (i == 0) AP6211_DEBUG("%d", channels[i]);
		else AP6211_DEBUG(",%d", channels[i]);
	}
	...
	while (ie) {
		...
		if (ie->id == DOT11_MNG_VS_ID) {
			...
			if (vndrie->len < (VNDR_IE_MIN_LEN + 1)) {
				AP6211_ERR("%s: invalid vndr ie. length is too small %d\n",
			...
			if (!bcmp(vndrie->oui, (u8*)WPA_OUI, WPA_OUI_LEN) &&
				...
				AP6211_DEBUG("Found WPA/WME oui. Do not add it\n");
				goto end;
			}
			...
			AP6211_DEBUG("\t ** OUI %02x %02x %02x, type 0x%02x \n",
...
#define IE_TYPE_LEN(type, bsstype) (wl_to_p2p_bss_saved_ie(wl, bsstype).p2p_ ## type ## _ie_len)
	...
	AP6211_DEBUG(" bssidx %d, pktflag : 0x%02X\n", bssidx, pktflag);
	if (wl->p2p != NULL) {
		switch (pktflag) {
	...
	if (vndr_ie_len > mgmt_ie_buf_len) {
		AP6211_ERR("extra IE size too big\n");
		ret = -ENOMEM;
	} else {
		...
		if (mgmt_ie_buf != NULL) {
			if (parsed_ie_buf_len && (parsed_ie_buf_len == *mgmt_ie_len) &&
				...
				AP6211_DEBUG("Previous mgmt IE is equals to current IE");
				goto exit;
			}
			...
			for (i = 0; i < old_vndr_ies.count; i++) {
				...
				AP6211_DEBUG("DELETED ID : %d, Len: %d , OUI:%02x:%02x:%02x\n",
		...
		*mgmt_ie_len = 0;
		...
		if (mgmt_ie_buf && parsed_ie_buf_len) {
			...
			for (i = 0; i < new_vndr_ies.count; i++) {
				...
				AP6211_DEBUG("ADDED ID : %d, Len: %d(%d), OUI:%02x:%02x:%02x\n",
					vndrie_info->vndrie.id, vndrie_info->vndrie.len,
				...
				if (remained_buf_len >= vndrie_info->ie_len) {
					...
					AP6211_ERR("no space in mgmt_ie_buf: pktflag = %d, "
		...
		if (total_ie_buf_len) {
			ret  = wldev_iovar_setbuf_bsscfg(ndev, "vndr_ie", g_mgmt_ie_buf,
				...
				bssidx, &wl->ioctl_buf_sync);
			if (ret)
				AP6211_ERR("vndr ie set error : %d\n", ret);
		}
	}
#undef IE_TYPE
#undef IE_TYPE_LEN
            ...
            s8 *oui, s32 ie_id, s8 *data, s32 datalen, const s8* add_del_cmd)
{
	...
	if ((pktflag & ~(VNDR_IE_BEACON_FLAG | VNDR_IE_PRBRSP_FLAG |
	            ...
	            VNDR_IE_PRBREQ_FLAG | VNDR_IE_ASSOCREQ_FLAG))) {
		AP6211_ERR("p2pwl_vndr_ie: Invalid packet flag 0x%x\n", pktflag);
		return -1;
	}
...
wl_cfgp2p_find_ndev(struct wl_priv *wl, s32 bssidx)
{
	...
	if (bssidx < 0) {
		AP6211_ERR(" bsscfg idx is invalid\n");
		goto exit;
	}
            ...
            const wl_event_msg_t *e, void *data)
{
	...
	if (wl_get_p2p_status(wl, LISTEN_EXPIRED) == 0) {
		...
		if (wl->afx_hdl->is_listen == TRUE &&
			...
			AP6211_DEBUG("Listen DONE for action frame\n");
			...
			AP6211_DEBUG("Listen DONE and wake up wait_next_af !!(%d)\n",
		...
		if (wl_get_drv_status_all(wl, REMAINING_ON_CHANNEL) ||
			...
			AP6211_DEBUG("Listen DONE for ramain on channel expired\n");
...
#define EXTRA_DELAY_TIME	100
	...
	AP6211_DEBUG(" Enter Listen Channel : %d, Duration : %d\n", channel, duration_ms);
	if (unlikely(wl_get_p2p_status(wl, DISCOVERY_ON) == 0)) {

		AP6211_ERR(" Discovery is not set, so we have noting to do\n");

		ret = BCME_NOTREADY;
		goto exit;
	}
	if (timer_pending(&wl->p2p->listen_timer)) {
		AP6211_DEBUG("previous LISTEN is not completed yet\n");
		goto exit;
	...
	if (wl_add_remove_eventmsg(netdev, WLC_E_P2P_PROBREQ_MSG, true) != BCME_OK) {
			AP6211_ERR(" failed to set WLC_E_P2P_PROPREQ_MSG\n");
	}
...
wl_cfgp2p_discover_enable_search(struct wl_priv *wl, u8 enable)
{
	...
	if (!wl_get_p2p_status(wl, DISCOVERY_ON)) {
		...
		AP6211_DEBUG(" do nothing, discovery is off\n");
		return ret;
	}
	if (wl_get_p2p_status(wl, SEARCH_ENABLED) == enable) {
		AP6211_DEBUG("already : %d\n", enable);
		return ret;
	}
            ...
            const wl_event_msg_t *e, void *data)
{
	...
	if (event_type == WLC_E_ACTION_FRAME_COMPLETE) {
		...
		AP6211_DEBUG(" WLC_E_ACTION_FRAME_COMPLETE is received : %d\n", status);
		if (status == WLC_E_STATUS_SUCCESS) {
			wl_set_p2p_status(wl, ACTION_TX_COMPLETED);
			AP6211_DEBUG("WLC_E_ACTION_FRAME_COMPLETE : ACK\n");
		}
		else {
			wl_set_p2p_status(wl, ACTION_TX_NOACK);
			AP6211_DEBUG("WLC_E_ACTION_FRAME_COMPLETE : NO ACK\n");
			wl_stop_wait_next_action_frame(wl, ndev);
		}
	} else {
		AP6211_DEBUG(" WLC_E_ACTION_FRAME_OFFCHAN_COMPLETE is received,"
					"status : %d\n", status);
...
#define MAX_WAIT_TIME 2000
	...
	if (ret < 0) {
		AP6211_ERR(" sending action frame is failed\n");
		goto exit;
	}
	...
	if (timeout > 0 && wl_get_p2p_status(wl, ACTION_TX_COMPLETED)) {
		AP6211_DEBUG("tx action frame operation is completed\n");
		ret = BCME_OK;
	} else {
		ret = BCME_ERROR;
		AP6211_DEBUG("tx action frame operation is failed\n");
	}
...
exit:
	AP6211_DEBUG(" via act frame iovar : status = %d\n", ret);
	...
	if ((ret = wl_cfg80211_apply_eventbuffer(wl_to_prmry_ndev(wl), wl, &buf)) < 0)
		AP6211_ERR("TX frame events revert back failed \n");
	...
	/* attribute id */
		...
		if (subelt_id == element_id) {
			if (subelt_id == P2P_SEID_INTINTADDR) {
				...
				AP6211_DEBUG("Intended P2P Interface Address ATTR FOUND\n");
			} else if (subelt_id == P2P_SEID_DEV_ID) {
				memcpy(subel, p2p_int_addr->octet, ETHER_ADDR_LEN);
				AP6211_DEBUG("Device ID ATTR FOUND\n");
			} else if (subelt_id == P2P_SEID_DEV_INFO) {
				memcpy(subel, p2p_int_addr->octet, ETHER_ADDR_LEN);
				AP6211_DEBUG("Device INFO ATTR FOUND\n");
			} else if (subelt_id == P2P_SEID_GROUP_ID) {
				memcpy(subel, p2p_int_addr->octet, ETHER_ADDR_LEN);
				AP6211_DEBUG("GROUP ID ATTR FOUND\n");
			}			return;
		} else {
			AP6211_DEBUG("OTHER id : %d\n", subelt_id);
		}
		subel += subelt_len;
	}
}
...
wl_cfgp2p_bss_isup(struct net_device *ndev, int bsscfg_idx)
{
	...
	*(int*)getbuf = -1;
	...
	if (result != 0) {
		AP6211_ERR("'wl bss -C %d' failed: %d\n", bsscfg_idx, result);
		AP6211_ERR("NOTE: this ioctl error is normal "
					"when the BSS has not been created yet.\n");
	} else {
		val = *(int*)getbuf;
		val = dtoh32(val);
		AP6211_DEBUG("---wl bss -C %d   ==> %d\n", bsscfg_idx, val);
		isup = (val ? TRUE : FALSE);
	}
	return isup;
}
...
wl_cfgp2p_bss(struct wl_priv *wl, struct net_device *ndev, s32 bsscfg_idx, s32 up)
{
	...
	AP6211_DEBUG("---wl bss -C %d %s\n", bsscfg_idx, up ? "up" : "down");
	ret = wldev_iovar_setbuf(ndev, "bss", &bss_setbuf, sizeof(bss_setbuf),
		wl->ioctl_buf, WLC_IOCTL_MAXLEN, &wl->ioctl_buf_sync);

	if (ret != 0) {
		AP6211_ERR("'bss %d' failed with %d\n", up, ret);
	}
...
wl_cfgp2p_supported(struct wl_priv *wl, struct net_device *ndev)
{
	...
	ret = wldev_iovar_getint(ndev, "p2p",
	               &p2p_supported);
	if (ret < 0) {
		AP6211_ERR("wl p2p error %d\n", ret);
		return 0;
	}
	if (p2p_supported == 1) {
		AP6211_DEBUG("p2p is supported\n");
	} else {
		AP6211_DEBUG("p2p is unsupported\n");
		p2p_supported = 0;
	}
...
wl_cfgp2p_set_p2p_noa(struct wl_priv *wl, struct net_device *ndev, char* buf, int len)
{
	...
	if (wl->p2p && wl->p2p->vif_created) {
		...
		AP6211_DEBUG("set_p2p_noa count %d start %d duration %d\n",
		...
		if (ret < 0) {
			AP6211_ERR("fw set p2p_noa failed %d\n", ret);
		}
	}
	else {
		AP6211_ERR("ERROR: set_noa in non-p2p mode\n");
	}
	return ret;
}
s32
wl_cfgp2p_get_p2p_noa(struct wl_priv *wl, struct net_device *ndev, char* buf, int buf_len)
{
	...
	if (wl->p2p && wl->p2p->vif_created) {
		if (wl->p2p->noa.desc[0].count || wl->p2p->ops.ops) {
			...
			if (buf_len <= len * 2) {
				AP6211_ERR("ERROR: buf_len %d in not enough for"
		...
		AP6211_ERR("ERROR: get_noa in non-p2p mode\n");
		return -1;
	}
	return len * 2;
}
...
wl_cfgp2p_set_p2p_ps(struct wl_priv *wl, struct net_device *ndev, char* buf, int len)
{
	...
	if (wl->p2p && wl->p2p->vif_created) {
		...
		AP6211_DEBUG(" Enter legacy_ps %d ps %d ctw %d\n", legacy_ps, ps, ctw);
		if (ctw != -1) {
			wl->p2p->ops.ctw = ctw;
			ret = 0;
		}
		if (ps != -1) {
			...
			ret = wldev_iovar_setbuf(wl_to_p2p_bss_ndev(wl, P2PAPI_BSSCFG_CONNECTION),
				...
				wl->ioctl_buf, WLC_IOCTL_MAXLEN, &wl->ioctl_buf_sync);
			if (ret < 0) {
				AP6211_ERR("fw set p2p_ops failed %d\n", ret);
			}
		}
		...
		if ((legacy_ps != -1) && ((legacy_ps == PM_MAX) || (legacy_ps == PM_OFF))) {
			...
			AP6211_ERR("ilegal setting\n");
	}
	else {
		AP6211_ERR("ERROR: set_p2p_ps in non-p2p mode\n");
		ret = -1;
	}
	return ret;
}
...
wl_cfgp2p_retreive_p2p_dev_addr(wl_bss_info_t *bi, u32 bi_length)
{
	...
	if (!(p2p_ie = wl_cfgp2p_find_p2pie(((u8 *) bi) + bi->ie_offset, bi->ie_length))) {
		AP6211_ERR("P2P IE not found");
		return NULL;
	}

	if (!(capability = wl_cfgp2p_retreive_p2pattrib(p2p_ie, P2P_SEID_P2P_INFO))) {
		AP6211_ERR("P2P Capability attribute not found");
		return NULL;
	}
	...
	if (!ptr)
		AP6211_ERR(" Both DEVICE_ID & DEVICE_INFO attribute not present in P2P IE ");
...
wl_cfgp2p_register_ndev(struct wl_priv *wl)
{
	...
	if (wl->p2p_net) {
		AP6211_ERR("p2p_net defined already.\n");
		return -EINVAL;
	}
	...
	if (ret) {
		AP6211_ERR(" register_netdevice failed (%d)\n", ret);
		free_netdev(net);
		kfree(wdev);
		return -ENODEV;
	}
	...
	AP6211_DEBUG("%s: P2P Interface Registered\n", net->name);
...
static int wl_cfgp2p_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	if (skb)
	{
		AP6211_DEBUG("(%s) is not used for data operations.Droping the packet.\n",
			ndev->name);
...
static int wl_cfgp2p_do_ioctl(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	if (cmd == SIOCDEVPRIVATE+1) {
		...
		AP6211_ERR("%s: IOCTL req 0x%x on p2p0 I/F. Ignoring. \n",
		__FUNCTION__, cmd);
		return -1;
	}
