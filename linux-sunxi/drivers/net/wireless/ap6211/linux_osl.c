...
osl_attach(void *pdev, uint bustype, bool pkttag)
{
	...
	if (!bcm_static_buf) {
		if (!(bcm_static_buf = (bcm_static_buf_t *)dhd_os_prealloc(osh, 3, STATIC_BUF_SIZE+
			...
			AP6211_DEBUG("can not alloc static buf!\n");
		}
		else
			AP6211_DEBUG("alloc static buf at %x!\n", (unsigned int)bcm_static_buf);
...
osl_ctfpool_add(osl_t *osh)
{
	...
	if (skb == NULL) {
		AP6211_DEBUG("%s: skb alloc of len %d failed\n", __FUNCTION__,
...
osl_pktget_static(osl_t *osh, uint len)
{
	...
	if (len > DHD_SKB_MAX_BUFSIZE) {
		AP6211_DEBUG("osl_pktget_static: Do we really need this big skb??"
			" len=%d\n", len);
	...
	up(&bcm_static_skb->osl_pkt_sem);
	AP6211_DEBUG("osl_pktget_static: all static pkt in use!\n");
	return osl_pktget(osh, len);
}
	...
	if (bcm_static_buf)
	{
		...
		if ((size >= PAGE_SIZE)&&(size <= STATIC_BUF_SIZE))
		{
			...
			if (i == STATIC_BUF_MAX_NUM)
			{
				...
				AP6211_DEBUG("all static buff in use!\n");
				goto original;
			}
