...
do { \
	...
	if (retryvar) { \
		bus->regfails += (retryvar-1); \
		if (retryvar > retry_limit) \
			AP6211_ERR("%s: FAILED REGISTER WRITE, LINE %d\n", \
			           __FUNCTION__, __LINE__); \
	} \
} while (0)
...
dhd_dongle_setmemsize(struct dhd_bus *bus, int mem_size)
{
	...
	AP6211_DEBUG("user: Restrict the dongle ram size to %d, min accepted %d\n",
...
dhdsdio_clk_kso_init(dhd_bus_t *bus)
{
	...
	if (!(val & SBSDIO_FUNC1_SLEEPCSR_KSO_MASK)) {
		...
		if (err)
			AP6211_ERR("%s: SBSDIO_FUNC1_SLEEPCSR err: 0x%x\n", __FUNCTION__, err);
	}
...
dhdsdio_clk_kso_enab(dhd_bus_t *bus, bool on)
{
	...
	if (try_cnt > MAX_KSO_ATTEMPTS)  {
		AP6211_ERR("%s> op:%s, ERROR: try_cnt:%d, rd_val:%x, ERR:%x \n",
			__FUNCTION__, (on ? "KSO_SET" : "KSO_CLR"), try_cnt, rd_val, err);
	}
	return err;
}
...
dhdsdio_clk_kso_iovar(dhd_bus_t *bus, bool on)
{
	...
	if (on == FALSE) {
		...
		AP6211_DEBUG("%s: KSO disable clk: 0x%x\n", __FUNCTION__,
			bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1,
			SBSDIO_FUNC1_CHIPCLKCSR, &err));
		dhdsdio_clk_kso_enab(bus, FALSE);
	} else {
		AP6211_DEBUG("%s: KSO enable\n", __FUNCTION__);

		/* Make sure we have SD bus access */
		if (bus->clkstate == CLK_NONE) {
			AP6211_DEBUG("%s: Request SD clk\n", __FUNCTION__);
...
dhdsdio_sleepcsr_get(dhd_bus_t *bus)
{
	...
	if (err)
		AP6211_DEBUG("Failed to read SLEEPCSR: %d\n", err);
...
dhdsdio_devcap_set(dhd_bus_t *bus, uint8 cap)
{
	...
	if (err)
		AP6211_ERR("%s: devcap set err: 0x%x\n", __FUNCTION__, err);
...
dhdsdio_clk_devsleep_iovar(dhd_bus_t *bus, bool on)
{
	...
	if (on == TRUE) {
		...
		if (!SLPAUTO_ENAB(bus))
			...
			if ((val & SBSDIO_CSR_MASK) == 0) {
				AP6211_DEBUG("%s: No clock before enter sleep:0x%x\n",
					__FUNCTION__, val);

				/* Reset clock request */
				bcmsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR,
					SBSDIO_ALP_AVAIL_REQ, &err);
				AP6211_DEBUG("%s: clock before sleep:0x%x\n", __FUNCTION__,
					bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1,
					SBSDIO_FUNC1_CHIPCLKCSR, &err));
			}
		}

		AP6211_DEBUG("%s: clk before sleep: 0x%x\n", __FUNCTION__,
			bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1,
			SBSDIO_FUNC1_CHIPCLKCSR, &err));
#ifdef USE_CMD14
		...
		if ((bus->sih->chip == BCM4334_CHIP_ID) && (bus->sih->chiprev == 2)) {
			...
			if (bcmsdh_gpioin(bus->sdh, GPIO_DEV_SRSTATE) == FALSE) {
				AP6211_ERR("ERROR: GPIO_DEV_SRSTATE still low!\n");
			}
		}
#ifdef USE_CMD14
		...
		if (SLPAUTO_ENAB(bus) && (err != 0)) {
			OSL_DELAY(10000);
			AP6211_DEBUG("%s: Resync device sleep\n", __FUNCTION__);
			...
			if (err) {
				OSL_DELAY(10000);
				AP6211_ERR("%s: CMD14 exit failed again!\n", __FUNCTION__);
				...
				err = bcmsdh_sleep(bus->sdh, FALSE);
				if (err) {
					AP6211_ERR("%s: CMD14 exit failed twice!\n", __FUNCTION__);
					AP6211_ERR("%s: FATAL: Device non-response!\n",
						__FUNCTION__);
					err = 0;
				}
			}
		...
		if (err != 0) {
			AP6211_ERR("ERROR: kso set failed retry: %d\n", retry);
		...
		if (err == 0) {
			...
			AP6211_DEBUG("%s: ExitSleep sleepcsr: 0x%x\n", __FUNCTION__, csr);

			if (!(csr & SBSDIO_FUNC1_SLEEPCSR_DEVON_MASK)) {
				AP6211_ERR("%s:ERROR: ExitSleep device NOT Ready! 0x%x\n",
					__FUNCTION__, csr);
				err = BCME_NODEVICE;
			}
	...
	if (err == 0)
		...
		AP6211_ERR("%s: Sleep request failed: on:%d err:%d\n", __FUNCTION__, on, err);
		if (!on && retry > 2)
			bus->kso = TRUE;
	}
	...
	if (on) {
		...
		if (err) {
			...
			if (ht_avail_error < HT_AVAIL_ERROR_MAX) {
				AP6211_ERR("%s: HT Avail request error: %d\n", __FUNCTION__, err);
			}
		...
		if (err) {
			AP6211_ERR("%s: HT Avail read error: %d\n", __FUNCTION__, err);
			return BCME_ERROR;
		}
		...
		if (!SBSDIO_CLKAV(clkctl, bus->alp_only) && pendok) {
			...
			if (err) {
				AP6211_ERR("%s: Devctl access error setting CA: %d\n",
				           __FUNCTION__, err);
				return BCME_ERROR;
			}
			...
			AP6211_DEBUG("CLKCTL: set PENDING\n");
		...
		if (!SBSDIO_CLKAV(clkctl, bus->alp_only)) {
			AP6211_ERR("%s: HT Avail timeout (%d): clkctl 0x%02x\n",
		...
		AP6211_DEBUG("CLKCTL: turned ON\n");

#if defined(DHD_DEBUG)
		if (bus->alp_only == TRUE) {
#if !defined(BCMLXSDMMC)
			if (!SBSDIO_ALPONLY(clkctl)) {
				AP6211_ERR("%s: HT Clock, when ALP Only\n", __FUNCTION__);
			}
#endif /* !defined(BCMLXSDMMC) */
		} else {
			if (SBSDIO_ALPONLY(clkctl)) {
				AP6211_ERR("%s: HT Clock should be on.\n", __FUNCTION__);
			}
		}
#endif /* defined (DHD_DEBUG) */
		...
		if (!SR_ENAB(bus)) {
			bcmsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, clkreq, &err);
			AP6211_DEBUG("CLKCTL: turned OFF\n");
			if (err) {
				AP6211_ERR("%s: Failed access turning clock off: %d\n",
				           __FUNCTION__, err);
				return BCME_ERROR;
			}
		}
...
dhdsdio_sdclk(dhd_bus_t *bus, bool on)
{
	...
	if (on) {
		if (bus->idleclock == DHD_IDLE_STOP) {
			...
			if (err) {
				AP6211_ERR("%s: error enabling sd_clock: %d\n",
				           __FUNCTION__, err);
				return BCME_ERROR;
			}
			...
			err = bcmsdh_iovar_op(bus->sdh, "sd_mode", NULL, 0,
			                      &iovalue, sizeof(iovalue), TRUE);
			if (err) {
				AP6211_ERR("%s: error changing sd_mode: %d\n",
				           __FUNCTION__, err);
				return BCME_ERROR;
			}
			...
			err = bcmsdh_iovar_op(bus->sdh, "sd_divisor", NULL, 0,
			                      &iovalue, sizeof(iovalue), TRUE);
			if (err) {
				AP6211_ERR("%s: error restoring sd_divisor: %d\n",
				           __FUNCTION__, err);
				return BCME_ERROR;
			}
		}
		...
		if ((bus->sd_divisor == -1) || (bus->sd_mode == -1)) {
			AP6211_DEBUG("%s: can't idle clock, divisor %d mode %d\n",
		...
		if (bus->idleclock == DHD_IDLE_STOP) {
			if (sd1idle) {
				...
				err = bcmsdh_iovar_op(bus->sdh, "sd_mode", NULL, 0,
				                      &iovalue, sizeof(iovalue), TRUE);
				if (err) {
					AP6211_ERR("%s: error changing sd_clock: %d\n",
					           __FUNCTION__, err);
					return BCME_ERROR;
				}
			}
			...
			err = bcmsdh_iovar_op(bus->sdh, "sd_clock", NULL, 0,
			                      &iovalue, sizeof(iovalue), TRUE);
			if (err) {
				AP6211_ERR("%s: error disabling sd_clock: %d\n",
				           __FUNCTION__, err);
				return BCME_ERROR;
			}
		} else if (bus->idleclock != DHD_IDLE_ACTIVE) {
			...
			err = bcmsdh_iovar_op(bus->sdh, "sd_divisor", NULL, 0,
			                      &iovalue, sizeof(iovalue), TRUE);
			if (err) {
				AP6211_ERR("%s: error changing sd_divisor: %d\n",
				           __FUNCTION__, err);
				return BCME_ERROR;
			}
		}
...
dhdsdio_clkctl(dhd_bus_t *bus, uint target, bool pendok)
{
	...
	case CLK_SDONLY:
		...
		if (bus->clkstate == CLK_NONE)
			...
			AP6211_DEBUG("dhdsdio_clkctl: request for %d -> %d\n",
			           bus->clkstate, target);
		if (ret == BCME_OK) {
			dhd_os_wd_timer(bus->dhd, dhd_watchdog_ms);
		}
...
#ifdef DHD_DEBUG
	AP6211_DEBUG("dhdsdio_clkctl: %d -> %d\n", oldstate, bus->clkstate);
#endif /* DHD_DEBUG */
...
dhdsdio_bussleep(dhd_bus_t *bus, bool sleep)
{
	...
	AP6211_DEBUG("dhdsdio_bussleep: request %s (currently %s)\n",
	          (sleep ? "SLEEP" : "WAKE"),
	...
	if (sleep) {
		...
		if (!SLPAUTO_ENAB(bus)) {
			...
			if (retries > retry_limit)
				AP6211_ERR("CANNOT SIGNAL CHIP, WILL NOT WAKE UP!!\n");
		...
		if (!SLPAUTO_ENAB(bus)) {
			...
			if (retries > retry_limit)
				AP6211_ERR("CANNOT SIGNAL CHIP TO CLEAR OOB!!\n");
...
dhdsdio_txpkt(dhd_bus_t *bus, void *pkt, uint chan, bool free_pkt, bool queue_only)
{
	...
	if (!((uintptr)frame & 1) && (pad1 = ((uintptr)frame % DHD_SDALIGN))) {
		if (PKTHEADROOM(osh, pkt) < pad1) {
			AP6211_ERR("%s: insufficient headroom %d for %d pad1\n",
			          __FUNCTION__, (int)PKTHEADROOM(osh, pkt), pad1);
			bus->dhd->tx_realloc++;
			new = PKTGET(osh, (PKTLEN(osh, pkt) + DHD_SDALIGN), TRUE);
			if (!new) {
				AP6211_ERR("%s: couldn't allocate new %d-byte packet\n",
	...
	*(((uint16*)frame) + 1) = htol16(~len);
	...
	if (forcealign && (len & (ALIGNMENT - 1))) {
		...
		if (PKTTAILROOM(osh, pkt))
			...
			AP6211_DEBUG("%s: sending unrounded %d-byte packet\n", __FUNCTION__, len);
#endif
	}
	}
	...
	do {
		...
		if (ret == BCME_NODEVICE) {
			AP6211_ERR("%s: Device asleep already\n", __FUNCTION__);
		} else if (ret < 0) {
			/* On failure, abort the command and terminate the frame */
			AP6211_ERR("%s: sdio error %d, abort command and terminate frame.\n",
			          __FUNCTION__, ret);
			bus->tx_sderrs++;
...
dhd_bus_txdata(struct dhd_bus *bus, void *pkt, bool wlfc_locked)
{
	...
	if (protocol == ETHER_TYPE_802_1X) {
		AP6211_DEBUG("ETHER_TYPE_802_1X: ver %d, type %d, replay %d\n",
	...
	if (dhd_deferred_tx || bus->fcstate || pktq_len(&bus->txq) || bus->dpc_sched ||
	    ...
	    (bus->clkstate != CLK_AVAIL)) {
		AP6211_DEBUG("%s: deferring pktq len %d\n", __FUNCTION__,
			pktq_len(&bus->txq));
		bus->fcqueued++;
		...
		if (bus->glom_enable) {
			...
			for (i = 0; i < glom_cnt; i++) {
				...
				if ((pkt = pktq_mdeq(&bus->txq, tx_prec_map, &prec_out)) == NULL) {
					...
					AP6211_DEBUG("No pkts in the queue for glomming\n");
					dhd_os_sdunlock_txq(bus->dhd);
					break;
				}
				dhd_os_sdunlock_txq(bus->dhd);
...
dhd_bus_txctl(struct dhd_bus *bus, uchar *msg, uint msglen)
{
	...
	*(((uint16*)frame) + 1) = htol16(~msglen);
	...
	if (!TXCTLOK(bus)) {
		AP6211_DEBUG("%s: No bus credit bus->tx_max %d, bus->tx_seq %d\n",
		...
		if (bus->ctrl_frame_stat == FALSE) {
			AP6211_DEBUG("%s: ctrl_frame_stat == FALSE\n", __FUNCTION__);
			ret = 0;
		} else {
			bus->dhd->txcnt_timeout++;
			if (!bus->dhd->hang_was_sent) {
				AP6211_DEBUG("%s: ctrl_frame_stat == TRUE txcnt_timeout=%d\n",
	...
	if (ret == -1) {
		...
		do {
			...
			if (ret == BCME_NODEVICE) {
				AP6211_DEBUG("%s: Device asleep already\n", __FUNCTION__);
			} else if (ret < 0) {
			/* On failure, abort the command and terminate the frame */
				AP6211_DEBUG("%s: sdio error %d, abort command and terminate frame.\n",
				          __FUNCTION__, ret);
				bus->tx_sderrs++;
...
dhd_bus_rxctl(struct dhd_bus *bus, uchar *msg, uint msglen)
{
	...
	if (rxlen) {
		AP6211_DEBUG("%s: resumed on rxctl frame, got %d expected %d\n",
		         __FUNCTION__, rxlen, msglen);
	} else if (timeleft == 0) {
#ifdef DHD_DEBUG
		uint32 status, retry = 0;
		R_SDREG(status, &bus->regs->intstatus, retry);
		AP6211_DEBUG("%s: resumed on timeout, INT status=0x%08X\n",
			__FUNCTION__, status);
#else
		AP6211_DEBUG("%s: resumed on timeout\n", __FUNCTION__);
#endif /* DHD_DEBUG */
#ifdef DHD_DEBUG
		dhd_os_sdlock(bus->dhd);
		...
		/* signal pending */
		AP6211_DEBUG("%s: signal pending\n", __FUNCTION__);
		return -EINTR;
	} else {
		AP6211_DEBUG("%s: resumed for unknown reason?\n", __FUNCTION__);
#ifdef DHD_DEBUG
		dhd_os_sdlock(bus->dhd);
		dhdsdio_checkdied(bus, NULL, 0);
		dhd_os_sdunlock(bus->dhd);
	...
	if (timeleft == 0) {
		...
		AP6211_DEBUG("%s: rxcnt_timeout=%d\n", __FUNCTION__, bus->dhd->rxcnt_timeout);
	}
	else
...
dhdsdio_membytes(dhd_bus_t *bus, bool write, uint32 address, uint8 *data, uint size)
{
	...
	if ((bcmerror = dhdsdio_set_siaddr_window(bus, address))) {
		AP6211_ERR("%s: window change failed\n", __FUNCTION__);
		goto xfer_done;
	}

	/* Do the transfer(s) */
	while (size) {
		AP6211_DEBUG("%s: %s %d bytes at offset 0x%08x in window 0x%08x\n",
		          __FUNCTION__, (write ? "write" : "read"), dsize, sdaddr,
		          (address & SBSDIO_SBWINDOW_MASK));
		if ((bcmerror = bcmsdh_rwdata(bus->sdh, write, sdaddr, data, dsize))) {
			AP6211_ERR("%s: membytes transfer failed\n", __FUNCTION__);
			break;
		}
...
xfer_done:
	...
	if (dhdsdio_set_siaddr_window(bus, bcmsdh_cur_sbwad(bus->sdh))) {
		AP6211_ERR("%s: FAILED to set window back to 0x%x\n", __FUNCTION__,
...
dhdsdio_readshared(dhd_bus_t *bus, sdpcm_shared_t *sh)
{
	...
	do {
		...
		AP6211_DEBUG("sdpcm_shared address 0x%08X\n", addr);
		...
		if (addr == 0 || ((~addr >> 16) & 0xffff) == (addr & 0xffff)) {
			if ((bus->srmemsize > 0) && (i++ == 0)) {
				...
				AP6211_ERR("%s: address (0x%08x) of sdpcm_shared invalid\n",
					__FUNCTION__, addr);
				return BCME_ERROR;
			}
		} else
	...
	if ((sh->flags & SDPCM_SHARED_VERSION_MASK) != SDPCM_SHARED_VERSION) {
		AP6211_ERR("%s: sdpcm_shared version %d in dhd "
                ...
                void *params, int plen, void *arg, int len, int val_size)
{
	...
	AP6211_DEBUG("%s: Enter, action %d name %s params %p plen %d arg %p len %d val_size %d\n",
	...
	if ((vi->varid == IOV_KSO) && (IOV_ISSET(actionid))) {
	...
	} else if ((vi->varid == IOV_DEVSLEEP) && (IOV_ISSET(actionid))) {
		{
			...
			if (!SLPAUTO_ENAB(bus) && (bool_val == FALSE) && (bus->ipend)) {
				AP6211_DEBUG("INT pending in devsleep 1, dpc_sched: %d\n",
					bus->dpc_sched);
				if (!bus->dpc_sched) {
					bus->dpc_sched = TRUE;
					dhd_sched_dpc(bus->dhd);
	...
	case IOV_SVAL(IOV_INTR):
		...
		if (bus->dhd->up) {
			if (bus->intr) {
				AP6211_DEBUG("%s: enable SDIO device interrupts\n", __FUNCTION__);
				bcmsdh_intr_enable(bus->sdh);
			} else {
				AP6211_DEBUG("%s: disable SDIO interrupts\n", __FUNCTION__);
				bcmsdh_intr_disable(bus->sdh);
			}
		}
		break;
	...
	case IOV_GVAL(IOV_MEMBYTES):
	{
		...
		if (dsize < size) {
			AP6211_ERR("%s: error on %s membytes, addr 0x%08x size %d dsize %d\n",
			           __FUNCTION__, (set ? "set" : "get"), address, size, dsize);
			bcmerror = BCME_BADARG;
			break;
		}

		AP6211_DEBUG("%s: Request to %s %d bytes at address 0x%08x\n", __FUNCTION__,
		    ...
		    ((address > bus->orig_ramsize) || (address + size > bus->orig_ramsize)))
		{
			...
			si_socdevram(bus->sih, FALSE, &enable, &protect, &remap);
			if (!enable || protect) {
				AP6211_ERR("%s: ramsize 0x%08x doesn't have %d bytes at 0x%08x\n",
					__FUNCTION__, bus->orig_ramsize, size, address);
				AP6211_DEBUG("%s: socram enable %d, protect %d\n",
			...
			if (!REMAP_ENAB(bus) && (address >= SOCDEVRAM_ARM_ADDR)) {
				...
				if ((address < SOCDEVRAM_ARM_ADDR) ||
					...
					AP6211_ERR("%s: bad address 0x%08x, size 0x%08x\n",
						__FUNCTION__, address, size);
					AP6211_DEBUG("%s: socram range 0x%08x,size 0x%08x\n",
				...
				address += SOCDEVRAM_BP_ADDR;
				AP6211_DEBUG("%s: Request to %s %d bytes @ Mapped address 0x%08x\n",
				 ...
				 */
				AP6211_ERR("%s: Need to disable remap for address 0x%08x\n",
	...
	case IOV_SVAL(IOV_WATERMARK):
		...
		AP6211_DEBUG("Setting watermark as 0x%x.\n", watermark);
	...
	case IOV_SVAL(IOV_MESBUSYCTRL):
		...
		AP6211_DEBUG("Setting mesbusyctrl as 0x%x.\n", mesbusyctrl);
	...
	case IOV_SVAL(IOV_DEVRESET):
		AP6211_DEBUG("%s: Called set IOV_DEVRESET=%d dongle_reset=%d busstate=%d\n",
	...
	case IOV_GVAL(IOV_FWPATH):
	{
		...
		AP6211_DEBUG("[softap] get fwpath, l=%d\n", len);
	...
	case IOV_SVAL(IOV_FWPATH):
		AP6211_DEBUG("[softap] set fwpath, idx=%d\n", int_val);
		...
		AP6211_DEBUG("[softap] new fw path: %s\n", (bus->fw_path[0] ? bus->fw_path : "NULL"));
		break;

#endif /* SOFTAP */
	case IOV_GVAL(IOV_DEVRESET):
		AP6211_DEBUG("%s: Called get IOV_DEVRESET\n", __FUNCTION__);
...
dhdsdio_write_vars(dhd_bus_t *bus)
{
	...
	if (bus->vars) {
		if ((bus->sih->buscoretype == SDIOD_CORE_ID) && (bus->sdpcmrev == 7)) {
			if (((varaddr & 0x3C) == 0x3C) && (varsize > 4)) {
				AP6211_DEBUG("PR85623WAR in place\n");
				varsize += 4;
				varaddr -= 4;
			}
		}
...
#ifdef DHD_DEBUG
		/* Verify NVRAM bytes */
		AP6211_DEBUG("Compare NVRAM dl & ul; varsize=%d\n", varsize);
		...
		if (bcmerror) {
				AP6211_ERR("%s: error %d on reading %d nvram bytes at 0x%08x\n",
					__FUNCTION__, bcmerror, varsize, varaddr);
		}
		/* Compare the org NVRAM with the one read from RAM */
		if (memcmp(vbuffer, nvram_ularray, varsize)) {
			AP6211_ERR("%s: Downloaded NVRAM image is corrupted.\n", __FUNCTION__);
		} else
			AP6211_DEBUG("%s: Download, Upload and compare of NVRAM succeeded.\n",
			__FUNCTION__);
	...
	AP6211_DEBUG("Physical memory size: %d, usable memory size: %d\n",
		phys_size, bus->ramsize);
	AP6211_DEBUG("Vars are at %d, orig varsize is %d\n",
		varaddr, varsize);
	...
	AP6211_DEBUG("New varsize is %d, length token=0x%08x\n", varsize, varsizew);
...
dhdsdio_download_state(dhd_bus_t *bus, bool enter)
{
	...
	if (enter) {
		...
		if (!(si_setcore(bus->sih, ARM7S_CORE_ID, 0)) &&
			...
			AP6211_ERR("%s: Failed to find ARM core!\n", __FUNCTION__);
			bcmerror = BCME_ERROR;
			goto fail;
		}
		}
		...
		if (!(si_setcore(bus->sih, SOCRAM_CORE_ID, 0))) {
			AP6211_ERR("%s: Failed to find SOCRAM core!\n", __FUNCTION__);
			bcmerror = BCME_ERROR;
			goto fail;
		}
		...
		if (bcmsdh_regfail(bus->sdh)) {
			AP6211_ERR("%s: Failure trying reset SOCRAM core?\n", __FUNCTION__);
			bcmerror = BCME_SDIO_ERROR;
			goto fail;
		}
		...
		if (!si_iscoreup(bus->sih)) {
			AP6211_ERR("%s: SOCRAM core is down after reset?\n", __FUNCTION__);
			bcmerror = BCME_ERROR;
			goto fail;
		}

		if ((bcmerror = dhdsdio_write_vars(bus))) {
			AP6211_ERR("%s: could not write vars to RAM\n", __FUNCTION__);
			goto fail;
		}
		...
		if (!si_setcore(bus->sih, PCMCIA_CORE_ID, 0) &&
			...
			AP6211_ERR("%s: Can't change back to SDIO core?\n", __FUNCTION__);
			bcmerror = BCME_ERROR;
			goto fail;
		}
		...
		if (!(si_setcore(bus->sih, ARM7S_CORE_ID, 0)) &&
			...
			if (!(si_setcore(bus->sih, ARMCR4_CORE_ID, 0))) {
				AP6211_ERR("%s: Failed to find ARM CR4 core!\n", __FUNCTION__);
				bcmerror = BCME_ERROR;
				goto fail;
			}
		...
		si_core_reset(bus->sih, 0, 0);
		if (bcmsdh_regfail(bus->sdh)) {
			AP6211_ERR("%s: Failure trying to reset ARM core?\n", __FUNCTION__);
			bcmerror = BCME_SDIO_ERROR;
			goto fail;
		}
                 ...
                 void *params, int plen, void *arg, int len, bool set)
{
	...
	if ((vi = bcm_iovar_lookup(dhdsdio_iovars, name)) == NULL) {
		...
		if (set && strcmp(name, "sd_divisor") == 0) {
			if (bcmsdh_iovar_op(bus->sdh, "sd_divisor", NULL, 0,
				...
				bus->sd_divisor = -1;
				AP6211_ERR("%s: fail on %s get\n", __FUNCTION__, name);
			} else {
				AP6211_DEBUG("%s: noted %s update, value now %d\n",
	...
	AP6211_DEBUG("%s: %s %s, len %d plen %d\n", __FUNCTION__,
...
dhd_bus_stop(struct dhd_bus *bus, bool enforce_mutex)
{
	...
	if ((bus->dhd->busstate == DHD_BUS_DOWN) || bus->dhd->hang_was_sent) {
		...
		if (KSO_ENAB(bus)) {
			...
			if (err) {
				AP6211_ERR("%s: Failed to force clock for F2: err %d\n", __FUNCTION__, err);
			}
...
dhd_bus_init(dhd_pub_t *dhdp, bool enforce_mutex)
{
	...
	dhdsdio_clkctl(bus, CLK_AVAIL, FALSE);
	if (bus->clkstate != CLK_AVAIL) {
		AP6211_ERR("%s: clock state is wrong. state = %d\n", __FUNCTION__, bus->clkstate);
		ret = -1;
		goto exit;
	}
	...
	AP6211_DEBUG("%s: enable 0x%02x, ready 0x%02x (waited %uus)\n",
...
dhdsdio_rxfail(dhd_bus_t *bus, bool abort, bool rtx)
{
	...
	AP6211_ERR("%s: %sterminate frame%s\n", __FUNCTION__,
	           (abort ? "abort command, " : ""), (rtx ? ", send NAK" : ""));

	if (!KSO_ENAB(bus)) {
		AP6211_ERR("%s: Device asleep\n", __FUNCTION__);
		return;
	}
	...
	for (lastrbc = retries = 0xffff; retries > 0; retries--) {
		...
		if ((hi > (lastrbc >> 8)) && (lo > (lastrbc & 0x00ff))) {
			AP6211_DEBUG("%s: count growing: last 0x%04x now 0x%04x\n",
	...
	if (!retries) {
		AP6211_ERR("%s: count never zeroed: last 0x%04x\n", __FUNCTION__, lastrbc);
	} else {
		AP6211_DEBUG("%s: flush took %d iterations\n", __FUNCTION__, (0xffff - retries));
	}
...
dhdsdio_read_control(dhd_bus_t *bus, uint8 *hdr, uint len, uint doff)
{
	...
	if ((rdlen + firstread) > bus->dhd->maxctl) {
		AP6211_ERR("%s: %d-byte control read exceeds %d-byte buffer\n",
	...
	if ((len - doff) > bus->dhd->maxctl) {
		AP6211_ERR("%s: %d-byte ctl frame (%d-byte ctl data) exceeds %d-byte limit\n",
	...
	if (sdret < 0) {
		AP6211_ERR("%s: read %d control bytes failed: %d\n", __FUNCTION__, rdlen, sdret);
...
dhdsdio_rxglom(dhd_bus_t *bus, uint8 rxseq)
{
	...
	AP6211_DEBUG("dhdsdio_rxglom: start: glomd %p glom %p\n", bus->glomd, bus->glom);
	...
	if (bus->glomd) {
		...
		if (!dlen || (dlen & 1)) {
			AP6211_ERR("%s: bad glomd len (%d), ignore descriptor\n",
			           __FUNCTION__, dlen);
			dlen = 0;
		}
		...
		for (totlen = num = 0; dlen; num++) {
			...
			if ((sublen < SDPCM_HDRLEN_RX) ||
				...
				AP6211_ERR("%s: descriptor len %d bad: %d\n",
				           __FUNCTION__, num, sublen);
				pnext = NULL;
				break;
			}
			if (sublen % DHD_SDALIGN) {
				AP6211_ERR("%s: sublen %d not a multiple of %d\n",
			...
			if ((pnext = PKTGET(osh, sublen + DHD_SDALIGN, FALSE)) == NULL) {
				AP6211_ERR("%s: PKTGET failed, num %d len %d\n",
				           __FUNCTION__, num, sublen);
				break;
			}
		...
		if (pnext) {
			AP6211_DEBUG("%s: allocated %d-byte packet chain for %d subframes\n",
			          __FUNCTION__, totlen, num);
			if (DHD_GLOM_ON() && bus->nextlen) {
				if (totlen != bus->nextlen) {
					AP6211_DEBUG("%s: glomdesc mismatch: nextlen %d glomdesc %d "
	...
	if (bus->glom) {
		if (DHD_GLOM_ON()) {
			AP6211_DEBUG("%s: attempt superframe read, packet chain:\n", __FUNCTION__);
			for (pnext = bus->glom; pnext; pnext = PKTNEXT(osh, pnext)) {
				AP6211_DEBUG("    %p: %p len 0x%04x (%d)\n",
		...
		if (usechain) {
			...
			if (sublen != dlen) {
				AP6211_ERR("%s: FAILED TO COPY, dlen %d sublen %d\n",
				           __FUNCTION__, dlen, sublen);
				errcode = -1;
			}
			pnext = NULL;
		} else {
			AP6211_ERR("COULDN'T ALLOC %d-BYTE GLOM, FORCE FAILURE\n", dlen);
			errcode = -1;
		}
		bus->f2rxdata++;
		...
		if (errcode < 0) {
			AP6211_ERR("%s: glom read of %d bytes failed: %d\n",
		...
		if ((bus->nextlen << 4) > MAX_RX_DATASZ) {
			AP6211_DEBUG("%s: got frame w/nextlen too large (%d) seq %d\n",
		...
		if ((uint16)~(sublen^check)) {
			AP6211_ERR("%s (superframe): HW hdr error: len/check 0x%04x/0x%04x\n",
			           __FUNCTION__, sublen, check);
			errcode = -1;
		} else if (ROUNDUP(sublen, bus->blocksize) != dlen) {
			AP6211_ERR("%s (superframe): len 0x%04x, rounded 0x%04x, expect 0x%04x\n",
			           __FUNCTION__, sublen, ROUNDUP(sublen, bus->blocksize), dlen);
			errcode = -1;
		} else if (SDPCM_PACKET_CHANNEL(&dptr[SDPCM_FRAMETAG_LEN]) != SDPCM_GLOM_CHANNEL) {
			AP6211_ERR("%s (superframe): bad channel %d\n", __FUNCTION__,
			           SDPCM_PACKET_CHANNEL(&dptr[SDPCM_FRAMETAG_LEN]));
			errcode = -1;
		} else if (SDPCM_GLOMDESC(&dptr[SDPCM_FRAMETAG_LEN])) {
			AP6211_ERR("%s (superframe): got second descriptor?\n", __FUNCTION__);
			errcode = -1;
		} else if ((doff < SDPCM_HDRLEN_RX) ||
		           (doff > (PKTLEN(osh, pfirst) - SDPCM_HDRLEN_RX))) {
			AP6211_ERR("%s (superframe): Bad data offset %d: HW %d pkt %d min %d\n",
		...
		if (rxseq != seq) {
			AP6211_DEBUG("%s: (superframe) rx_seq %d, expected %d\n",
		...
		if ((uint8)(txmax - bus->tx_seq) > 0x40) {
			AP6211_ERR("%s: got unlikely tx max %d with tx_seq %d\n",
		...
		for (num = 0, pnext = pfirst; pnext && !errcode;
			...
			if ((uint16)~(sublen^check)) {
				AP6211_ERR("%s (subframe %d): HW hdr error: "
				           "len/check 0x%04x/0x%04x\n",
				           __FUNCTION__, num, sublen, check);
				errcode = -1;
			} else if ((sublen > dlen) || (sublen < SDPCM_HDRLEN_RX)) {
				AP6211_ERR("%s (subframe %d): length mismatch: "
				           "len 0x%04x, expect 0x%04x\n",
				           __FUNCTION__, num, sublen, dlen);
				errcode = -1;
			} else if ((chan != SDPCM_DATA_CHANNEL) &&
			           (chan != SDPCM_EVENT_CHANNEL)) {
				AP6211_ERR("%s (subframe %d): bad channel %d\n",
				           __FUNCTION__, num, chan);
				errcode = -1;
			} else if ((doff < SDPCM_HDRLEN_RX) || (doff > sublen)) {
				AP6211_ERR("%s (subframe %d): Bad data offset %d: HW %d min %d\n",
		...
		for (num = 0; pfirst; rxseq++, pfirst = pnext) {
			...
			AP6211_DEBUG("%s: Get subframe %d, %p(%p/%d), sublen %d chan %d seq %d\n",
			...
			if (rxseq != seq) {
				AP6211_DEBUG("%s: rx_seq %d, expected %d\n",
				          __FUNCTION__, seq, rxseq);
				bus->rx_badseq++;
				rxseq = seq;
			}
			...
			if (PKTLEN(osh, pfirst) == 0) {
				...
				&reorder_info_len) != 0) {
				AP6211_ERR("%s: rx protocol error\n", __FUNCTION__);
				bus->dhd->rx_errors++;
				PKTFREE(osh, pfirst, FALSE);
				continue;
			}
...
#ifdef DHD_DEBUG
			if (DHD_GLOM_ON()) {
				AP6211_DEBUG("%s subframe %d to stack, %p(%p/%d) nxt/lnk %p/%p\n",
	...
	*finished = FALSE;
	...
	for (rxseq = bus->rx_seq, rxleft = maxframes;
		...
		if (bus->glom || bus->glomd) {
			uint8 cnt;
			AP6211_DEBUG("%s: calling rxglom: glomd %p, glom %p\n",
			          __FUNCTION__, bus->glomd, bus->glom);
			cnt = dhdsdio_rxglom(bus, rxseq);
			AP6211_DEBUG("%s: rxglom returned %d\n", __FUNCTION__, cnt);
			rxseq += cnt - 1;
		...
		if (dhd_readahead && bus->nextlen) {
			...
			if (!(pkt = PKTGET(osh, rdlen + DHD_SDALIGN, FALSE))) {
				if (bus->bus == SPI_BUS) {
					...
					AP6211_ERR("%s (nextlen): PKTGET failed: len %d rdlen %d "
					           "expected rxseq %d\n",
				...
				if (sdret < 0) {
					AP6211_ERR("%s (nextlen): read %d bytes failed: %d\n",
					   __FUNCTION__, rdlen, sdret);
					PKTFREE(bus->dhd->osh, pkt, FALSE);
					bus->dhd->rx_errors++;
					dhd_os_sdunlock_rxq(bus->dhd);
			...
			if (!(len|check)) {
				AP6211_DEBUG("%s (nextlen): read zeros in HW header???\n",
				           __FUNCTION__);
				dhd_os_sdlock_rxq(bus->dhd);
				PKTFREE2();
				dhd_os_sdunlock_rxq(bus->dhd);
			...
			if ((uint16)~(len^check)) {
				AP6211_ERR("%s (nextlen): HW hdr error: nextlen/len/check"
			...
			if (len < SDPCM_HDRLEN_RX) {
				AP6211_ERR("%s (nextlen): HW hdr length invalid: %d\n",
				           __FUNCTION__, len);
				dhd_os_sdlock_rxq(bus->dhd);
				PKTFREE2();
				dhd_os_sdunlock_rxq(bus->dhd);
			...
			if (len_consistent) {
				...
				AP6211_ERR("%s (nextlen): mismatch, nextlen %d len %d rnd %d; "
				           "expected rxseq %d\n",
			...
			txmax = SDPCM_WINDOW_VALUE(&bus->rxhdr[SDPCM_FRAMETAG_LEN]);
				...
				if ((bus->nextlen << 4) > MAX_RX_DATASZ) {
					AP6211_DEBUG("%s (nextlen): got frame w/nextlen too large"
					          " (%d), seq %d\n", __FUNCTION__, bus->nextlen,
					          seq);
					bus->nextlen = 0;
				}
			...
			if (rxseq != seq) {
				AP6211_DEBUG("%s (nextlen): rx_seq %d, expected %d\n",
				          __FUNCTION__, seq, rxseq);
				bus->rx_badseq++;
				rxseq = seq;
			}
			...
			if (chan == SDPCM_CONTROL_CHANNEL) {
				if (bus->bus == SPI_BUS) {
					...
					AP6211_ERR("%s (nextlen): readahead on control"
					           " packet %d?\n", __FUNCTION__, seq);
			...
			if ((bus->bus == SPI_BUS) && !bus->usebufpool) {
				AP6211_ERR("Received %d bytes on %d channel. Running out of "
			...
			if ((doff < SDPCM_HDRLEN_RX) || (doff > len)) {
				AP6211_ERR("%s (nextlen): bad data offset %d: HW len %d min %d\n",
		...
		if (sdret < 0) {
			AP6211_ERR("%s: RXHEADER FAILED: %d\n", __FUNCTION__, sdret);
			bus->rx_hdrfail++;
			dhdsdio_rxfail(bus, TRUE, TRUE);
			continue;
		}
		...
		/* Validate check bytes */
		if ((uint16)~(len^check)) {
			AP6211_ERR("%s: HW hdr error: len/check 0x%04x/0x%04x\n",
		...
		/* Validate frame length */
		if (len < SDPCM_HDRLEN_RX) {
			AP6211_ERR("%s: HW hdr length invalid: %d\n", __FUNCTION__, len);
			continue;
		}
		...
		/* Validate data offset */
		if ((doff < SDPCM_HDRLEN_RX) || (doff > len)) {
			AP6211_ERR("%s: Bad data offset %d: HW len %d, min %d seq %d\n",
		...
		if ((bus->nextlen << 4) > MAX_RX_DATASZ) {
			AP6211_DEBUG("%s (nextlen): got frame w/nextlen too large (%d), seq %d\n",
			          __FUNCTION__, bus->nextlen, seq);
			bus->nextlen = 0;
		}
		...
		if ((rdlen + firstread) > MAX_RX_DATASZ) {
			...
			AP6211_ERR("%s: too long: len %d rdlen %d\n", __FUNCTION__, len, rdlen);
			bus->dhd->rx_errors++; bus->rx_toolong++;
			dhdsdio_rxfail(bus, FALSE, FALSE);
			continue;
		}
		...
		if (!(pkt = PKTGET(osh, (rdlen + firstread + DHD_SDALIGN), FALSE))) {
			/* Give up on data, request rtx of events */
			AP6211_ERR("%s: PKTGET failed: rdlen %d chan %d\n",
		...
		if (sdret < 0) {
			AP6211_ERR("%s: read %d %s bytes failed: %d\n", __FUNCTION__, rdlen,
...
deliver:
		...
		if (chan == SDPCM_GLOM_CHANNEL) {
			if (SDPCM_GLOMDESC(&bus->rxhdr[SDPCM_FRAMETAG_LEN])) {
				AP6211_DEBUG("%s: got glom descriptor, %d bytes:\n",
				          __FUNCTION__, len);
#ifdef DHD_DEBUG
				if (DHD_GLOM_ON()) {
				...
				AP6211_ERR("%s: glom superframe w/o descriptor!\n", __FUNCTION__);
				dhdsdio_rxfail(bus, FALSE, FALSE);
			}
			continue;
		}
	...
	if (!rxleft && !sdtest)
		AP6211_DEBUG("%s: hit rx limit of %d frames\n", __FUNCTION__, maxframes);
	else
#endif /* DHD_DEBUG */
	AP6211_DEBUG("%s: processed %d frames\n", __FUNCTION__, rxcount);
...
dhdsdio_hostmail(dhd_bus_t *bus)
{
	...
	if (hmb_data & HMB_DATA_NAKHANDLED) {
		...
		if (!bus->rxskip) {
			AP6211_ERR("%s: unexpected NAKHANDLED!\n", __FUNCTION__);
		}
		bus->rxskip = FALSE;
	...
	if (hmb_data & (HMB_DATA_DEVREADY | HMB_DATA_FWREADY)) {
		...
		if (bus->sdpcm_ver != SDPCM_PROT_VERSION)
			AP6211_ERR("Version mismatch, dongle reports %d, expecting %d\n",
			           bus->sdpcm_ver, SDPCM_PROT_VERSION);
		else
			AP6211_ERR("Dongle ready, protocol version %d\n", bus->sdpcm_ver);
	...
	if (hmb_data & HMB_DATA_FWHALT) {
		AP6211_ERR("INTERNAL ERROR: FIRMWARE HALTED : set BUS DOWN\n");
		dhdsdio_checkdied(bus, NULL, 0);
		bus->dhd->busstate = DHD_BUS_DOWN;
	}
#endif /* DHD_DEBUG */
	...
	if (hmb_data & ~(HMB_DATA_DEVREADY |
		...
		AP6211_ERR("Unknown mailbox data content: 0x%02x\n", hmb_data);
	}
...
dhdsdio_dpc(dhd_bus_t *bus)
{
	...
	if (!SLPAUTO_ENAB(bus) && (bus->clkstate == CLK_PENDING)) {
		...
		devctl = bcmsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, &err);
		if (err) {
			AP6211_ERR("%s: error reading DEVCTL: %d\n", __FUNCTION__, err);
			bus->dhd->busstate = DHD_BUS_DOWN;
		} else {
		...
		clkctl = bcmsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, &err);
		if (err) {
			AP6211_ERR("%s: error reading CSR: %d\n", __FUNCTION__, err);
			bus->dhd->busstate = DHD_BUS_DOWN;
		}

		AP6211_DEBUG("DPC: PENDING, devctl 0x%02x clkctl 0x%02x\n", devctl, clkctl);
		...
		if (SBSDIO_HTAV(clkctl)) {
			...
			bcmsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, devctl, &err);
			if (err) {
				AP6211_ERR("%s: error writing DEVCTL: %d\n",
				           __FUNCTION__, err);
				bus->dhd->busstate = DHD_BUS_DOWN;
			}
			bus->clkstate = CLK_AVAIL;
	...
	if (intstatus & I_RD_OOSYNC) {
		AP6211_DEBUG("Dongle reports RD_OOSYNC\n");
		intstatus &= ~I_RD_OOSYNC;
	}

	if (intstatus & I_SBINT) {
		AP6211_DEBUG("Dongle reports SBINT\n");
		intstatus &= ~I_SBINT;
	}

	/* Would be active due to wake-wlan in gSPI */
	if (intstatus & I_CHIPACTIVE) {
		AP6211_DEBUG("Dongle reports CHIPACTIVE\n");
		intstatus &= ~I_CHIPACTIVE;
	}
...
clkwait:
	...
	if (bus->intr && bus->intdis && !bcmsdh_regfail(sdh)) {
		AP6211_DEBUG("%s: enable SDIO interrupts, rxdone %d framecnt %d\n",
	...
	if (TXCTLOK(bus) && bus->ctrl_frame_stat && (bus->clkstate == CLK_AVAIL))  {
		...
		if (*frame_seq != bus->tx_seq) {
			AP6211_DEBUG("%s IOCTL frame seq lag detected!"
	...
	if ((bus->dhd->busstate == DHD_BUS_DOWN) || bcmsdh_regfail(sdh)) {
		if ((bus->sih && bus->sih->buscorerev >= 12) && !(dhdsdio_sleepcsr_get(bus) &
			...
			AP6211_ERR("%s: Bus failed due to KSO\n", __FUNCTION__);
			bus->kso = FALSE;
		} else {
			AP6211_ERR("%s: failed backplane access over SDIO, halting operation\n",
				__FUNCTION__);
			bus->dhd->busstate = DHD_BUS_DOWN;
			bus->intstatus = 0;
		}
...
dhd_bus_dpc(struct dhd_bus *bus)
{
	...
	AP6211_DEBUG("Calling dhdsdio_dpc() from %s\n", __FUNCTION__);
...
dhdsdio_isr(void *arg)
{
	...
	if (!bus) {
		AP6211_ERR("%s : bus is null pointer , exit \n", __FUNCTION__);
		return;
	}
	sdh = bus->sdh;

	if (bus->dhd->busstate == DHD_BUS_DOWN) {
		AP6211_ERR("%s : bus is down. we have nothing to do\n", __FUNCTION__);
		return;
	}
	...
	if (!SLPAUTO_ENAB(bus)) {
		if (bus->sleeping) {
			AP6211_ERR("INTERRUPT WHILE SLEEPING??\n");
			return;
		} else if (!KSO_ENAB(bus)) {
			AP6211_ERR("ISR in devsleep 1\n");
		}
	}
	...
	if (bus->intr) {
		...
		AP6211_ERR("dhdsdio_isr() w/o interrupt configured!\n");
	}
...
dhdsdio_pktgen(dhd_bus_t *bus)
{
	...
	for (pktcount = 0; pktcount < bus->pktgen_count; pktcount++) {
		...
		default:
			AP6211_ERR("Unrecognized pktgen mode %d\n", bus->pktgen_mode);
			PKTFREE(osh, pkt, TRUE);
			bus->pktgen_count = 0;
			return;
		}
...
dhdsdio_testrcv(dhd_bus_t *bus, void *pkt, uint seq)
{
	...
	extra = *data++;
	len = *data++; len += *data++ << 8;
	AP6211_DEBUG("%s:cmd:%d, xtra:%d,len:%d\n", __FUNCTION__, cmd, extra, len);
	/* Check length for relevant commands */
	if (cmd == SDPCM_TEST_DISCARD || cmd == SDPCM_TEST_ECHOREQ || cmd == SDPCM_TEST_ECHORSP) {
		if (pktlen != len + SDPCM_TEST_HDRLEN) {
			AP6211_ERR("dhdsdio_testrcv: frame length mismatch, pktlen %d seq %d"
	...
	case SDPCM_TEST_ECHORSP:
		...
		for (offset = 0; offset < len; offset++, data++) {
			if (*data != SDPCM_TEST_FILL(offset, extra)) {
				AP6211_ERR("dhdsdio_testrcv: echo data mismatch: "
	...
	case SDPCM_TEST_DISCARD:
		{
			...
			for (i = 0; i < len; i++) {
				if (*prn != testval) {
					AP6211_ERR("DIErr@Pkt#:%d,Ix:%d, expected:0x%x, got:0x%x\n",
	...
	default:
		AP6211_DEBUG("dhdsdio_testrcv: unsupported or unknown command, pktlen %d seq %d"
		          " cmd %d extra %d len %d\n", pktlen, seq, cmd, extra, len);
		PKTFREE(osh, pkt, FALSE);
		break;
	}
	...
	if (bus->pktgen_mode == DHD_PKTGEN_RECV) {
		if (bus->pktgen_rcv_state != PKTGEN_RCV_IDLE) {
			...
			bus->pktgen_count = 0;
			AP6211_ERR("Pktgen:rcv test complete!\n");
...
{
	uint byte, tag, tdata;
	AP6211_DEBUG("Function %d CIS:\n", fn);

	for (tdata = byte = 0; byte < SBSDIO_CIS_SIZE_LIMIT; byte++) {
		if ((byte % 16) == 0)
			AP6211_DUMP("    ");
		AP6211_DUMP("%02x ", cis[byte]);
		if ((byte % 16) == 15)
			AP6211_DUMP("\n");
		if (!tdata--) {
			tag = cis[byte];
	...
	uint16 func, uint bustype, void *regsva, osl_t * osh, void *sdh)
{
	...
	if (mutex_is_locked(&_dhd_sdio_mutex_lock_) == 0) {
		AP6211_DEBUG("%s : no mutex held. set lock\n", __FUNCTION__);
	}
	else {
		AP6211_DEBUG("%s : mutex is locked!. wait for unlocking\n", __FUNCTION__);
	}
	...
	AP6211_DEBUG("%s: Enter\n", __FUNCTION__);
	AP6211_DEBUG("%s: venid 0x%04x devid 0x%04x\n", __FUNCTION__, venid, devid);
	...
	switch (venid) {
		...
		case VENDOR_BROADCOM:
			break;
		default:
			AP6211_ERR("%s: unknown vendor: 0x%04x\n",
			           __FUNCTION__, venid);
			goto forcereturn;
	}
	...
	switch (devid) {
		...
		case BCM4325_D11A_ID:			/* 4325 802.11a 5Ghz band id */
			AP6211_DEBUG("%s: found 4325 Dongle\n", __FUNCTION__);
			break;
		case BCM4329_D11N_ID:		/* 4329 802.11n dualband device */
		case BCM4329_D11N2G_ID:		/* 4329 802.11n 2.4G device */
		case BCM4329_D11N5G_ID:		/* 4329 802.11n 5G device */
		case 0x4329:
			AP6211_DEBUG("%s: found 4329 Dongle\n", __FUNCTION__);
			break;
		case BCM4315_D11DUAL_ID:		/* 4315 802.11a/g id */
		case BCM4315_D11G_ID:			/* 4315 802.11g id */
		case BCM4315_D11A_ID:			/* 4315 802.11a id */
			AP6211_DEBUG("%s: found 4315 Dongle\n", __FUNCTION__);
			break;
		case BCM4319_D11N_ID:			/* 4319 802.11n id */
		case BCM4319_D11N2G_ID:			/* 4319 802.11n2g id */
		case BCM4319_D11N5G_ID:			/* 4319 802.11n5g id */
			AP6211_DEBUG("%s: found 4319 Dongle\n", __FUNCTION__);
			break;
		case 0:
			AP6211_DEBUG("%s: allow device id 0, will check chip internals\n",
			          __FUNCTION__);
			break;

		default:
			AP6211_ERR("%s: skipping 0x%04x/0x%04x, not a dongle\n",
			           __FUNCTION__, venid, devid);
			goto forcereturn;
	}
	...
	if (osh == NULL) {
		...
		if (!(osh = dhd_osl_attach(sdh, DHD_BUS))) {
			AP6211_ERR("%s: osl_attach failed!\n", __FUNCTION__);
			goto forcereturn;
		}
	}
	...
	if (!(bus = MALLOC(osh, sizeof(dhd_bus_t)))) {
		AP6211_ERR("%s: MALLOC of dhd_bus_t failed\n", __FUNCTION__);
		goto fail;
	}
	...
	if (!(dhdsdio_probe_attach(bus, osh, sdh, regsva, devid))) {
		AP6211_ERR("%s: dhdsdio_probe_attach failed\n", __FUNCTION__);
		goto fail;
	}

	/* Attach to the dhd/OS/network interface */
	if (!(bus->dhd = dhd_attach(osh, bus, SDPCM_RESERVE))) {
		AP6211_ERR("%s: dhd_attach failed\n", __FUNCTION__);
		goto fail;
	}

	/* Allocate buffers */
	if (!(dhdsdio_probe_malloc(bus, osh, sdh))) {
		AP6211_ERR("%s: dhdsdio_probe_malloc failed\n", __FUNCTION__);
		goto fail;
	}

	if (!(dhdsdio_probe_init(bus, osh, sdh))) {
		AP6211_ERR("%s: dhdsdio_probe_init failed\n", __FUNCTION__);
		goto fail;
	}

	if (bus->intr) {
		/* Register interrupt callback, but mask it (not operational yet). */
		AP6211_DEBUG("%s: disable SDIO interrupts (not interested yet)\n", __FUNCTION__);
		bcmsdh_intr_disable(sdh);
		if ((ret = bcmsdh_intr_reg(sdh, dhdsdio_isr, bus)) != 0) {
			AP6211_ERR("%s: FAILED: bcmsdh_intr_reg returned %d\n",
			           __FUNCTION__, ret);
			goto fail;
		}
		AP6211_DEBUG("%s: registered SDIO interrupt function ok\n", __FUNCTION__);
	} else {
		AP6211_DEBUG("%s: SDIO interrupt function is NOT registered due to polling mode\n",
		           __FUNCTION__);
	}

	AP6211_DEBUG("%s: completed!!\n", __FUNCTION__);
	...
	if (dhd_download_fw_on_driverload) {
		if ((ret = dhd_bus_start(bus->dhd)) != 0) {
			AP6211_ERR("%s: dhd_bus_start failed\n", __FUNCTION__);
			goto fail;
		}
	}
	/* Ok, have the per-port tell the stack we're open for business */
	if (dhd_net_attach(bus->dhd, 0) != 0) {
		AP6211_ERR("%s: Net attach failed!!\n", __FUNCTION__);
		goto fail;
	}

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 25))
	mutex_unlock(&_dhd_sdio_mutex_lock_);
	AP6211_DEBUG("%s : the lock is released.\n", __FUNCTION__);
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)) */
                     ...
                     uint16 devid)
{
	...
	if (dhdsdio_set_siaddr_window(bus, SI_ENUM_BASE)) {
		AP6211_ERR("%s: FAILED to return to SI_ENUM_BASE\n", __FUNCTION__);
	}

#ifdef DHD_DEBUG
	AP6211_DEBUG("F1 signature read @0x18000000=0x%4x\n",
	...
	if (err || ((clkctl & ~SBSDIO_AVBITS) != DHD_INIT_CLKCTL1)) {
		AP6211_ERR("dhdsdio_probe: ChipClkCSR access: err %d wrote 0x%02x read 0x%02x\n",
	...
	if (DHD_INFO_ON()) {
		...
		for (fn = 0; fn <= numfn; fn++) {
			if (!(cis[fn] = MALLOC(osh, SBSDIO_CIS_SIZE_LIMIT))) {
				AP6211_DEBUG("dhdsdio_probe: fn %d cis malloc failed\n", fn);
				break;
			}
			bzero(cis[fn], SBSDIO_CIS_SIZE_LIMIT);

			if ((err = bcmsdh_cis_read(sdh, fn, cis[fn], SBSDIO_CIS_SIZE_LIMIT))) {
				AP6211_DEBUG("dhdsdio_probe: fn %d cis read err %d\n", fn, err);
		...
		if (err) {
			AP6211_ERR("dhdsdio_probe: failure reading or parsing CIS\n");
			goto fail;
		}
	}
#endif /* DHD_DEBUG */
	...
	if (!dhdsdio_chipmatch((uint16)bus->sih->chip)) {
		AP6211_ERR("%s: unsupported chip: 0x%04x\n",
	...
	if (!DHD_NOPMU(bus)) {
		...
		if (!(bus->orig_ramsize = si_socram_size(bus->sih))) {
			AP6211_ERR("%s: failed to find SOCRAM memory!\n", __FUNCTION__);
			goto fail;
		}
		} else {
			/* cr4 has a different way to find the RAM size from TCM's */
			if (!(bus->orig_ramsize = si_tcm_size(bus->sih))) {
				AP6211_ERR("%s: failed to find CR4-TCM memory!\n", __FUNCTION__);
				goto fail;
			}
		...
		AP6211_DEBUG("DHD: dongle ram size is set to %d(orig %d)\n",
			bus->ramsize, bus->orig_ramsize);
	...
	if (!(bus->regs = si_setcore(bus->sih, PCMCIA_CORE_ID, 0)) &&
	    !(bus->regs = si_setcore(bus->sih, SDIOD_CORE_ID, 0))) {
		AP6211_ERR("%s: failed to find SDIODEV core!\n", __FUNCTION__);
		goto fail;
	}
...
dhdsdio_probe_malloc(dhd_bus_t *bus, osl_t *osh, void *sdh)
{
	...
	if (bus->dhd->maxctl) {
		...
		if (!(bus->rxbuf = DHD_OS_PREALLOC(osh, DHD_PREALLOC_RXBUF, bus->rxblen))) {
			AP6211_ERR("%s: MALLOC of %d-byte rxbuf failed\n",
	...
	if (!(bus->databuf = DHD_OS_PREALLOC(osh, DHD_PREALLOC_DATABUF, MAX_DATA_BUF))) {
		AP6211_ERR("%s: MALLOC of %d-byte databuf failed\n",
...
dhdsdio_probe_init(dhd_bus_t *bus, osl_t *osh, void *sdh)
{
	...
	if (bcmsdh_iovar_op(sdh, "sd_divisor", NULL, 0,
		...
		AP6211_DEBUG("%s: Initial value for %s is %d\n",
	...
	if (bcmsdh_iovar_op(sdh, "sd_rxchain", NULL, 0,
		...
		AP6211_DEBUG("%s: bus module (through bcmsdh API) %s chaining\n",
...
dhd_bus_select_firmware_name_by_chip(struct dhd_bus *bus, char *dst, char *src)
{
	...
	if (src[0] == '\0') {
#ifdef CONFIG_AP6211_FW_PATH
		bcm_strncpy_s(src, sizeof(fw_path), CONFIG_AP6211_FW_PATH, MOD_PARAM_PATHLEN-1);
		if (src[0] == '\0')
#endif
		{
			AP6211_DEBUG("src firmware path is null\n");
			return;
		}
	}
	...
	AP6211_DEBUG("%s: firmware_path=%s\n", __FUNCTION__, dst);
}
...
dhdsdio_download_firmware(struct dhd_bus *bus, osl_t *osh, void *sdh)
{
	...
	AP6211_ERR("Final fw_path=%s\n", bus->fw_path);
	AP6211_ERR("Final nv_path=%s\n", bus->nv_path);
...
{
	AP6211_DEBUG("%s: Enter bus->dhd %p bus->dhd->dongle_reset %d \n", __FUNCTION__,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 25))
	mutex_unlock(&_dhd_sdio_mutex_lock_);
	AP6211_ERR("%s : the lock is released.\n", __FUNCTION__);
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 25)) */
...
dhdsdio_download_code_array(struct dhd_bus *bus)
{
	...
	AP6211_ERR("%s: download embedded firmware...\n", __FUNCTION__);
	...
	while ((offset + MEMBLOCK) < sizeof(dlarray)) {
		...
		if (bcmerror) {
			AP6211_ERR("%s: error %d on writing %d membytes at 0x%08x\n",
	...
	/* Upload and compare the downloaded code */
	{
		...
		while ((offset + MEMBLOCK) < sizeof(dlarray)) {
			...
			if (bcmerror) {
				AP6211_ERR("%s: error %d on reading %d membytes at 0x%08x\n",
					__FUNCTION__, bcmerror, MEMBLOCK, offset);
				goto err;
			}
		...
		if (memcmp(dlarray, ularray, sizeof(dlarray))) {
			AP6211_ERR("%s: Downloaded image is corrupted (%s, %s, %s).\n",
			           __FUNCTION__, dlimagename, dlimagever, dlimagedate);
			goto err;
		} else
			AP6211_ERR("%s: Download, Upload and compare succeeded (%s, %s, %s).\n",
			           __FUNCTION__, dlimagename, dlimagever, dlimagedate);
...
dhdsdio_download_code_file(struct dhd_bus *bus, char *pfw_path)
{
	...
	AP6211_ERR("download firmware %s\n", pfw_path);
	...
	if (memblock == NULL) {
		AP6211_ERR("%s: Failed to allocate memory %d bytes\n", __FUNCTION__, MEMBLOCK);
		goto err;
	}
	...
	while ((len = dhd_os_get_image_block((char*)memptr, MEMBLOCK, image))) {
		if (len < 0) {
			AP6211_ERR("%s: dhd_os_get_image_block failed (%d)\n", __FUNCTION__, len);
			bcmerror = BCME_ERROR;
			goto err;
		}
		...
		if (dhd_msg_level & DHD_TRACE_VAL) {
			...
			if (memcmp(memptr_tmp, memptr, len)) {
				AP6211_ERR("%s: Downloaded image is corrupted.\n", __FUNCTION__);
				goto err;
			} else
				AP6211_ERR("%s: Download, Upload and compare succeeded.\n", __FUNCTION__);
		}
		offset += MEMBLOCK;
	}
...
dhdsdio_download_nvram(struct dhd_bus *bus)
{
	...
	if (len > 0 && len < MAX_NVRAMBUF_SIZE) {
		...
		*bufp++ = 0;
		...
		if (bcmerror) {
			AP6211_ERR("%s: error downloading vars: %d\n",
			           __FUNCTION__, bcmerror);
		}
	}
	else {
		AP6211_ERR("%s: error reading nvram file: %d\n",
		           __FUNCTION__, len);
		bcmerror = BCME_SDIO_ERROR;
	}
...
_dhdsdio_download_firmware(struct dhd_bus *bus)
{
	...
	if (dhdsdio_download_state(bus, TRUE)) {
		AP6211_ERR("%s: error placing ARM core in reset\n", __FUNCTION__);
		goto err;
	}
	...
	if ((bus->fw_path != NULL) && (bus->fw_path[0] != '\0')) {
		if (dhdsdio_download_code_file(bus, bus->fw_path)) {
			AP6211_ERR("%s: dongle image file download failed\n", __FUNCTION__);
#ifdef BCMEMBEDIMAGE
			embed = TRUE;
#else
			goto err;
	...
	if (embed) {
		if (dhdsdio_download_code_array(bus)) {
			AP6211_ERR("%s: dongle image array download failed\n", __FUNCTION__);
			goto err;
		}
		else {
			dlok = TRUE;
...
#endif
	if (!dlok) {
		AP6211_ERR("%s: dongle image download failed\n", __FUNCTION__);
		goto err;
	}
	...
	if (dhdsdio_download_nvram(bus)) {
		AP6211_ERR("%s: dongle nvram file download failed\n", __FUNCTION__);
		goto err;
	}

	/* Take arm out of reset */
	if (dhdsdio_download_state(bus, FALSE)) {
		AP6211_ERR("%s: error getting out of ARM core reset\n", __FUNCTION__);
		goto err;
	}
...
dhd_bus_devreset(dhd_pub_t *dhdp, uint8 flag)
{
	...
	if (flag == TRUE) {
		...
		if (bus->dhd->dongle_reset) {
			...
			bcmerror = BCME_SDIO_ERROR;
			AP6211_DEBUG("%s called when dongle is not in reset\n",
				__FUNCTION__);
			AP6211_DEBUG("Will call dhd_bus_start instead\n");
			sdioh_start(NULL, 1);
#if defined(HW_OOB)
			...
			if ((bcmerror = dhd_bus_start(dhdp)) != 0)
				AP6211_ERR("%s: dhd_bus_start fail with %d\n",
					__FUNCTION__, bcmerror);
		}
	}
	return bcmerror;
