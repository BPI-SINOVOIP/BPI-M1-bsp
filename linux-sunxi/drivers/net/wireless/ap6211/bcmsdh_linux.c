...
int bcmsdh_probe(struct device *dev)
{
	...
	if  (irq < 0) {
		AP6211_ERR("%s: Host irq is not defined\n", __FUNCTION__);
		return 1;
	}
#endif 
	...
	if (!(sdhc = MALLOC(osh, sizeof(bcmsdh_hc_t)))) {
		AP6211_ERR("%s: out of memory, allocated %d bytes\n",
			__FUNCTION__,
			MALLOCED(osh));
		goto err;
	}
	...
	if (!(sdh = bcmsdh_attach(osh, (void *)0,
	                          (void **)&regs, irq))) {
		AP6211_ERR("%s: bcmsdh_attach failed\n", __FUNCTION__);
		goto err;
	}
#else
	...
	if (!(sdhc->ch = drvinfo.attach((vendevid >> 16),
	                                ...
	                                (void *)regs, NULL, sdh))) {
		AP6211_ERR("%s: device attach failed\n", __FUNCTION__);
		goto err;
	}
...
bcmsdh_pci_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (sd_pci_slot != 0xFFFFffff) {
		if (pdev->bus->number != (sd_pci_slot>>16) ||
			...
			AP6211_DEBUG("%s: %s: bus %X, slot %X, vend %X, dev %X\n",
				__FUNCTION__,
		...
		AP6211_DEBUG("%s: %s: bus %X, slot %X, vendor %X, device %X (good PCI location)\n",
			__FUNCTION__,
			bcmsdh_chipmatch(pdev->vendor, pdev->device)
	...
	if ((pdev->vendor == VENDOR_TI) && ((pdev->device == PCIXX21_FLASHMEDIA_ID) ||
		...
		AP6211_ERR("%s: Disabling TI FlashMedia Controller.\n", __FUNCTION__);
	...
	if (rc) {
		AP6211_ERR("%s: Cannot enable PCI device\n", __FUNCTION__);
		goto err;
	}
...
bcmsdh_register(bcmsdh_driver_t *driver)
{
...
#if defined(BCMPLATFORM_BUS)
	AP6211_ERR("Linux Kernel SDIO/MMC Driver\n");
	...
	AP6211_ERR("%s: pci_module_init failed 0x%x\n", __FUNCTION__, error);
#endif /* BCMPLATFORM_BUS */
...
#if defined(OOB_INTR_ONLY)
#define CONFIG_ARCH_SUN6I_AP6211 1
...
static irqreturn_t wlan_oob_irq(int irq, void *dev_id)
{
	...
	if (dhdp == NULL) {
		AP6211_ERR("Out of band GPIO interrupt fired way too early\n");
		return IRQ_HANDLED;
	}
...
int bcmsdh_register_oob_intr(void * dhdp)
{
	...
	if (!sdhcinfo->oob_irq_registered) {
		...
		if (ret) {
                        AP6211_ERR("request irq%d failed\n", wl_host_wake_irqno);
			return -1;
                }

#if defined(CONFIG_ARCH_RHEA) || defined(CONFIG_ARCH_CAPRI) || defined(CONFIG_ARCH_SUN6I_AP6211)
...
#endif
		if (error)
			AP6211_ERR("%s enable_irq_wake error=%d \n", __FUNCTION__, error);
