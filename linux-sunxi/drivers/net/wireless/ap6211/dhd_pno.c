...
_dhd_pno_clean(dhd_pub_t *dhd)
{
	...
	if (err < 0) {
		AP6211_ERR("%s : failed to execute pfn(error : %d)\n",
			__FUNCTION__, err);
		goto exit;
	}
	...
	if (err < 0) {
		AP6211_ERR("%s : failed to execute pfnclear(error : %d)\n",
			__FUNCTION__, err);
	}
exit:
	return err;
...
_dhd_pno_suspend(dhd_pub_t *dhd)
{
	...
	if (err < 0) {
		AP6211_ERR("%s : failed to suspend pfn(error :%d)\n", __FUNCTION__, err);
		goto exit;
...
_dhd_pno_enable(dhd_pub_t *dhd, int enable)
{
	...
	if (enable & 0xfffe) {
		AP6211_ERR("%s invalid value\n", __FUNCTION__);
		err = BCME_BADARG;
		goto exit;
	}
	if (!dhd_support_sta_mode(dhd)) {
		AP6211_ERR("PNO is not allowed for non-STA mode");
		err = BCME_BADOPTION;
		goto exit;
	}
	if (enable) {
		if ((_pno_state->pno_mode & DHD_PNO_LEGACY_MODE) &&
			...
			AP6211_ERR("%s Legacy PNO mode cannot be enabled "
	...
	if (err < 0) {
		AP6211_ERR("%s : failed to execute pfn_set\n", __FUNCTION__);
		goto exit;
	}
...
_dhd_pno_set(dhd_pub_t *dhd, const dhd_pno_params_t *pno_params, dhd_pno_mode_t mode)
{
	...
	if (mode == DHD_PNO_LEGACY_MODE) {
		...
		if (_pno_state->pno_mode & DHD_PNO_BATCH_MODE) {
			AP6211_DEBUG("will enable combined scan with BATCHIG SCAN MODE\n");
			mode |= DHD_PNO_BATCH_MODE;
			combined_scan = TRUE;
		} else if (_pno_state->pno_mode & DHD_PNO_HOTLIST_MODE) {
			AP6211_DEBUG("will enable combined scan with HOTLIST SCAN MODE\n");
	...
	if (pfn_param.scan_freq < htod32(PNO_SCAN_MIN_FW_SEC) ||
		...
		AP6211_ERR("%s pno freq(%d sec) is not valid \n",
	...
	if (mode == DHD_PNO_BATCH_MODE) {
		...
		if (err < 0) {
			AP6211_ERR("%s : failed to set pfnmscan\n", __FUNCTION__);
			goto exit;
		}
		/* get max mscan which the firmware supports */
		err = dhd_iovar(dhd, 0, "pfnmem", (char *)&_tmp, sizeof(_tmp), 0);
		if (err < 0) {
			AP6211_ERR("%s : failed to get pfnmscan\n", __FUNCTION__);
			goto exit;
		}
		AP6211_DEBUG(" returned mscan : %d, set bestn : %d\n", _tmp, pfn_param.bestn);
...
_dhd_pno_add_ssid(dhd_pub_t *dhd, wlc_ssid_t* ssids_list, int nssid)
{
	...
	for (i = 0; i < nssid; i++) {
		...
		if (err < 0) {
			AP6211_ERR("%s : failed to execute pfn_add\n", __FUNCTION__);
			goto exit;
		}
	}
exit:
	...
	int *nchan, uint8 band, bool skip_dfs)
{
	...
	if (err < 0) {
		AP6211_ERR("failed to get channel list (err: %d)\n", err);
		goto exit;
	}
	...
	char *buf, int nbufsize)
{
	...
	list_for_each_entry_safe(siter, snext,
		...
		while (phead != NULL) {
			...
			AP6211_DEBUG("%s", SCAN_END_MARKER);
			nleftsize -= nreadsize;
			pprev = phead;
			/* reset the header */
...
exit:
	if (cnt < params_batch->get_batch.expired_tot_scan_cnt) {
		AP6211_ERR("Buffer size is small to save all batch entry,"
...
_dhd_pno_cfg(dhd_pub_t *dhd, uint16 *channel_list, int nchan)
{
	...
	if (err < 0) {
		AP6211_ERR("%s : failed to execute pfn_cfg\n", __FUNCTION__);
		goto exit;
	}
exit:
	return err;
...
dhd_pno_stop_for_ssid(dhd_pub_t *dhd)
{
	...
	_pno_state = PNO_GET_PNOSTATE(dhd);
	if (!(_pno_state->pno_mode & DHD_PNO_LEGACY_MODE)) {
		AP6211_ERR("%s : LEGACY PNO MODE is not enabled\n", __FUNCTION__);
		goto exit;
	}
	AP6211_DEBUG("%s enter\n", __FUNCTION__);
	...
	if (_pno_state->pno_mode & (DHD_PNO_BATCH_MODE | DHD_PNO_HOTLIST_MODE)) {
		...
		if (_pno_state->pno_mode & DHD_PNO_BATCH_MODE) {
			...
			if (err < 0) {
				...
				AP6211_ERR("%s : failed to restart batch scan(err: %d)\n",
					__FUNCTION__, err);
				goto exit;
			}
		} else if (_pno_state->pno_mode & DHD_PNO_HOTLIST_MODE) {
			...
			if (p_pfn_bssid == NULL) {
				AP6211_ERR("%s : failed to allocate wl_pfn_bssid_t array"
				" (count: %d)",
			...
			if (err < 0) {
				_pno_state->pno_mode &= ~DHD_PNO_HOTLIST_MODE;
				AP6211_ERR("%s : failed to restart hotlist scan(err: %d)\n",
					__FUNCTION__, err);
				goto exit;
			}
		}
		...
		if (err < 0) {
			AP6211_ERR("%s : failed to call _dhd_pno_clean (err: %d)\n",
				__FUNCTION__, err);
			goto exit;
		}
	}
	...
	uint16  scan_fr, int pno_repeat, int pno_freq_expo_max, uint16 *channel_list, int nchan)
{
	...
	AP6211_DEBUG("%s enter : scan_fr :%d, pno_repeat :%d,"
	...
	if (!(_pno_state->pno_mode & DHD_PNO_LEGACY_MODE)) {
		...
		if (err < 0) {
			AP6211_ERR("%s : failed to reinitialize profile (err %d)\n",
				__FUNCTION__, err);
			goto exit;
		}
	}
	...
	if (_pno_state->pno_mode & (DHD_PNO_BATCH_MODE | DHD_PNO_HOTLIST_MODE)) {
		AP6211_DEBUG("BATCH SCAN is on progress in firmware\n");
		/* retrieve the batching data from firmware into host */
		dhd_pno_get_for_batch(dhd, NULL, 0, PNO_STATUS_DISABLE);
		...
		if (err < 0) {
			AP6211_ERR("%s : failed to disable PNO\n", __FUNCTION__);
			goto exit;
		}
		...
		if (_pno_state->pno_mode & DHD_PNO_BATCH_MODE) {
			...
			if (_params2->params_batch.nchan > 0 && nchan > 0) {
				...
				if (err < 0) {
					AP6211_ERR("%s : failed to merge channel list"
					" between legacy and batch\n",
						__FUNCTION__);
					goto exit;
				}
			}  else {
				AP6211_DEBUG("superset channel will use"
	...
	if ((err = _dhd_pno_set(dhd, _params, DHD_PNO_LEGACY_MODE)) < 0) {
		AP6211_ERR("failed to set call pno_set (err %d) in firmware\n", err);
		goto exit;
	}
	if ((err = _dhd_pno_add_ssid(dhd, ssid_list, nssid)) < 0) {
		AP6211_ERR("failed to add ssid list (err %d) in firmware\n", err);
		goto exit;
	}
	for (i = 0; i < nssid; i++) {
		_pno_ssid = kzalloc(sizeof(struct dhd_pno_ssid), GFP_KERNEL);
		if (_pno_ssid == NULL) {
			AP6211_ERR("%s : failed to allocate struct dhd_pno_ssid\n",
				__FUNCTION__);
			goto exit;
		}
	...
	if (tot_nchan > 0) {
		if ((err = _dhd_pno_cfg(dhd, _chan_list, tot_nchan)) < 0) {
			AP6211_ERR("%s : failed to set call pno_cfg (err %d) in firmware\n",
				__FUNCTION__, err);
			goto exit;
		}
	}
	if (_pno_state->pno_status == DHD_PNO_DISABLED) {
		if ((err = _dhd_pno_enable(dhd, PNO_ON)) < 0)
			AP6211_ERR("%s : failed to enable PNO\n", __FUNCTION__);
	}
exit:
...
dhd_pno_set_for_batch(dhd_pub_t *dhd, struct dhd_pno_batch_params *batch_params)
{
	...
	if (!WLS_SUPPORTED(_pno_state)) {
		AP6211_ERR("%s : wifi location service is not supported\n", __FUNCTION__);
	...
	if (!(_pno_state->pno_mode & DHD_PNO_BATCH_MODE)) {
		...
		if (err < 0) {
			AP6211_ERR("%s : failed to call _dhd_pno_reinitialize_prof\n",
				__FUNCTION__);
			goto exit;
		}
	}
	...
	if (batch_params->band == WLC_BAND_2G || batch_params->band == WLC_BAND_5G) {
		...
		if (err < 0) {
			AP6211_ERR("%s: failed to get valid channel list(band : %d)\n",
...
#ifdef PNO_DEBUG
{
		AP6211_DEBUG("Channel list : ");
		for (i = 0; i < _params->params_batch.nchan; i++) {
			AP6211_DEBUG("%d ", _params->params_batch.chan_list[i]);
		}
		AP6211_DEBUG("\n");
}
#endif
	...
	if (_pno_state->pno_mode & DHD_PNO_LEGACY_MODE) {
		struct dhd_pno_ssid *iter, *next;
		AP6211_DEBUG("PNO SSID is on progress in firmware\n");
		/* store current pno_mode before disabling pno */
		mode = _pno_state->pno_mode;
		err = _dhd_pno_enable(dhd, PNO_OFF);
		if (err < 0) {
		...
		if (_params2->params_legacy.nchan > 0 && _params->params_batch.nchan > 0) {
			...
			AP6211_DEBUG("superset channel will use all channels in firmware\n");
		}
		p_ssid_list = kzalloc(sizeof(wlc_ssid_t) *
							_params2->params_legacy.nssid, GFP_KERNEL);
		if (p_ssid_list == NULL) {
			AP6211_ERR("%s : failed to allocate wlc_ssid_t array (count: %d)",
	...
	if ((err = _dhd_pno_set(dhd, _params, DHD_PNO_BATCH_MODE)) < 0) {
		AP6211_ERR("%s : failed to set call pno_set (err %d) in firmware\n",
			__FUNCTION__, err);
		goto exit;
	} else {
...
_dhd_pno_get_for_batch(dhd_pub_t *dhd, char *buf, int bufsize, int reason)
{
	...
	if (!(_pno_state->pno_mode & DHD_PNO_BATCH_MODE)) {
		AP6211_ERR("%s: Batching SCAN mode is not enabled\n", __FUNCTION__);
		goto exit;
	}
	mutex_lock(&_pno_state->pno_mutex);
	_params = &_pno_state->pno_params_arr[INDEX_OF_BATCH_PARAMS];
	if (buf && bufsize) {
		if (!list_empty(&_params->params_batch.get_batch.expired_scan_results_list)) {
			...
			AP6211_DEBUG("%s: have cashed batching data in Driver\n",
				__FUNCTION__);
	...
	if (pscan_results == NULL) {
		err = BCME_NOMEM;
		AP6211_ERR("failed to allocate dhd_pno_scan_results_t\n");
		goto exit;
	}
	...
	if (_params->params_batch.get_batch.top_node_cnt < MAX_NODE_CNT) {
		...
		AP6211_DEBUG("%s : Remove oldest node and add new one\n", __FUNCTION__);
	...
	while (plbestnet->status != PFN_COMPLETE) {
		...
		if (err < 0) {
			if (err == BCME_EPERM) {
				AP6211_ERR("we cannot get the batching data "
					"during scanning in firmware, try again\n,");
				msleep(500);
				continue;
			} else {
				AP6211_ERR("%s : failed to execute pfnlbest (err :%d)\n",
					__FUNCTION__, err);
				goto exit;
			}
		}
		AP6211_DEBUG("ver %d, status : %d, count %d\n", plbestnet->version,
			plbestnet->status, plbestnet->count);
		if (plbestnet->version != PFN_SCANRESULT_VERSION) {
			err = BCME_VERSION;
			AP6211_ERR("bestnet version(%d) is mismatch with Driver version(%d)\n",
		...
		for (i = 0; i < plbestnet->count; i++) {
			...
			if (pbestnet_entry == NULL) {
				err = BCME_NOMEM;
				AP6211_ERR("failed to allocate dhd_pno_bestnet_entry\n");
				goto exit;
			}
			...
			if (plnetinfo->flags & PFN_PARTIAL_SCAN_MASK) {
				 ...
				 */
				AP6211_DEBUG("This scan is aborted\n");
...
convert_format:
		...
		if (err < 0) {
			AP6211_ERR("failed to convert the data into upper layer format\n");
			goto exit;
		}
	}
exit:
...
dhd_pno_stop_for_batch(dhd_pub_t *dhd)
{
	...
	if (!(_pno_state->pno_mode & DHD_PNO_BATCH_MODE)) {
		AP6211_ERR("%s : PNO BATCH MODE is not enabled\n", __FUNCTION__);
		goto exit;
	}
	_pno_state->pno_mode &= ~DHD_PNO_BATCH_MODE;
	if (_pno_state->pno_mode & (DHD_PNO_LEGACY_MODE | DHD_PNO_HOTLIST_MODE)) {
		...
		if (_pno_state->pno_mode & DHD_PNO_LEGACY_MODE) {
			...
			if (err < 0) {
				_pno_state->pno_mode &= ~DHD_PNO_LEGACY_MODE;
				AP6211_ERR("%s : failed to restart legacy PNO scan(err: %d)\n",
					__FUNCTION__, err);
				goto exit;
			}
		} else if (_pno_state->pno_mode & DHD_PNO_HOTLIST_MODE) {
...
#ifdef PNO_DEBUG
{
	...
	if (err < 0) {
		AP6211_ERR("%s : failed to call _dhd_pno_add_bssid(err :%d)\n",
			__FUNCTION__, err);
		goto exit;
	}
...
dhd_pno_stop_for_hotlist(dhd_pub_t *dhd)
{
	...
	if (!(_pno_state->pno_mode & DHD_PNO_HOTLIST_MODE)) {
		AP6211_ERR("%s : Hotlist MODE is not enabled\n",
			__FUNCTION__);
		goto exit;
	}
	_pno_state->pno_mode &= ~DHD_PNO_BATCH_MODE;
...
dhd_pno_event_handler(dhd_pub_t *dhd, wl_event_msg_t *event, void *event_data)
{
	...
	AP6211_DEBUG("%s enter : event_type :%d\n", __FUNCTION__, event_type);
	switch (event_type) {
	...
	case WLC_E_PFN_BEST_BATCHING:
	{
		struct dhd_pno_batch_params *params_batch;
		params_batch = &_pno_state->pno_params_arr[INDEX_OF_BATCH_PARAMS].params_batch;
		AP6211_DEBUG("%s : WLC_E_PFN_BEST_BATCHING\n", __FUNCTION__);
...
int dhd_pno_init(dhd_pub_t *dhd)
{
	...
	if (err == BCME_UNSUPPORTED) {
		...
		AP6211_DEBUG("Current firmware doesn't support"
