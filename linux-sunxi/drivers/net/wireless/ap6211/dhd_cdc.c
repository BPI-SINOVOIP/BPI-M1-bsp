...
dhdcdc_query_ioctl(dhd_pub_t *dhd, int ifidx, uint cmd, void *buf, uint len, uint8 action)
{
	...
	if ((ret = dhdcdc_msg(dhd)) < 0) {
		...
		AP6211_ERR("dhdcdc_query_ioctl: dhdcdc_msg failed w/status %d\n", ret);
		goto done;
	}
...
retry:
	...
	if ((id < prot->reqid) && (++retries < RETRIES))
		goto retry;
	if (id != prot->reqid) {
		AP6211_ERR("%s: %s: unexpected request id %d (expected %d)\n",
...
dhdcdc_set_ioctl(dhd_pub_t *dhd, int ifidx, uint cmd, void *buf, uint len, uint8 action)
{
	...
	if (dhd->hang_was_sent) {
		AP6211_ERR("%s: HANG was sent up earlier. Not talking to the chip\n",
			__FUNCTION__);
		return -EIO;
	}
	...
	if ((ret = dhdcdc_msg(dhd)) < 0) {
		AP6211_ERR("%s: dhdcdc_msg failed w/status %d\n", __FUNCTION__, ret);
		goto done;
	}
...
dhd_prot_ioctl(dhd_pub_t *dhd, int ifidx, wl_ioctl_t * ioc, void * buf, int len)
{
	...
	if (prot->pending == TRUE) {
		AP6211_ERR("CDC packet is pending!!!! cmd=0x%x (%lu) lastcmd=0x%x (%lu)\n",
			ioc->cmd, (unsigned long)ioc->cmd, prot->lastcmd,
			(unsigned long)prot->lastcmd);
		if ((ioc->cmd == WLC_SET_VAR) || (ioc->cmd == WLC_GET_VAR)) {
			AP6211_DEBUG("iovar cmd=%s\n", (char*)buf);
		}
		goto done;
	}
...
_dhd_wlfc_pullheader(athost_wl_status_info_t* ctx, void* pktbuf)
{
	...
	if (PKTLEN(ctx->osh, pktbuf) < BDC_HEADER_LEN) {
		AP6211_DEBUG("%s: rx data too short (%d < %d)\n", __FUNCTION__,
...
_dhd_wlfc_flow_control_check(athost_wl_status_info_t* ctx, struct pktq* pq, uint8 if_id)
{
	if ((pq->len <= WLFC_FLOWCONTROL_LOWATER) && (ctx->hostif_flow_state[if_id] == ON)) {
		...
		/*
		AP6211_DEBUG("qlen:%02d, if:%02d, ->OFF, start traffic %s()\n",
	...
	if ((pq->len >= WLFC_FLOWCONTROL_HIWATER) && (ctx->hostif_flow_state[if_id] == OFF)) {
		...
		/*
		AP6211_DEBUG("qlen:%02d, if:%02d, ->ON, stop traffic   %s()\n",
		pq->len, if_id, __FUNCTION__);
		*/
		AP6211_DEBUG("N");
		...
		if (rc == BCME_OK) {
			...
			*/
			...
			if (rc == BCME_OK) {
				...
				AP6211_DEBUG("%s() hanger_pushpkt() failed, rc: %d\n",
					__FUNCTION__, rc);
			}
		}
	}
	...
	for (ac = AC_COUNT; ac >= 0; ac--) {
		...
		for (credit = 0; credit < ctx->FIFO_credit[ac];) {
			...
			if (rc == BCME_OK) {
				...
				bus_retry_count++;
				if (bus_retry_count >= BUS_RETRIES) {
					AP6211_ERR("dhd_wlfc_commit_packets(): bus error\n");
...
dhd_wlfc_txcomplete(dhd_pub_t *dhd, void *txp, bool success, bool wake_locked)
{
	...
	if (!success) {
		AP6211_DEBUG("At: %s():%d, bus_complete() failure for %p, htod_tag:0x%08x\n",
...
dhd_wlfc_dbg_senum_check(dhd_pub_t *dhd, uint8 *value)
{
	...
	AP6211_DEBUG("RXPKT: SEQ: %d, timestamp %d\n", value[1], timestamp);
	return BCME_OK;
}
...
dhd_wlfc_mac_table_update(dhd_pub_t *dhd, uint8* value, uint8 type)
{
	...
	AP6211_DEBUG("%s(), mac [%02x:%02x:%02x:%02x:%02x:%02x],%s,idx:%d,id:0x%02x\n",
	...
	if (if_id < WLFC_MAX_IFNUM) {
		...
		if (table[if_id].occupied) {
			if (type == WLFC_CTL_TYPE_INTERFACE_OPEN) {
				...
				/* AP6211_DEBUG("INTERFACE[%d] OPEN\n", if_id); */
			}
			else {
				table[if_id].state = WLFC_STATE_CLOSE;
				/* AP6211_DEBUG("INTERFACE[%d] CLOSE\n", if_id); */
			}
			return BCME_OK;
		}
	}
...
dhd_wlfc_init(dhd_pub_t *dhd)
{
	...
	*/
	...
	if (dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0) < 0) {
		AP6211_ERR("dhd_wlfc_init(): failed to enable/disable bdcv2 tlv signaling\n");
	}
	else {
		/*
		...
		*/
		AP6211_DEBUG("dhd_wlfc_init(): successfully %s bdcv2 tlv signaling, %d\n",
	...
	uint *reorder_info_len)
{
	...
	if (PKTLEN(dhd->osh, pktbuf) < BDC_HEADER_LEN) {
		AP6211_ERR("%s: rx data too short (%d < %d)\n", __FUNCTION__,
	...
	if ((*ifidx = BDC_GET_IF_IDX(h)) >= DHD_MAX_IFS) {
		AP6211_ERR("%s: rx data ifnum out of range (%d)\n",
		           __FUNCTION__, *ifidx);
		return BCME_ERROR;
	}

	if (((h->flags & BDC_FLAG_VER_MASK) >> BDC_FLAG_VER_SHIFT) != BDC_PROTO_VER) {
		AP6211_ERR("%s: non-BDC packet received, flags = 0x%x\n",
	...
	if (h->flags & BDC_FLAG_SUM_GOOD) {
		AP6211_DEBUG("%s: BDC packet received with good rx-csum, flags 0x%x\n",
		          dhd_ifname(dhd, *ifidx), h->flags);
...
dhd_prot_attach(dhd_pub_t *dhd)
{
	...
	if ((uintptr)(&cdc->msg + 1) != (uintptr)cdc->buf) {
		AP6211_ERR("dhd_prot_t is not correctly defined\n");
		goto fail;
	}
	...
	uint32 *pkt_count, void **pplast, uint8 start, uint8 end)
{
	...
	if (ptr->pend_pkts == 0) {
		AP6211_DEBUG("%s: no packets in reorder queue \n", __FUNCTION__);
		*pplast = NULL;
		*pkt_count = 0;
		*pkt = NULL;
		return;
	...
	void **pkt, uint32 *pkt_count)
{
	...
	AP6211_DEBUG("flow_id %d, flags 0x%02x, idx(%d, %d, %d)\n", flow_id, flags,
	...
	if (flags == 0xFF) {
		AP6211_ERR("%s: invalid flags...so ignore this packet\n", __FUNCTION__);
		*pkt_count = 1;
		return 0;
	}
	...
	*pkt = NULL;
	...
	if (flags & WLHOST_REORDERDATA_DEL_FLOW) {
		...
		AP6211_DEBUG("%s: Flags indicating to delete a flow id %d\n",
			__FUNCTION__, flow_id);

		if (ptr == NULL) {
			AP6211_ERR("%s: received flags to cleanup, but no flow (%d) yet\n",
				__FUNCTION__, flow_id);
			*pkt_count = 1;
			*pkt = cur_pkt;
			return 0;
		...
		if (plast) {
			...
			if (cnt != 0) {
				AP6211_ERR("%s: del flow: something fishy, pending packets %d\n",
					__FUNCTION__, cnt);
			}
			*pkt = cur_pkt;
			cnt = 1;
	...
	if (ptr == NULL) {
		...
		AP6211_DEBUG("%s: alloc buffer of size %d size, reorder info id %d, maxidx %d\n",
			__FUNCTION__, buf_size_alloc, flow_id, max_idx);
		ptr = (struct reorder_info *)MALLOC(dhd->osh, buf_size_alloc);
		if (ptr == NULL) {
			AP6211_ERR("%s: Malloc failed to alloc buffer\n", __FUNCTION__);
			*pkt_count = 1;
			return 0;
		}
	...
	if (flags & WLHOST_REORDERDATA_NEW_HOLE)  {
		AP6211_DEBUG("%s: new hole, so cleanup pending buffers\n", __FUNCTION__);
		if (ptr->pend_pkts) {
			dhd_get_hostreorder_pkts(dhd->osh, ptr, pkt, &cnt, &plast,
				ptr->exp_idx, ptr->exp_idx);
			ptr->pend_pkts = 0;
		...
		*pkt_count = cnt;
		...
		if ((exp_idx == ptr->exp_idx) && (cur_idx != ptr->exp_idx)) {
			...
			if (ptr->p[cur_idx] != NULL) {
				AP6211_DEBUG("%s: HOLE: ERROR buffer pending..free it\n",
					__FUNCTION__);
			...
			ptr->pend_pkts++;
			ptr->cur_idx = cur_idx;
			AP6211_DEBUG("%s: fill up a hole..pending packets is %d\n",
			...
			*pkt = NULL;
			...
			AP6211_DEBUG("%s: got the right one now, cur_idx is %d\n",
				__FUNCTION__, cur_idx);
			if (ptr->p[cur_idx] != NULL) {
				AP6211_DEBUG("%s: Error buffer pending..free it\n",
					__FUNCTION__);
				PKTFREE(dhd->osh, ptr->p[cur_idx], TRUE);
				ptr->p[cur_idx] = NULL;
			}
			...
			*pkt_count = cnt;
			AP6211_DEBUG("%s: freeing up buffers %d, still pending %d\n",
			...
			AP6211_DEBUG("%s:, flow %d, both moved, cur %d(%d), exp %d(%d)\n",
