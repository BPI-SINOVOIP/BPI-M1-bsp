	...
	s8 *iovar_buf, s32 buflen, s32 bssidx)
{
	...
	{
		AP6211_ERR("%s: buffer is too short\n", __FUNCTION__);
	...
	struct net_device *dev, char *country_code)
{
	...
	if (error < 0)
		AP6211_ERR("%s: get country failed = %d\n", __FUNCTION__, error);
	...
	if ((error < 0) ||
		...
		if (error < 0) {
			AP6211_ERR("%s: set country failed due to Disassoc error %d\n",
				__FUNCTION__, error);
			return error;
		}
		cspec.rev = -1;
		...
		error = wldev_iovar_setbuf(dev, "country", &cspec, sizeof(cspec),
			smbuf, sizeof(smbuf), NULL);
		if (error < 0) {
			AP6211_ERR("%s: set country for %s as %s rev %d failed\n",
				__FUNCTION__, country_code, cspec.ccode, cspec.rev);
			return error;
		}
		dhd_bus_country_set(dev, &cspec);
		AP6211_ERR("%s: set country for %s as %s rev %d\n",
			__FUNCTION__, country_code, cspec.ccode, cspec.rev);
	}
	return 0;
}
