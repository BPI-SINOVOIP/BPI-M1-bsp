                       ...
                       uint bustype, void *sdh, char **vars, uint *varsz)
{
	...
	if ((sii = MALLOC(osh, sizeof (si_info_t))) == NULL) {
		AP6211_ERR("si_attach: malloc failed! malloced %d bytes\n", MALLOCED(osh));
		return (NULL);
	}
...
si_kattach(osl_t *osh)
{
	...
	if (!ksii_attached) {
		...
		if (si_doattach(&ksii, BCM4710_DEVICE_ID, osh, regs,
			...
			AP6211_ERR("si_kattach: si_doattach failed\n");
			REG_UNMAP(regs);
			return NULL;
		}
		REG_UNMAP(regs);
		...
		ksii_attached = TRUE;
		AP6211_DEBUG("si_kattach done. ccrev = %d, wd_msticks = %d\n",
		...
		if (!err) {
			...
			if ((clkval & ~SBSDIO_AVBITS) == clkset) {
				...
				if (!SBSDIO_ALPAV(clkval)) {
					AP6211_ERR("timeout on ALPAV wait, clkval 0x%02x\n",
						clkval);
					return FALSE;
				}
	...
	uint *origidx, void *regs)
{
	...
	AP6211_DEBUG("Chipc: rev %d, caps 0x%x, chipst 0x%x pmurev %d, pmucaps 0x%x\n",
	...
	for (i = 0; i < sii->numcores; i++) {
		...
		/* Display cores found */
		AP6211_DEBUG("CORE[%d]: id 0x%x rev %d base 0x%x regs 0x%p\n",
	...
	AP6211_DEBUG("Buscore id/type/rev %d/0x%x/%d\n", sii->pub.buscoreidx, sii->pub.buscoretype,
	         sii->pub.buscorerev);
                       ...
                       uint bustype, void *sdh, char **vars, uint *varsz)
{
	...
	if (bustype != BUSTYPE(bustype)) {
		AP6211_ERR("si_doattach: bus type %d does not match configured bus type %d\n",
	...
	if (!si_buscore_prep(sii, bustype, devid, sdh)) {
		AP6211_ERR("si_doattach: si_core_clk_prep failed %d\n", bustype);
		return NULL;
	}
	 ...
	 */
	if (!cc) {
		AP6211_ERR("%s: chipcommon register space is null \n", __FUNCTION__);
		return NULL;
	}
	...
	if (CHIPTYPE(sii->pub.socitype) == SOCI_SB) {
		AP6211_DEBUG("Found chip type SB (0x%08x)\n", w);
		sb_scan(&sii->pub, regs, devid);
	} else if (CHIPTYPE(sii->pub.socitype) == SOCI_AI) {
		AP6211_DEBUG("Found chip type AI (0x%08x)\n", w);
		/* pass chipc address instead of original core base */
		ai_scan(&sii->pub, (void *)(uintptr)cc, devid);
	} else if (CHIPTYPE(sii->pub.socitype) == SOCI_UBUS) {
		AP6211_DEBUG("Found chip type UBUS (0x%08x), chip id = 0x%4x\n", w, sih->chip);
		/* pass chipc address instead of original core base */
		ub_scan(&sii->pub, (void *)(uintptr)cc, devid);
	} else {
		AP6211_ERR("Found chip of unknown type (0x%08x)\n", w);
		return NULL;
	}
	/* no cores found, bail out */
	if (sii->numcores == 0) {
		AP6211_ERR("si_doattach: could not find any cores\n");
		return NULL;
	}
	/* bus/core/clk setup */
	origidx = SI_CC_IDX;
	if (!si_buscore_setup(sii, cc, bustype, savewin, &origidx, regs)) {
		AP6211_ERR("si_doattach: si_buscore_setup failed\n");
		goto exit;
	}
	...
	if (CHIPID(sih->chip) == BCM4322_CHIP_ID && (((sih->chipst & CST4322_SPROM_OTP_SEL_MASK)
		...
		AP6211_ERR("%s: Invalid setting: both SPROM and OTP strapped.\n", __FUNCTION__);
		return NULL;
	}
