...
static int ctp_fetch_sysconfig_para(void)
{
	...
	if(SCRIPT_PARSER_OK != script_parser_fetch("ctp_para", "ctp_exchange_x_y_flag", &exchange_x_y_flag, 1)){
		pr_err("gt9xx: script_parser_fetch err. \n");
		goto script_parser_fetch_err;
	}
...
static int ctp_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	GTP_INFO("%s, twi_id = %d, adapter->nr = %d", __func__, twi_id, adapter->nr);
...
void gtp_gpio_as_input(u32 p_handler, const char *gpio_name)
{
	gpio_set_one_pin_io_status(p_handler, 0, gpio_name);
}

void gtp_gpio_as_output(u32 p_handler, __u32 value, const char *gpio_name)
{
...
void gtp_gpio_free(u32 p_handler)
{
...
static void goodix_ts_work_func(struct work_struct *work)
{
	...
	//GTP_INFO("%s(%d): finger = %x", __func__, __LINE__, finger);
	...
	//GTP_INFO("%s(%d): finger = %d", __func__, __LINE__, touch_num);
    ...
    //GTP_DEBUG("%s(%d)pre_touch:%02x, finger:%02x.", __func__, __LINE__, pre_touch, finger);
...
void gtp_int_sync(s32 ms)
{
    gtp_gpio_as_output(gpio_wakeup_hdle, 0, "ctp_wakeup");
    msleep(ms);
    gtp_gpio_as_int();
}
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    msleep(ms);                         // T2: > 10ms
    // HIGH: 0x28/0x29, LOW: 0xBA/0xBB
    gtp_gpio_as_output(gpio_wakeup_hdle, client->addr == 0x14, "ctp_wakeup");
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ...
        gtp_gpio_as_output(gpio_wakeup_hdle, 1, "ctp_wakeup");
        msleep(5);
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	gtp_gpio_as_input(gpio_reset_hdle, "ctp_reset");
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
    ...
    if(0 != ret){
		GTP_ERROR("set gpio irq mode error\n");
		return ret;
    }

	ret  = request_irq(SW_INT_IRQNO_PIO, goodix_ts_irq_handler, IRQF_SHARED, "gt9xx", ts);
    if (ret != 0){
		GTP_ERROR("goodix_probe: request irq failed, using polling mode\n");
		gtp_gpio_as_input(gpio_int_hdle, "ctp_int_port");
        gpio_release(gpio_int_hdle, 2);
...
#endif			/* use gpio defined in gt9xx.h */
